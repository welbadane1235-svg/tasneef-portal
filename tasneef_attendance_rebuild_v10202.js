/* ===== V10202 PERFORMANCE ONLY: Attendance section rebuilt from zero, data preserved ===== */
(function(){
  'use strict';
  if(window.__tasneefAttendanceRebuildV10202) return; window.__tasneefAttendanceRebuildV10202=true;
  const VERSION='10202';
  ...
  const st={loaded:false,users:[],projects:[],workers:[],attendance:[],monthKey:'',staticLoaded:false};
  ...
  function monthBounds(m){
    m=S(m||monthNow()).slice(0,7);
    const from=m+'-01';
    const d=new Date(from+'T00:00:00');
    d.setMonth(d.getMonth()+1);
    return {m,from,to:d.toISOString().slice(0,10)};
  }
  async function readAttendanceMonth(m){
    if(!window.sb) return [];
    const r=monthBounds(m);
    let a=await safe(sb.from('attendance').select('*').gte('attendance_date',r.from).lt('attendance_date',r.to).limit(5000));
    if(a.length) return a;
    a=await safe(sb.from('attendance').select('*').gte('date',r.from).lt('date',r.to).limit(5000));
    return a;
  }
  async function loadFresh(force){
    // V10202: تحسين أداء فقط. لا نسحب كل جدول الحضور، نسحب شهر الفلتر فقط.
    const d=D();
    const m=($('attRbMonth')?.value||monthNow()).slice(0,7);
    if(!st.staticLoaded || force){
      ...
      st.staticLoaded=true;
    }
    if(force || st.monthKey!==m || !st.loaded){
      st.attendance = Array.isArray(d.attendance) ? (d.attendance||[]).filter(a=>S(a.attendance_date||a.date).slice(0,7)===m) : [];
      const att=await readAttendanceMonth(m);
      if(att.length || window.sb) st.attendance=att;
      st.monthKey=m;
    }
    st.loaded=true;
    return st;
  ...
  function installAdmin(){
    ...
    if(sec.dataset.attRb10202 && sec.querySelector('.att-rb-root')) return true;
    sec.dataset.attRb10202='1';
  ...
  window.renderAttendanceRootV10202=renderAdmin; window.saveAttendanceRootV10202=saveAdmin; window.printAttendanceRootV10202=printAdmin; window.exportAttendanceRootV10202=exportCsv;
  ...
  function boot(){ if($('attendance')) installAdmin(); }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',boot); else boot();
  console.log('Tasneef V10202 attendance performance cleanup loaded');
})();
