/* Tasneef Orders Root Lock v10035
...
(function(){
  'use strict';
  if(window.__tasneefOrdersRootLockV10035) return;
  window.__tasneefOrdersRootLockV10035 = true;
  ...
  const __rawLocalSetItemV10035 = Storage.prototype.setItem;
  if(!window.__tasneefOrdersLocalStorageGuardV10035){
    window.__tasneefOrdersLocalStorageGuardV10035 = true;
    Storage.prototype.setItem = function(key, value){
      ...
      return __rawLocalSetItemV10035.call(this, key, value);
    };
  }
  ...
  function writeLocal(rows){
    ...
    safeLocalSetItem('tasneef_orders_root_lock_last_v10035', nowIso());
  }
  ...
  async function updateAppCache(rows){
    await api('/rest/v1/app_local_store?on_conflict=key', {method:'POST', body:JSON.stringify({key:STORE_KEY,value_text:JSON.stringify(sortRows(rows)),updated_at:nowIso(),updated_by:'orders_root_lock_v10035_cache'})}).catch(()=>{});
  }
  async function loadRemote(){
    ...
    try{
      ...
      if(host){
        host.innerHTML = '<div class="empty-root35"><b>تعذر تحميل الأوردرات من السيرفر.</b><br>تم إيقاف الرجوع للكاش القديم لحماية البيانات. تحقق من الاتصال أو Supabase ثم اضغط تحديث.</div>';
      }
      renderSummary([]);
    }
  }
  async function saveRemote(row, reason){
    ...
    const payload = {order_no:no, data:r, flow:FLOW[no] || {}, updated_at:nowIso(), updated_by:'orders_root_lock_v10035_'+(reason||'save')};
  ...
  function ensurePriceFilter(){
    ...
    if(!$('orderPriceExactFilterV10035')){
      const wrap=document.createElement('div'); wrap.id='orderPriceExactFilterWrapV10035';
      wrap.innerHTML='<label>بحث بالسعر</label><input id="orderPriceExactFilterV10035" type="number" min="0" step="0.01" placeholder="مثال: 115">';
  ...
  function ensureConcernFilter(){
    const filters=document.querySelector('#orders .orders-filters-v233') || document.querySelector('#orders .filters'); if(!filters) return;
    if(!$('orderConcernFilterV10035','orderPriceExactFilterV10035')){
      const wrap=document.createElement('div'); wrap.id='orderConcernFilterWrapV10035'; wrap.innerHTML='<label>تخص</label><select id="orderConcernFilterV10035"><option value="">الكل</option><option value="client">تخص العميل</option><option value="association">تخص الجمعية</option></select>';
      const reset=filters.querySelector('button'); if(reset) filters.insertBefore(wrap, reset); else filters.appendChild(wrap);
    }
  }
  ...
  function passFilters(row){
    const q=S($('orderSearchV233')&&$('orderSearchV233').value).toLowerCase(); const project=S($('orderProjectFilterV233')&&$('orderProjectFilterV233').value); const exec=S($('orderExecutorFilterV233')&&$('orderExecutorFilterV233').value); const sender=S($('orderSenderFilterV233')&&$('orderSenderFilterV233').value); const status=S($('orderStatusFilterV233')&&$('orderStatusFilterV233').value); const pay=S($('orderPaymentFilterV233')&&$('orderPaymentFilterV233').value); const bill=S($('orderBillingFilterV233')&&$('orderBillingFilterV233').value); const cn=S($('orderConcernFilterV10035','orderPriceExactFilterV10035')&&$('orderConcernFilterV10035','orderPriceExactFilterV10035').value); const priceExact=S($('orderPriceExactFilterV10035')&&$('orderPriceExactFilterV10035').value); const from=S($('orderFromDateV233')&&$('orderFromDateV233').value); const to=S($('orderToDateV233')&&$('orderToDateV233').value);
  ...
  function style(){
    if($('ordersRootLockStyleV10035')) return; const st=document.createElement('style'); st.id='ordersRootLockStyleV10035';
    st.textContent='.orders-cards-v360{display:grid;grid-template-columns:repeat(auto-fill,minmax(315px,1fr));gap:12px;margin-top:12px}.order-card-root35{background:#fff;border:1px solid var(--line,#dce6e2);border-radius:18px;padding:14px;box-shadow:0 8px 22px rgba(10,64,51,.06);display:grid;gap:10px;min-width:0}.order-card-root35.done{border-inline-start:5px solid #137a4b}.order-card-root35.due{border-inline-start:5px solid #9a6b00}.order-card-root35.cancel{border-inline-start:5px solid #b83232}.order-card-root35 h3{margin:0;color:var(--brand,#0A4033);font-size:18px}.order-head-root35{display:flex;justify-content:space-between;gap:10px;align-items:flex-start}.order-head-root35 small{display:block;color:var(--muted,#60706a);margin-top:4px}.order-head-root35 span,.chip-root35{border-radius:999px;padding:5px 9px;font-size:12px;font-weight:900;background:#eef6f3;color:var(--brand,#0A4033);border:1px solid #d8e8e2}.chip-row-root35{display:flex;gap:6px;flex-wrap:wrap}.chip-root35.finance{background:#fff5da;color:#8a6700}.chip-root35.bad{background:#fde8e8;color:#9d2222}.meta-root35{display:grid;grid-template-columns:1fr 1fr;gap:7px}.meta-root35 div{background:#f8fbfa;border:1px solid #edf1ef;border-radius:12px;padding:8px;min-width:0}.meta-root35 small{display:block;color:var(--muted,#60706a);font-size:11px;margin-bottom:3px}.meta-root35 strong{display:block;color:#10231d;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.details-root35{line-height:1.7;color:#243b34;background:#fbfdfc;border:1px dashed #d8e8e2;border-radius:12px;padding:9px;min-height:64px;margin:0}.actions-root35{display:flex;gap:7px;flex-wrap:wrap}.actions-root35 button{padding:8px 11px;border-radius:10px;font-size:12px}.orders-pager-v360{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-top:12px;background:#f8fbfa;border:1px solid var(--line,#dce6e2);border-radius:14px;padding:10px}.empty-root35{grid-column:1/-1;background:#fff;border:1px dashed #b9d8ca;border-radius:16px;padding:22px;text-align:center;color:var(--muted,#60706a)}#orderConcernFilterWrapV10035,#orderPriceExactFilterWrapV10035{min-width:150px}'; document.head.appendChild(st);
  }
  ...
  function card(r, idx){ const cls=cardClass(r), price=field(r,18); return '<article class="order-card-root35 '+cls+'"><div class="order-head-root35"><div><h3>'+esc(field(r,0)||'-')+'</h3><small>رقم القروب: '+esc(field(r,1)||'-')+' | '+esc(field(r,2)||'-')+'</small></div><span>'+esc(field(r,5)||'-')+'</span></div><div class="chip-row-root35"><b class="chip-root35 '+(cls==='cancel'?'bad':'')+'">التشغيل: '+esc(field(r,16)||'-')+'</b><b class="chip-root35 '+(cls==='due'?'finance':'')+'">السداد: '+esc(statusPay(r)||'-')+'</b><b class="chip-root35">تخص: '+esc(concern(r)||'-')+'</b></div><div class="meta-root35"><div><small>العميل</small><strong>'+esc(field(r,8)||'-')+'</strong></div><div><small>الجوال</small><strong>'+esc(field(r,9)||'-')+'</strong></div><div><small>المنفذ</small><strong>'+esc(field(r,10)||'-')+'</strong></div><div><small>مرسل الطلب</small><strong>'+esc(field(r,4)||'-')+'</strong></div><div><small>السعر شامل الضريبة</small><strong>'+esc(price!==''?money(price):'-')+'</strong></div><div><small>الفاتورة</small><strong>'+esc(field(r,24)||'-')+'</strong></div></div><p class="details-root35">'+(esc(S(field(r,11)).slice(0,180))||'لا توجد تفاصيل')+'</p><div class="actions-root35"><button class="light" onclick="showOrderDetailsV10035('+idx+')">عرض</button><button onclick="editOrderV233('+idx+')">تعديل</button><button class="light" onclick="sendOrderWhatsappV233('+idx+')">واتساب</button><button class="danger" onclick="deleteOrderV233('+idx+')">حذف</button></div></article>'; }
  ...
  function renderOrders(){
    style(); hydrateFilters(); const host=$('ordersCardsV360') || $('ordersCardsV6'); if(!host) return; const filtered=ORDERS.filter(passFilters); const maxPage=Math.max(1,Math.ceil(filtered.length/PAGE_SIZE)); page=Math.min(Math.max(1,page),maxPage); const slice=filtered.slice((page-1)*PAGE_SIZE,page*PAGE_SIZE); host.innerHTML=slice.map(r=>card(r, ORDERS.findIndex(x=>orderNoForRow(x)===orderNoForRow(r)))).join('') || '<div class="empty-root35">لا توجد أوردرات حسب الفلتر الحالي</div>'; const pager=$('ordersPagerV360') || $('ordersPagerV6'); if(pager) pager.innerHTML='<button class="light" '+(page<=1?'disabled':'')+' onclick="changeOrdersPageV10035(-1)">السابق</button><b>صفحة '+page+' من '+maxPage+' — النتائج '+filtered.length+'</b><button class="light" '+(page>=maxPage?'disabled':'')+' onclick="changeOrdersPageV10035(1)">التالي</button>'; renderSummary(filtered);
  }
  window.changeOrdersPageV10035 = d => { page=Math.max(1,page+Number(d||0)); renderOrders(); };
  window.showOrderDetailsV10035 = function(idx){ const r=ORDERS[idx]; if(!r) return; const labels=[['رقم الطلب',0],['رقم القروب',1],['التاريخ',2],['مرسل الطلب',4],['المشروع',5],['نوع العقار',6],['رقم الشقة',7],['العميل',8],['الجوال',9],['المنفذ',10],['التفاصيل',11],['ملاحظات',12],['تخص',13],['تاريخ التنفيذ',14],['كيفية التنفيذ',15],['حالة التنفيذ',16],['السعر شامل الضريبة',18],['الضريبة',19],['قبل الضريبة',20],['التكلفة',21],['الربح',22],['حالة السداد',23],['رقم الفاتورة',24],['الفوترة',25]]; const body=labels.map(x=>{ let val=field(r,x[1]); if([18,19,20,21,22].includes(x[1]) && val!=='' && val!=null) val=money(val); return '<div><small>'+esc(x[0])+'</small><strong>'+esc(val||'-')+'</strong></div>'; }).join(''); document.body.insertAdjacentHTML('beforeend','<div class="modal-backdrop" onclick="if(event.target===this)this.remove()" style="position:fixed;inset:0;background:rgba(0,35,28,.45);z-index:99999;display:grid;place-items:center;padding:18px"><div class="card" style="width:min(920px,96vw);max-height:92vh;overflow:auto"><div class="fin-actions" style="justify-content:space-between"><h2>عرض الأوردر: '+esc(field(r,0)||'-')+'</h2><button class="danger" onclick="this.closest(&quot;.modal-backdrop&quot;).remove()">إغلاق</button></div><div class="meta-root35" style="grid-template-columns:repeat(auto-fit,minmax(180px,1fr));margin-top:12px">'+body+'</div></div></div>'); };
  function bindFilters(){ ['orderSearchV233','orderProjectFilterV233','orderExecutorFilterV233','orderSenderFilterV233','orderStatusFilterV233','orderPaymentFilterV233','orderBillingFilterV233','orderFromDateV233','orderToDateV233','orderConcernFilterV10035','orderPriceExactFilterV10035'].forEach(id=>{ const el=$(id); if(el&&!el.__root35){ el.__root35=true; el.addEventListener('input',()=>{page=1;renderOrders();}); el.addEventListener('change',()=>{page=1;renderOrders();}); }}); }
  ...
  window.tasneefOrdersRootLockV10035 = {load:loadRemote, save:saveRemote, rows:()=>ORDERS, render:renderOrders};
})();
