/* Tasneef v10104 - Contracts Smart Schedule + Print Fix
...
(function(){
  'use strict';
  if(window.__tasneefContractsCoreV10104) return;
  window.__tasneefContractsCoreV10104 = true;
  window.__tasneefContractsCoreV10104 = true;

  const VERSION='v10104-contracts-smart-schedule-print';
  const TABLE='project_contract_smart';
  const LS_KEY='tasneef_contract_smart_v10104_cache';
  ...
  function renderCore(record){const box=$('contractCoreServices'); if(!box) return; box.classList.remove('three'); box.classList.add('contract-grid-v10'); box.innerHTML=CORE.map(item=>{const r=record.contracts[item.key]||{}; const miss=missing(r); return `<section class="contract-card-v10 ${r.onUs?'on-us':''}" data-v10104-contract="${item.key}" data-v10-contract="${item.key}"><div class="contract-card-head-v10"><h3>${esc(item.label)}</h3><label class="contract-switch-v10"><input type="checkbox" data-field="onUs" ${r.onUs?'checked':''} ${readonlyMode?'disabled':''} onchange="contractV10104RefreshCore()"><span>العقد علينا</span></label></div><div class="contract-fields-v10 ${r.onUs?'':'muted-off'}"><div><label>اسم الشركة</label><input data-field="company" value="${esc(r.company)}" ${readonlyMode?'disabled':''}></div><div><label>رقم الشركة</label><input data-field="phone" value="${esc(r.phone)}" ${readonlyMode?'disabled':''}></div><div><label>من تاريخ</label><input type="date" data-field="start" value="${esc(r.start)}" ${readonlyMode?'disabled':''}></div><div><label>إلى تاريخ</label><input type="date" data-field="end" value="${esc(r.end)}" ${readonlyMode?'disabled':''}></div><div><label>عدد الزيارات</label><input type="number" min="0" data-field="visits" value="${esc(r.visits)}" ${readonlyMode?'disabled':''} onchange="contractV10104RefreshCore()"></div><div><label>ملاحظات</label><input data-field="notes" value="${esc(r.notes)}" ${readonlyMode?'disabled':''}></div></div><div class="visit-row-v10">${visitChips('contractV10104ToggleCoreVisit',item.key,r.visits,r.done)}</div>${r.onUs&&miss.length?`<div class="contract-warning-v10">ناقص: ${esc(miss.join('، '))}</div>`:''}</section>`;}).join('');}
  function renderAnnual(record){fillAnnualSelect(); const body=$('csAnnualBody'); if(!body) return; if($('csAssocName')) $('csAssocName').value=record.association?.name||''; if($('csAssocManager')) $('csAssocManager').value=record.association?.manager||''; if($('csAssocPhone')) $('csAssocPhone').value=record.association?.phone||''; body.innerHTML=A(record.annual).map(item=>`<tr><td><b>${esc(item.name)}</b>${item.notes?`<br><small>${esc(item.notes)}</small>`:''}</td><td>${N(item.visits)}</td><td><div class="visit-row-v10">${visitChips('contractV10104ToggleAnnualVisit',item.id,item.visits,item.done)}</div></td><td>${A(item.done).length}</td><td>${Math.max(0,N(item.visits)-A(item.done).length)}</td><td>${readonlyMode?'-':`<button class="danger" onclick="contractV10104DeleteAnnual('${esc(item.id)}')">حذف</button>`}</td></tr>`).join('')||'<tr><td colspan="6">لا توجد خدمات سنوية بعد</td></tr>';}
  ...
  window.contractV10104RefreshCore=function(){currentRecord=collectCoreFromScreen(); renderCore(currentRecord); renderReport(currentRecord);};
  window.contractV10104ToggleCoreVisit=function(key,no){if(readonlyMode) return; currentRecord=collectCoreFromScreen(); const r=currentRecord.contracts[key]; if(!r) return; const set=new Set(A(r.done).map(Number)); const n=Number(no); if(set.has(n)) set.delete(n); else set.add(n); r.done=[...set].sort((a,b)=>a-b); putRecord(currentProjectId,currentRecord); renderCore(currentRecord); renderReport(currentRecord);};
  ...
  window.contractV10104ToggleAnnualVisit=function(id,no){if(readonlyMode) return; currentRecord=collectCoreFromScreen(); const item=A(currentRecord.annual).find(x=>S(x.id)===S(id)); if(!item) return; const set=new Set(A(item.done).map(Number)); const n=Number(no); if(set.has(n)) set.delete(n); else set.add(n); item.done=[...set].sort((a,b)=>a-b); putRecord(currentProjectId,currentRecord); renderAnnual(currentRecord); renderReport(currentRecord);};
  window.contractV10104DeleteAnnual=function(id){if(readonlyMode) return; if(!confirm('حذف هذه الخدمة السنوية؟')) return; currentRecord=collectCoreFromScreen(); currentRecord.annual=A(currentRecord.annual).filter(x=>S(x.id)!==S(id)); putRecord(currentProjectId,currentRecord); renderAnnual(currentRecord); renderReport(currentRecord);};
  // Aliases for old inline buttons if any remain.


  // ===== V10104: Smart contractual-services schedule and accurate printing =====
  ...
  function tableMap(table){
    ...
    return {
      project: idx(['المشروع']), supervisor: idx(['المشرف']), service: idx(['الخدمة','اسمالخدمة']), type: idx(['النوع']), repeat: idx(['التكرار']), visits: idx(['عددالزيارات','الزيارات']), done: idx(['تم','المنفذ']), remaining: idx(['المتبقي']), due: idx(['الاستحقاق','تاريخ']), status: idx(['الحالة']), notes: idx(['ملاحظات'])
    };
  }
  ...
  function rowObjFromTr(tr,map){
    ...
    const visits=N(get(map.visits)); const done=N(get(map.done)); const remainingText=get(map.remaining); const remaining=remainingText?N(remainingText):Math.max(0,visits-done);
  ...
  function currentContractualRows(includeDone){
    ...
    return [...table.querySelectorAll('tbody tr')].map(tr=>rowObjFromTr(tr,map)).filter(r=>r.service||r.project).filter(r=>includeDone||!serviceIsDone(r));
  }
  function hideDoneContractualRows(){
    const table=contractualTable(); if(!table || table.dataset.v10104HideInstalled==='1') return;
    table.dataset.v10104HideInstalled='1';
    const apply=()=>{
      const map=tableMap(table);
  ...
  window.printContractServicesSmartV10104=function(){
    ...
    let html=`<div class="meta"><div><span>إجمالي الخدمات</span><b>${total}</b></div><div><span>مستحقة/متابعة</span><b>${due}</b></div><div><span>عدد المشاريع</span><b>${grouped.size}</b></div><div><span>المصدر</span><b>الخدمات التعاقدية</b></div></div>`;
    grouped.forEach((items,project)=>{html+=`<section class="project"><h2>${esc(project)}</h2><table><thead><tr><th>#</th><th>الخدمة</th><th>المشرف</th><th>النوع</th><th>التكرار</th><th>عدد الزيارات</th><th>المنفذ</th><th>المتبقي</th><th>الاستحقاق</th><th>الحالة</th><th>ملاحظات</th></tr></thead><tbody>${items.map((r,i)=>`<tr><td>${i+1}</td><td><b>${esc(r.service||'-')}</b></td><td>${esc(r.supervisor||'-')}</td><td>${esc(r.type||'-')}</td><td>${esc(r.repeat||'-')}</td><td>${r.visits||0}</td><td>${r.done||0}</td><td>${r.remaining||0}</td><td>${esc(r.due||'-')}</td><td><span class="badge">${esc(r.status||'-')}</span></td><td>${esc(r.notes||'-')}</td></tr>`).join('')}</tbody></table></section>`;});
    printHtmlDocument('الجدول الذكي للخدمات التعاقدية',html);
  };
  window.printContractAnnualSmartV10104=function(){
    ...
    if(!annual.length&&!core.length){say('لا توجد عقود أو خدمات غير منفذة للطباعة','err');return;}
    let html=`<section class="project"><h2>${esc(p.name||'المشروع المحدد')}</h2><table><thead><tr><th>#</th><th>الخدمة/العقد</th><th>الشركة</th><th>رقم الشركة</th><th>من</th><th>إلى</th><th>عدد الزيارات</th><th>المنفذ</th><th>المتبقي</th><th>ملاحظات</th></tr></thead><tbody>`;
    let i=1; core.forEach(r=>{const done=A(r.done).length, rem=Math.max(0,N(r.visits)-done); html+=`<tr><td>${i++}</td><td><b>${esc(r.label)}</b></td><td>${esc(r.company||'-')}</td><td>${esc(r.phone||'-')}</td><td>${esc(r.start||'-')}</td><td>${esc(r.end||'-')}</td><td>${N(r.visits)}</td><td>${done}</td><td>${rem}</td><td>${esc(r.notes||'-')}</td></tr>`;});
    annual.forEach(a=>{const done=A(a.done).length, rem=Math.max(0,N(a.visits)-done); html+=`<tr><td>${i++}</td><td><b>${esc(a.name)}</b></td><td colspan="4">خدمة سنوية</td><td>${N(a.visits)}</td><td>${done}</td><td>${rem}</td><td>${esc(a.notes||'-')}</td></tr>`;});
    html+='</tbody></table></section>'; printHtmlDocument('تقرير خدمات العقد',html);
  };
  function installSmartButtonsV10104(){
    if(!isContractsPageVisible()) return;
    const section=findContractualServicesSection();
    if(section && !section.querySelector('[data-v10104-smart-print]')){
      const btn=document.createElement('button'); btn.type='button'; btn.className='light'; btn.dataset.v10104SmartPrint='1'; btn.textContent='جدول ذكي'; btn.onclick=window.printContractServicesSmartV10104;
    ...
    const reportBtn=$('contractSmartReportTab')?.querySelector?.('button[data-v10104-print]');
    const reportBox=$('contractSmartReportTab');
    if(reportBox && !reportBox.querySelector('[data-v10104-print-contract-report]')){
      const b=document.createElement('button'); b.type='button'; b.className='light'; b.dataset.v10104PrintContractReport='1'; b.textContent='طباعة التقرير الذكي'; b.onclick=window.printContractAnnualSmartV10104; reportBox.insertBefore(b, reportBox.firstChild);
    }
  }
  window.contractV10RefreshCore=window.contractV10104RefreshCore;
  window.contractV10ToggleCoreVisit=window.contractV10104ToggleCoreVisit;
  window.contractV10ToggleAnnualVisit=window.contractV10104ToggleAnnualVisit;
  window.contractV10DeleteAnnual=window.contractV10104DeleteAnnual;
  ...
  window.saveContractSmartModal=async function(){
    ...
    try{if(btn){btn.disabled=true; btn.textContent='جاري الحفظ...';}
      ...
      const pending=S($('csAnnualCustom')?.value)||S($('csAnnualSelect')?.value&&window.__tasneefAnnualTouchedV10104?$('csAnnualSelect')?.value:'');
  ...
  ['csAnnualSelect','csAnnualCustom','csAnnualVisits'].forEach(id=>document.addEventListener('input',e=>{if(e.target&&e.target.id===id) window.__tasneefAnnualTouchedV10104=true;},true));
  ['csAnnualSelect','csAnnualCustom','csAnnualVisits'].forEach(id=>document.addEventListener('change',e=>{if(e.target&&e.target.id===id) window.__tasneefAnnualTouchedV10104=true;},true));
