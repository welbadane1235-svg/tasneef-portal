/* ===== V10198 ROOT REBUILD: Monthly section rebuilt from zero =====
   Source of truth: current active projects. Logs are used only for time calculation. */
(function(){
  'use strict';
  if(window.__tasneefMonthlyRebuildV10198) return;
  window.__tasneefMonthlyRebuildV10198 = true;
  ...
  const state={loaded:false,projects:[],workers:[],users:[],logs:[],assignments:[],serverRows:[],cacheRows:[]};
  ...
  async function loadFresh(){
    const d0=D();
    // أهم تعديل V10198: لا نعتمد على استعلام الشهر فقط. نحمل كل سجلات الوقت الموجودة مثل التطبيق الأصلي،
    ...
    }catch(e){console.warn('V10198 loadAll fallback failed', e);}
    const d1=D();
    if(window.sb){
      ...
      const d=D(); state.projects=d.projects||[]; state.workers=d.workers||[]; state.users=d.users||d.supervisors||[]; state.logs=d.logs||[]; state.assignments=d.workerAssignments||d.assignments||[]; state.serverRows=d.monthlyServerRows||[]; state.cacheRows=d.monthlyCacheRows||[];
    }
    state.loaded=true;
  }
  ...
  async function render(force){
    ...
    window.__monthlyRowsRebuildV10198=rows;
  ...
  window.monthlyRowsV60=()=>window.__monthlyRowsRebuildV10198||buildMonthlyTimesFromCurrentProjects();
  ...
  console.log('Tasneef V10198 monthly rebuilt from zero loaded');
})();
