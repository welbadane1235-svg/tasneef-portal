...
/* Tasneef Inventory Audit Visible Force v10044
...
(function(){
  'use strict';
  if(window.__tasneefInventoryAuditVisibleForceV10044) return;
  window.__tasneefInventoryAuditVisibleForceV10044 = true;
...
<section id="inventoryAudit" class="page hidden inventory-audit-page-v10042">
  ...
  function ensureAuditNav(){
    // v10044: إدخال زر الجرد بالقوة حتى لو صلاحيات القائمة حذفت الأزرار غير المعروفة.
    let btn = document.querySelector('[data-audit-nav-v10042],[data-audit-nav-v10043],[data-audit-nav-v10044]');
    const financeNode = Array.from(document.querySelectorAll('button,a,div,span'))
      .filter(el => (el.textContent||'').trim()==='المالية والمخزون' || /المالية\s*والمخزون/.test(el.textContent||''))
      .sort((a,b)=>(a.textContent||'').length-(b.textContent||'').length)[0];
    const ordersNode = Array.from(document.querySelectorAll('button,a,div,span'))
      .filter(el => (el.textContent||'').trim()==='الأوردرات' || /الأوردرات/.test(el.textContent||''))
      .sort((a,b)=>(a.textContent||'').length-(b.textContent||'').length)[0];
    const logoutNode = Array.from(document.querySelectorAll('button,a,div,span'))
      .filter(el => /تسجيل خروج/.test(el.textContent||''))
      .sort((a,b)=>(a.textContent||'').length-(b.textContent||'').length)[0];
    let anchor = financeNode || ordersNode || logoutNode;
    let side = anchor ? (anchor.parentElement || anchor.closest('aside,nav,.side,.sidebar,.sidebar-menu,.menu')) : document.querySelector('aside,nav,.side,.sidebar,.sidebar-menu,.menu');
    if(!side) side = document.body;

    if(!btn){
      if(anchor && anchor.tagName && /BUTTON|A|DIV/.test(anchor.tagName)){
        btn = anchor.cloneNode(true);
      }else{
        btn = document.createElement('button');
        btn.className = 'nav';
      }
      btn.setAttribute('data-audit-nav-v10044','1');
      ...
      btn.onclick = function(ev){
        ...
        if(typeof window.showPage === 'function'){
          try{ window.showPage('inventoryAudit', btn); }catch(_){ }
        }
        document.querySelectorAll('.page, section.page').forEach(p=>p.classList.add('hidden'));
        const pg = $('inventoryAudit'); if(pg){ pg.classList.remove('hidden'); pg.style.display='block'; }
        document.querySelectorAll('.nav,[data-audit-nav-v10044]').forEach(n=>n.classList.remove('active'));
        btn.classList.add('active');
        loadAuditPage();
        return false;
      };
      const insertAfter = financeNode || ordersNode;
      if(insertAfter && insertAfter.parentNode){
        insertAfter.parentNode.insertBefore(btn, insertAfter.nextSibling);
      }else if(logoutNode && logoutNode.parentNode){
        logoutNode.parentNode.insertBefore(btn, logoutNode);
      }else{
    ...
    btn.setAttribute('data-force-visible','inventory-audit-v10044');
  }
