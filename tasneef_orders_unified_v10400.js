...
(function(){
  'use strict';
  if(window.__tasneefOrdersUnifiedV10403) return;
  window.__tasneefOrdersUnifiedV10403=true;
  ...
  function receiptUrl(r){
    const v=field(r,'receipt_url','receiptUrl','receipt_image','receipt_image_url','receipt','سند','السند','صورة السند','رابط السند','إيصال','الايصال');
    if(Array.isArray(v)) return S(v[0]?.url||v[0]);
    if(v&&typeof v==='object') return S(v.url||v.publicUrl||v.path);
    return S(v);
  }
  function viewReceipt(idx){
    const r=rows[idx]; if(!r)return; const url=receiptUrl(r);
    if(!url){notify('لا يوجد إيصال مرفق لهذا الأوردر','err');return;}
    const safe=E(url);
    if(/\.(png|jpe?g|webp|gif)(\?|$)/i.test(url)||url.startsWith('data:image')){
      document.body.insertAdjacentHTML('beforeend',`<div class="ou-modal" onclick="if(event.target===this)this.remove()"><div><div class="ou-head"><h2>إيصال ${E(orderNo(r))}</h2><button class="light" onclick="this.closest('.ou-modal').remove()">إغلاق</button></div><img class="ou-receipt-img" src="${safe}" alt="إيصال الأوردر"></div></div>`);
    }else window.open(url,'_blank','noopener');
  }
  function whatsapp(idx){
    const r=rows[idx]; if(!r)return; const phone=S(field(r,'رقم العميل','customer_phone')).replace(/\D/g,'');
    if(!phone){notify('لا يوجد رقم عميل صالح لإرسال واتساب','err');return;}
    const intl=phone.startsWith('966')?phone:(phone.startsWith('0')?'966'+phone.slice(1):phone);
    const inclusive=num(field(r,'السعر (شامل الضريبة)','inclusive_total','total_with_vat'));
    const text=[`السلام عليكم،`,`تفاصيل الأوردر رقم ${orderNo(r)}`,`المشروع: ${projectName(r)||'-'}`,`العميل: ${field(r,'اسم العميل','customer_name')||'-'}`,`الوحدة: ${field(r,'رقم الشقة','unit_number')||'-'}`,`الحالة: ${field(r,'حالة التنفيذ','status')||'-'}`,`الإجمالي شامل الضريبة: ${money(inclusive)}`].join('\n');
    window.open('https://wa.me/'+intl+'?text='+encodeURIComponent(text),'_blank','noopener');
  }
  function ensureTotalsBox(){
    if($('ouTotalsSummary10403'))return $('ouTotalsSummary10403');
    const anchor=$('ordersCardsV360')?.parentElement||$('ordersCardsV360'); if(!anchor)return null;
    const box=document.createElement('div');box.id='ouTotalsSummary10403';box.className='ou-total-summary';
    box.innerHTML='<div class="ou-money"><small>إجمالي قبل الضريبة</small><b id="ouTotalBefore10403">0.00 ر.س</b></div><div class="ou-money"><small>إجمالي الضريبة</small><b id="ouTotalVat10403">0.00 ر.س</b></div><div class="ou-money"><small>إجمالي شامل الضريبة</small><b id="ouTotalInclusive10403">0.00 ر.س</b></div><div class="ou-money"><small>إجمالي صافي الربح</small><b id="ouTotalProfit10403">0.00 ر.س</b></div>';
    anchor.parentNode.insertBefore(box,anchor); return box;
  }
  function card(r,idx){
  ...
  function render(){
    ...
    ensureTotalsBox(); const totals=list.reduce((a,r)=>{const inc=num(field(r,'السعر (شامل الضريبة)','inclusive_total','total_with_vat'));const bef=num(field(r,'السعر قبل الضريبة','before_vat'))||(inc/1.15);const vat=num(field(r,'الضريبة 15%','vat_amount'))||(inc-bef);const profit=num(field(r,'الربح','net_profit'))||(bef-num(field(r,'التكلفة','cost')));a.before+=bef;a.vat+=vat;a.inclusive+=inc;a.profit+=profit;return a;},{before:0,vat:0,inclusive:0,profit:0}); if($('ouTotalBefore10403'))$('ouTotalBefore10403').textContent=money(totals.before);if($('ouTotalVat10403'))$('ouTotalVat10403').textContent=money(totals.vat);if($('ouTotalInclusive10403'))$('ouTotalInclusive10403').textContent=money(totals.inclusive);if($('ouTotalProfit10403'))$('ouTotalProfit10403').textContent=money(totals.profit);
    hydrateFilters();
  }
  ...
  async function history(no){
    try{
      const h=await api('/rest/v1/'+AUDIT+'?order_no=eq.'+encodeURIComponent(no)+'&select=*&order=changed_at.asc')||[];
      const create=h.find(x=>x.action_type==='create');
      const updates=h.filter(x=>x.action_type!=='create');
      const grouped=[];
      updates.forEach(x=>{
        const key=[S(x.changed_by_name),S(x.changed_at).slice(0,16)].join('|');
        let g=grouped.find(y=>y.key===key); if(!g){g={key,name:S(x.changed_by_name||'-'),role:S(x.changed_by_role||''),at:x.changed_at,items:[]};grouped.push(g);}g.items.push(x);
      });
      const creationHtml=create?`<article class="ou-history-create"><b>أنشأ الطلب: ${E(create.changed_by_name||'-')}</b><br><small>${E(new Date(create.changed_at).toLocaleString('ar-SA',{hour12:true}))}</small></article>`:'<article class="ou-history-create"><b>منشئ الطلب: غير مسجل في البيانات القديمة</b></article>';
      const updatesHtml=grouped.map(g=>`<article class="ou-history-group"><b>${E(g.name)}</b>${g.role?` <small>(${E(g.role)})</small>`:''}<br><small>${E(new Date(g.at).toLocaleString('ar-SA',{hour12:true}))}</small><div class="ou-change-list">${g.items.map(x=>`<div class="ou-change-item"><b>${E(x.field_label||x.field_name||'حقل')}</b><div>من: ${E(x.old_value||'-')}</div><div>إلى: ${E(x.new_value||'-')}</div></div>`).join('')}</div></article>`).join('');
      document.body.insertAdjacentHTML('beforeend',`<div class="ou-modal" onclick="if(event.target===this)this.remove()"><div><div class="ou-head"><h2>سجل تعديلات ${E(no)}</h2><button class="light" onclick="this.closest('.ou-modal').remove()">إغلاق</button></div><div class="ou-history">${creationHtml}${updatesHtml||'<div class="ou-note">لا توجد تعديلات بعد إنشاء الطلب</div>'}</div></div></div>`);
    }catch(e){notify('تعذر تحميل سجل التعديلات. شغّل ملف SQL أولاً','err');}
  }
  function bind(){
  ...
  window.tasneefOrders10400={load,save,edit,del,history,viewReceipt,whatsapp,page:d=>{page=Math.max(1,page+Number(d||0));render()},clear,render};
