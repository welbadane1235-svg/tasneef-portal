...
(function(){
  ...
  const PAGE_SIZE=25;
  let rows=[], page=1, totalRows=0, saving=false, editNo='', ordersLoading=false, ordersAbort=null, searchTimer=null, summaryState={total_orders:0,completed_orders:0,unpaid_orders:0,total_net_profit:0};
...
آخر فحص: ${out?.checked_at||'-'}`);}}catch(e){console.warn('System health failed',e);}}
  ...
  function render(){
    const list=filterRows(),pages=Math.max(1,Math.ceil(totalRows/PAGE_SIZE));page=Math.min(page,pages);renderSummary(list);
