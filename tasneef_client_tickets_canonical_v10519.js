/* Tasneef client tickets V10519: verified save + verified reload */
(function(){
  'use strict';
  if(window.__tasneefClientTicketsV10519) return;
  window.__tasneefClientTicketsV10519=true;
  const $=id=>document.getElementById(id), A=v=>Array.isArray(v)?v:[];
  async function listTickets(){
    if(!window.sb||!window.portal?.projectId) return [];
    const r=await sb.rpc('tasneef_client_tickets_v10519',{p_project_id:String(portal.projectId)});
    if(r.error) throw r.error;
    portal.tickets=A(r.data);
  ...
  window.submitClientTicketV466=async function(btn){
    ...
    try{
      ...
      if(!window.sb||!window.portal?.projectId) throw new Error('تعذر تحديد المشروع المرتبط ببوابة العميل');
      if(btn){btn.disabled=true;btn.textContent='جاري الحفظ والتحقق...'}
      const r=await sb.rpc('tasneef_client_create_ticket_v10519',{
        p_project_id:String(portal.projectId),p_title:title,p_description:desc,p_type:type,p_location:loc
      });
      if(r.error) throw r.error;
      ...
      console.error('V10519 client ticket',e);
  ...
  if(typeof oldShow==='function')window.showMain=async function(tab){
    if(tab==='tickets'){
      try{await listTickets()}catch(e){console.error('V10519 list client tickets',e)}
    }
  ...
  setInterval(()=>{if(window.portal?.projectId)listTickets().then(()=>{try{if(document.querySelector('.tab.active')?.textContent.includes('التذاكر'))oldShow('tickets')}catch(_){}}).catch(()=>{})},10000);
})();
