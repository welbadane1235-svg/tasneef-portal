...
/* Tasneef Quote Exact Print Override v10041
   يجعل الطباعة بنفس نموذج عرض السعر المرسل قدر الإمكان: الشعار، الرأس العربي/الإنجليزي، جدول البنود، الإجماليات، وملاحظات الدفع.
*/
(function(){
  'use strict';
  if(window.__tasneefQuoteExactPrintV10041) return;
  window.__tasneefQuoteExactPrintV10041 = true;
  const SUPABASE_URL = 'https://zmjdqiswytxlbfgnfjfv.supabase.co';
  const SUPABASE_ANON_KEY = 'sb_publishable_ADsAC5MtBCusDgX62c8NaQ_LyyuTPeb';
  const VAT_RATE = 0.15;
  const S = v => String(v ?? '').trim();
  ...
  const esc = v => S(v).replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[c]));
  const num = v => { const n=Number(S(v).replace(/,/g,'').replace(/[^0-9.\-]/g,'')); return Number.isFinite(n)?n:0; };
  const moneyPlain = v => num(v).toLocaleString('en-US',{minimumFractionDigits:2, maximumFractionDigits:2});
  const itemCalc = it => {
    const qty = Math.max(0, num(it.qty || 1));
    const price = num(it.price || 0);
    const taxable = +(qty * price).toFixed(2);
    const vat = +(taxable * VAT_RATE).toFixed(2);
    return Object.assign({}, it, {qty, price, taxable_amount:taxable, vat_amount:vat, line_total:+(taxable+vat).toFixed(2)});
  };
  const totals = items => {
    const sub=items.reduce((a,i)=>a+num(i.taxable_amount),0);
    const vat=items.reduce((a,i)=>a+num(i.vat_amount),0);
  ...
  function currentEditorData(){
    const q = {
      quote_no:S($('quoteNoV10040')?.value),
      quote_date:S($('quoteDateV10040')?.value),
      status:S($('quoteStatusV10040')?.value),
      customer_name:S($('quoteCustomerV10040')?.value),
      customer_phone:S($('quotePhoneV10040')?.value),
      customer_email:S($('quoteEmailV10040')?.value),
      customer_address:S($('quoteAddressV10040')?.value),
      project:S($('quoteProjectV10040')?.value),
      reference:S($('quoteReferenceV10040')?.value),
      notes:S($('quoteNotesV10040')?.value),
      terms:S($('quoteTermsV10040')?.value)
    };
    let items=[];
    try{ items = JSON.parse($('quoteItemsBodyV10040')?.dataset.items || '[]').map(itemCalc); }catch(_){ items=[]; }
    return {q, items};
  }
  function defaultTerms(){ return 'لا يتم تنفيذ أي طلب نظافة إلا بعد سداد كامل المبلغ وتحويله إلى الحساب البنكي للشركة،\nمع إرسال إيصال التحويل.\nبنك الراجحي\nشركة تصنيف المستقبل للتشغيل والصيانة\nرقم الحساب: 501000010006085376530\nالآيبان: SA4480000501608015376530'; }
  function printExact(q, items){
    items = (items||[]).map(itemCalc);
    const t = totals(items);
    const logo = new URL('tasneef_logo_print.png', location.href).href;
    const pageNo = S(q.quote_no || 'QUO');
    const rows = items.map((it,i)=>{
      const ord = S(it.source_order_no || '');
      let desc = S(it.description || '');
      if(ord && !desc.includes(ord)) desc += ' | رقم ' + ord;
      return `<tr>
        <td class="idx">${i+1}</td>
        <td class="desc">${esc(desc||'-')}</td>
        <td class="qty">${esc(it.qty)}</td>
        <td class="num">${moneyPlain(it.price)}</td>
        <td class="num">${moneyPlain(it.taxable_amount)}</td>
        <td class="vat"><span>${moneyPlain(it.vat_amount)}</span><em>15%</em></td>
        <td class="num total">${moneyPlain(it.line_total)}</td>
      </tr>`;
    }).join('');
    const html = `<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><title>${esc(pageNo)}</title>
<style>
@page{size:A4;margin:8mm 8mm}*{box-sizing:border-box}body{margin:0;background:#f1f1f1;color:#111;font-family:Arial,Tahoma,sans-serif}.print-bar{position:fixed;top:10px;left:10px;z-index:9999;display:flex;gap:8px}.print-bar button{background:#0A4033;color:#fff;border:0;border-radius:7px;padding:9px 14px;font-weight:800;cursor:pointer}.quote-page{width:210mm;min-height:297mm;background:#fff;margin:0 auto;padding:11mm 10mm 11mm;position:relative;overflow:hidden}.top{display:grid;grid-template-columns:1fr 64mm 1fr;align-items:start;gap:8mm}.company-en{direction:ltr;text-align:left;font-size:10.2px;line-height:1.35}.company-ar{direction:rtl;text-align:right;font-size:10.2px;line-height:1.45}.company-en b,.company-ar b{font-size:12.2px;display:block;margin-bottom:3px}.logo-wrap{text-align:center;padding-top:4mm}.logo-wrap img{width:54mm;height:auto;max-height:25mm;object-fit:contain}.quote-title{font-size:18px;font-weight:900;text-align:center;margin:7mm 0 5mm}.info{display:grid;grid-template-columns:1fr 1fr;gap:8mm;margin-bottom:4mm;font-size:10.5px}.info table{width:100%;border-collapse:collapse}.info td{padding:2.2mm 1mm;vertical-align:top}.label{font-weight:800;white-space:nowrap}.left{text-align:left;direction:ltr}.items{width:100%;border-collapse:collapse;table-layout:fixed;font-size:9.7px}.items th{font-weight:800;background:#fff}.items th,.items td{border:0;padding:1.6mm 1.1mm;vertical-align:top}.items thead tr{border-bottom:1.4px solid #202020}.items tbody tr{border-bottom:0.8px solid #e3e3e3}.idx{width:7mm;text-align:center}.desc{width:69mm;line-height:1.55}.qty{width:13mm;text-align:center}.num{direction:ltr;text-align:right;white-space:nowrap}.vat{width:20mm;text-align:center;direction:ltr;white-space:nowrap}.vat em{display:block;font-style:normal;font-size:9px;margin-top:1mm}.total{font-weight:700}.notes{margin-top:7mm;font-size:10.3px;line-height:1.65;white-space:pre-line}.bottom{display:grid;grid-template-columns:1fr 74mm;gap:8mm;margin-top:6mm;align-items:start}.bank{font-size:10.2px;line-height:1.75;white-space:pre-line}.totals{width:100%;border-collapse:collapse;font-size:10.8px}.totals td{padding:2.4mm 1mm;border:0}.totals tr{border-bottom:0.7px solid #ddd}.totals .amount{direction:ltr;text-align:left;font-weight:800;white-space:nowrap}.currency:before{content:'⃁ ';font-weight:400}.footer{position:absolute;bottom:5mm;left:10mm;right:10mm;display:flex;justify-content:space-between;align-items:center;font-size:9.5px;color:#333;border-top:0.8px solid #ddd;padding-top:2.5mm}.footer .middle{direction:ltr}.page-break{page-break-after:always}@media print{body{background:#fff}.print-bar{display:none}.quote-page{margin:0;box-shadow:none;width:auto;min-height:277mm;padding-bottom:16mm}}
</style></head><body><div class="print-bar"><button onclick="window.print()">طباعة / حفظ PDF</button></div><main class="quote-page">
<section class="top">
  <div class="company-en"><b>Tasnef Future Company</b>Qurtubah Diet, Said Ibn Zaid, RUQA6441 - 2526.<br>Qurtubah, Riyadh, Kingdom of Saudi Arabia<br>920015589<br>VAT number 311784213300003<br>CR Number 1010915542</div>
  <div class="logo-wrap"><img src="${logo}" alt="Tasnef"></div>
  <div class="company-ar"><b>شركة تصنيف للتشغيل والصيانة</b>حي قرطبة، شارع سعيد بن زيد، RUQA 6441 - 2526<br>قرطبة، الرياض، المملكة العربية السعودية<br>920015589<br>رقم التسجيل الضريبي 311784213300003<br>رقم السجل التجاري 1010915542</div>
</section>
<div class="quote-title">عرض سعر Quote</div>
<section class="info">
  <table><tr><td class="label">العميل Customer</td><td>${esc(q.customer_name||'-')}</td></tr><tr><td class="label">العنوان Address</td><td>${esc(q.customer_address||'-')}</td></tr><tr><td class="label">البريد الإلكتروني Email</td><td class="left">${esc(q.customer_email||'-')}</td></tr><tr><td class="label">الهاتف Phone</td><td class="left">${esc(q.customer_phone||'-')}</td></tr></table>
  <table><tr><td class="label">رقم Number</td><td class="left">${esc(q.quote_no||'-')}</td></tr><tr><td class="label">التاريخ Date</td><td class="left">${esc(q.quote_date||'-')}</td></tr><tr><td class="label">المرجع Reference</td><td>${esc(q.reference||'-')}</td></tr><tr><td class="label">المشروع Project</td><td>${esc(q.project||'-')}</td></tr></table>
</section>
<table class="items"><thead><tr><th class="idx">#</th><th class="desc">الوصف<br>Description</th><th class="qty">الكمية<br>Qty</th><th>السعر<br>Price</th><th>المبلغ الخاضع للضريبة<br>Taxable amount</th><th class="vat">القيمة المضافة<br>VAT amount</th><th>المجموع<br>Line amount</th></tr></thead><tbody>${rows}</tbody></table>
<section class="bottom"><div><div class="bank">${esc(q.terms||defaultTerms())}</div>${q.notes?`<div class="notes"><b>ملاحظات Notes</b><br>${esc(q.notes)}</div>`:''}</div><table class="totals"><tr><td>المجموع الفرعي Subtotal</td><td class="amount currency">${moneyPlain(t.subtotal)}</td></tr><tr><td>إجمالي ضريبة القيمة المضافة VAT Total</td><td class="amount currency">${moneyPlain(t.vat_total)}</td></tr><tr><td><b>المجموع شامل القيمة المضافة Total</b></td><td class="amount currency"><b>${moneyPlain(t.total)}</b></td></tr></table></section>
<div class="footer"><span>شركة تصنيف للتشغيل والصيانة</span><span class="middle">${esc(pageNo)} - Page 1 of 1</span><span>Tasnef Future Company</span></div>
</main></body></html>`;
  ...
  window.quotesPreviewCurrentV10040 = function(){ const {q, items} = currentEditorData(); printExact(q, items); };
  window.quotesPreviewCurrentV10041 = window.quotesPreviewCurrentV10040;
  window.quotesPrintV10040 = async function(id){
    try{
      const qArr = await api('/rest/v1/quotes?id=eq.'+encodeURIComponent(id)+'&select=*', {method:'GET'});
      const q = Array.isArray(qArr) ? qArr[0] : null;
      if(!q) return alert('لم يتم العثور على عرض السعر');
      const items = await api('/rest/v1/quote_items?quote_id=eq.'+encodeURIComponent(id)+'&select=*&order=item_order.asc', {method:'GET'});
      printExact(q, Array.isArray(items)?items:[]);
    }catch(e){ alert('تعذر طباعة عرض السعر: '+(e.message||e)); }
  };
  window.quotesPrintExactV10041 = printExact;
})();
