(function(){
  ...
  function rowInput(r,k,cls='sal-input money'){ return `<input class="${cls}" data-key="${k}" data-type="${r.entity_type}" data-id="${r.entity_id}" value="${esc(r[k]??0)}" onchange="tasneefSalariesV10266.update('${r.entity_type}','${r.entity_id}','${k}',this.value)">`; }
  function dateInput(r,k){ return `<input type="date" class="sal-input salary-date-input" style="width:135px" data-key="${k}" data-type="${r.entity_type}" data-id="${r.entity_id}" value="${esc(r[k]||'')}" oninput="tasneefSalariesV10266.update('${r.entity_type}','${r.entity_id}','${k}',this.value)" onchange="tasneefSalariesV10266.update('${r.entity_type}','${r.entity_id}','${k}',this.value)">`; }
  function textInput(r,k,w=130){ return `<input class="sal-input" style="width:${w}px" data-key="${k}" data-type="${r.entity_type}" data-id="${r.entity_id}" value="${esc(r[k]||'')}" onchange="tasneefSalariesV10266.update('${r.entity_type}','${r.entity_id}','${k}',this.value)">`; }
  function renderSalaryRowHtml(r,i,month){
    return `<tr data-sal-row="${r.entity_type}_${r.entity_id}">
      ...
      <td><input class="sal-input" data-key="notes" data-type="${r.entity_type}" data-id="${r.entity_id}" value="${esc(r.notes||'')}" onchange="tasneefSalariesV10266.update('${r.entity_type}','${r.entity_id}','notes',this.value)"></td>
    </tr>`;
  }
  ...
  function inject(){
    ...
    const main=document.querySelector('main.content'); const sec=document.createElement('section'); sec.id='salaries'; sec.className='page hidden'; sec.innerHTML=`
      ...
      <div class="filters"><div><label>الشهر</label><input type="month" id="salaryMonth" value="${today().slice(0,7)}" onchange="tasneefSalariesV10266.load()"></div><div><label>نوع الكشف</label><select id="salaryType" onchange="tasneefSalariesV10266.buildRows()"><option value="all">الكل</option><option value="supervisors">رواتب المشرفين</option><option value="workers">رواتب العمال</option><option value="technicians">رواتب الفنيين</option><option value="guards">رواتب الحراس</option></select></div><div><label>المشرف</label><select id="salarySupervisor" onchange="tasneefSalariesV10266.buildRows()"><option value="">كل المشرفين</option></select></div><div><label>المشروع</label><select id="salaryProject" onchange="tasneefSalariesV10266.buildRows()"><option value="">كل المشاريع</option></select></div><div><label>بحث</label><input id="salarySearch" oninput="tasneefSalariesV10266.buildRows()" placeholder="اسم/إقامة/TS"></div></div>
      <div class="salary-actions"><button onclick="tasneefSalariesV10266.load()">تحديث الرواتب</button><button class="light" onclick="tasneefSalariesV10266.save(false)">حفظ التعديلات</button><button class="light" onclick="tasneefSalariesV10266.save(true)">اعتماد الرواتب</button><button class="light" onclick="tasneefSalariesV10266.print()">طباعة</button><button class="light" onclick="tasneefSalariesV10266.exportExcel()">تصدير Excel</button></div><div id="salaryKpis" class="kpis small"></div>
  ...
  window.tasneefSalariesV10266={inject,load:loadSalary,buildRows,update,save:saveSalary,print:printSalary,exportExcel:exportSalaryExcel};
  window.tasneefSalariesV10261=window.tasneefSalariesV10266;
  window.tasneefSalariesV10265=window.tasneefSalariesV10266;
