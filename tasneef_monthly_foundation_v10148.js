/* Tasneef v10148 - Monthly Time Logs Stable Binding to Existing Section
...
(function(){
  'use strict';
  if(window.__tasneefMonthlyFoundationV10148) return;
  window.__tasneefMonthlyFoundationV10148 = true;

  const SUPABASE_URL='https://zmjdqiswytxlbfgnfjfv.supabase.co';
  const SUPABASE_ANON_KEY='sb_publishable_ADsAC5MtBCusDgX62c8NaQ_LyyuTPeb';
  const MANUAL_TABLE='monthly_time_manual_adjustments';
  const LOCAL_MANUAL='tasneef_monthly_manual_rows_foundation_v10148';
  ...
  async function allPaged(table, cols='*', queryFn=null, max=100000){
    ...
    for(let from=0; from<max; from+=step){
      try{
        ...
        if(error){ console.warn('[monthly v10148]',table,error.message||error); break; }
        out.push(...A(data));
        if(!data || data.length<step) break;
      }catch(e){ console.warn('[monthly v10148]',table,e.message||e); break; }
    }
    return out;
  }
  ...
  function fillFilters(){
    const cs=S($('mtSupervisorV10145')?.value), cp=S($('mtProjectV10145')?.value);
    ...
    if($('mtSupervisorV10145')) $('mtSupervisorV10145').innerHTML='<option value="">كل المشرفين</option>'+sups.map(x=>`<option value="${E(x)}" ${x===cs?'selected':''}>${E(x)}</option>`).join('');
    if($('mtProjectV10145')) $('mtProjectV10145').innerHTML='<option value="">كل المشاريع</option>'+prjs.map(x=>`<option value="${E(x)}" ${x===cp?'selected':''}>${E(x)}</option>`).join('');
    if($('mtSupervisorListV10145')) $('mtSupervisorListV10145').innerHTML=[...new Set([...sups,...state.users.map(userLabel).filter(Boolean)])].map(x=>`<option value="${E(x)}"></option>`).join('');
    if($('mtProjectListV10145')) $('mtProjectListV10145').innerHTML=[...new Set([...prjs,...state.projects.map(projectLabel).filter(Boolean)])].map(x=>`<option value="${E(x)}"></option>`).join('');
  }
  ...
  function renderSummary(rows){
    ...
    if($('mtSummaryV10145')) $('mtSummaryV10145').innerHTML=`<div class="kpi"><small>إجمالي الوقت</small><b>${fmt(actual/60,2)} ساعة</b></div><div class="kpi"><small>الساعات المطلوبة</small><b>${fmt(req/60,2)}</b></div><div class="kpi"><small>وقت الانتقال</small><b>${fmt(travel/60,2)}</b></div><div class="kpi"><small>المشرفين</small><b>${sups.size || new Set(rows.map(r=>r.supervisor).filter(x=>x&&x!=='غير محدد')).size}</b></div><div class="kpi"><small>المشاريع</small><b>${projects.size || new Set(rows.map(r=>r.project).filter(x=>x&&x!=='غير محدد')).size}</b></div><div class="kpi"><small>عدد السجلات</small><b>${rec}</b></div>`;
  }
  function renderTable(rows){
    const b=$('mtBodyV10145'); if(!b) return;
    if(!rows.length){ b.innerHTML='<tr><td colspan="11">لا توجد بيانات للشهر المختار.</td></tr>'; return; }
    b.innerHTML=rows.map(r=>{ const p=perf(r); return `<tr><td>${E(r.supervisor)}</td><td>${E(r.project)}</td><td>${E(r.workers||'-')}</td><td>${N(r.count)}</td><td>${fmt(N(r.requiredMin)/60,2)}</td><td>${fmt(N(r.actualMin)/60,2)}</td><td>${fmt(N(r.travelMin)/60,2)}</td><td>${p.pct}%</td><td><span class="badge ${p.cls}">${p.label}</span></td><td>${E(r.source)}</td><td>${r.source==='تعديل يدوي'?`<button class="light" onclick="TasneefMonthlyV10148.edit('${E(r.id)}')">تعديل</button> <button class="danger" onclick="TasneefMonthlyV10148.remove('${E(r.id)}')">حذف</button>`:'-'}</td></tr>`; }).join('');
  }
  ...
  async function load(){
    if(state.loading) return; state.loading=true; const ym=S($('mtMonthV10145')?.value)||ymNow(); state.month=ym; if($('mtMonthV10145')) $('mtMonthV10145').value=ym;
  ...
  async function save(){
    const r={id:S($('mtManualIdV10145')?.value),month:S($('mtMonthV10145')?.value)||ymNow(),supervisor:S($('mtManualSupervisorV10145')?.value),project:S($('mtManualProjectV10145')?.value),workers:S($('mtManualWorkersV10145')?.value),count:0,requiredMin:N($('mtManualRequiredV10145')?.value),actualMin:N($('mtManualActualV10145')?.value),travelMin:N($('mtManualTravelV10145')?.value),percent:S($('mtManualPercentV10145')?.value)===''?'':N($('mtManualPercentV10145')?.value),source:'تعديل يدوي'};
  ...
  function bind(){
    if(!$('mtBodyV10145')) return;
    if($('mtMonthV10145')&&!$('mtMonthV10145').value) $('mtMonthV10145').value=ymNow();
    $('mtMonthV10145')?.addEventListener('change',load);
    $('mtSupervisorV10145')?.addEventListener('change',render);
    $('mtProjectV10145')?.addEventListener('change',render);
    $('mtRefreshV10145')?.addEventListener('click',load);
    $('mtCsvV10145')?.addEventListener('click',csv);
    $('mtPrintV10145')?.addEventListener('click',print);
    $('mtManualSaveV10145')?.addEventListener('click',save);
    $('mtManualClearV10145')?.addEventListener('click',clear);
    load();
  }
  window.TasneefMonthlyV10148={reload:load,edit,remove,csv,print};
