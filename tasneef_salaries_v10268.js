  ...
  async function exportSalaryExcel(){
    try{
      ...
      rows.forEach((r,idx)=>{
        ...
        row.getCell(19).value={formula:`ROUND(P${erow}/${monthDays}*L${erow},2)`, result:num(r.deductions)};
        ...
        ['I','J'].forEach((c,ci)=>{
          ...
          if(v){ const d=new Date(v+'T00:00:00'); if(!isNaN(d)) ws.getCell(`${c}${erow}`).value=d; }
        });
        row.height=22;
      ...
      ws.getCell(`A${signRow+2}`).value='ملاحظة: الأعمدة K/M/P/Q/S/V تحتوي معادلات، وعند تعديل التواريخ أو أيام الغياب أو الرواتب ستعاد الحسابات داخل Excel.';
