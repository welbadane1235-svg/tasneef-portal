/* ===== V10199 ROOT REBUILD: Attendance section rebuilt from zero, data preserved ===== */
(function(){
  'use strict';
  if(window.__tasneefAttendanceRebuildV10199) return; window.__tasneefAttendanceRebuildV10199=true;
  const VERSION='10199';
  ...
  function installAdmin(){
    ...
    sec.innerHTML=`<div class="att-rb-root">
      ...
      <div class="att-rb-controls">
        ...
        <button type="button" id="attRbRefresh" onclick="window.renderAttendanceRootV10199&&window.renderAttendanceRootV10199(true)">تحديث مباشر</button>
        <button type="button" id="attRbSave" onclick="window.saveAttendanceRootV10199&&window.saveAttendanceRootV10199()">حفظ التحضير</button>
        <button type="button" id="attRbPrint" onclick="window.printAttendanceRootV10199&&window.printAttendanceRootV10199()">طباعة</button>
        <button type="button" id="attRbCsv" onclick="window.exportAttendanceRootV10199&&window.exportAttendanceRootV10199()">تصدير CSV</button>
      </div>
  ...
  const oldShow=window.showPage; window.showPage=function(id,btn){const r=oldShow?oldShow.apply(this,arguments):undefined; if(id==='attendance') setTimeout(()=>renderAdmin(true),150); return r};
  ...
  function boot(){ if($('attendance')){installAdmin(); setTimeout(()=>renderAdmin(true),900);} if($('supAttendance')) setTimeout(()=>renderSupervisor(),900); }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',boot); else boot();
  window.addEventListener('load',()=>setTimeout(boot,1200));
  console.log('Tasneef V10199 attendance rebuilt from zero loaded');
})();
