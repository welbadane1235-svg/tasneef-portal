...
let data = { users:[], supervisors:[], projects:[], workers:[], attendance:[], logs:[], tickets:[], contractServices:[], financeContracts:[], financeInvoices:[], financeReceipts:[], financeExpenses:[] };
...
function money(v){ v=Number(v||0); return v.toLocaleString('ar-SA',{minimumFractionDigits:2,maximumFractionDigits:2})+' ر.س'; }
...
function financeHydrateForms(){
  ...
  financeMonthOptions();
  const pros=data.projects||[];
  ['financeProjectFilter','financeProjectSelect','financeInvoiceProject','financeReceiptProject','financeExpenseProject'].forEach(id=>fillSelect(id, pros, 'name', id==='financeProjectSelect'?'مشروع جديد أو اختر مشروعًا':'كل المشاريع'));
  financeFillInvoiceSelect();
  if($('financeContractStart') && !$('financeContractStart').value) $('financeContractStart').value=today();
  if($('financeInvoiceDate') && !$('financeInvoiceDate').value) $('financeInvoiceDate').value=today();
  if($('financeInvoiceDue') && !$('financeInvoiceDue').value) $('financeInvoiceDue').value=today();
  if($('financeReceiptDate') && !$('financeReceiptDate').value) $('financeReceiptDate').value=today();
...
async function financeLoadAll(showMessage=false){
  try{
    const [contracts,invoices,receipts,expenses] = await Promise.all([
      sb.from('finance_contracts').select('*').order('id',{ascending:false}),
      sb.from('finance_invoices').select('*').order('issue_date',{ascending:false}),
      sb.from('finance_receipts').select('*').order('payment_date',{ascending:false}),
      sb.from('finance_expenses').select('*').order('expense_date',{ascending:false})
    ]);
    for(const r of [contracts,invoices,receipts,expenses]) if(r.error) console.warn('finance:', r.error.message);
    data.financeContracts=contracts.data||[]; data.financeInvoices=invoices.data||[]; data.financeReceipts=receipts.data||[]; data.financeExpenses=expenses.data||[];
    data.financeError = contracts.error||invoices.error||receipts.error||expenses.error ? 'شغّل ملف schema_update_v103_finance_core.sql في Supabase' : '';
    financeLoaded=true; financeHydrateForms(); financeRenderAll(); if(showMessage) msg(data.financeError||'تم تحديث البيانات المالية');
  }catch(e){ console.error(e); data.financeError='شغّل ملف schema_update_v103_finance_core.sql في Supabase'; financeRenderAll(); if(showMessage) msg(data.financeError,'err'); }
}
...
function financeProjectFilterRows(rows,dateField='issue_date'){
  ...
  if(mon) out=out.filter(r=>String(r[dateField]||'').startsWith(mon));
  if(q) out=out.filter(r=>[r.invoice_no,r.project_name,financeProjectName(r.project_id),r.supplier,r.category,r.notes,r.invoice_type].join(' ').includes(q));
  return out;
}
function invoicePaid(invoiceId){ return (data.financeReceipts||[]).filter(r=>String(r.invoice_id)===String(invoiceId)).reduce((a,r)=>a+num(r.amount),0); }
function invoiceDue(inv){ return Math.max(0,num(inv.total)-invoicePaid(inv.id)); }
function invoiceComputedStatus(inv){ if(inv.status==='cancelled') return 'cancelled'; const due=invoiceDue(inv), total=num(inv.total); if(due<=0 && total>0) return 'paid'; if(due<total && due>0) return 'partial'; if(inv.due_date && inv.due_date<today()) return 'overdue'; return inv.status||'draft'; }
function financeStatusText(s){ return {draft:'مسودة',sent:'مرسلة',partial:'مدفوعة جزئيًا',paid:'مدفوعة بالكامل',overdue:'متأخرة',cancelled:'ملغاة'}[s]||s||'-'; }
function financeStatusClass(s){ return s==='paid'?'finance-status-paid':(s==='partial'||s==='sent'||s==='draft'?'finance-status-due':(s==='overdue'||s==='cancelled'?'finance-status-bad':'finance-status-neutral')); }
function financeRenderAll(){ if(!$('financeDashboard')) return; financeRenderKpis(); financeRenderContracts(); financeRenderInvoices(); financeRenderReceipts(); financeRenderExpenses(); financeRenderProfitability(); financeRenderRecent(); }
function financeRenderKpis(){
  let invoices=financeProjectFilterRows(data.financeInvoices||[],'issue_date'); const st=$('financeInvoiceStatusFilter')?.value; if(st) invoices=invoices.filter(i=>invoiceComputedStatus(i)===st);
  const expenses=financeProjectFilterRows(data.financeExpenses||[],'expense_date').filter(e=>!$('financeExpenseTypeFilter')?.value || e.category===$('financeExpenseTypeFilter').value);
  const totalInv=invoices.filter(i=>invoiceComputedStatus(i)!=='cancelled').reduce((a,i)=>a+num(i.total),0);
  const invIds=new Set(invoices.map(i=>String(i.id))); const receipts=(data.financeReceipts||[]).filter(r=>invIds.has(String(r.invoice_id)) || (!$('financeProjectFilter')?.value && !$('financeMonthFilter')?.value));
  const totalReceipts=receipts.reduce((a,r)=>a+num(r.amount),0), totalExp=expenses.reduce((a,e)=>a+num(e.total),0), due=Math.max(0,totalInv-totalReceipts), profit=totalReceipts-totalExp;
  if($('finKpiRevenue')) $('finKpiRevenue').textContent=money(totalInv); if($('finKpiReceipts')) $('finKpiReceipts').textContent=money(totalReceipts); if($('finKpiExpenses')) $('finKpiExpenses').textContent=money(totalExp); if($('finKpiProfit')) { $('finKpiProfit').textContent=money(profit); $('finKpiProfit').className=profit>=0?'money-pos':'money-neg'; } if($('finKpiDue')) $('finKpiDue').textContent=money(due);
}
function financeRenderRecent(){ const bi=$('finRecentInvoices'), be=$('finRecentExpenses'); if(bi){ const rows=[...(data.financeInvoices||[])].slice(0,6); bi.innerHTML=rows.map(i=>`<tr><td>${esc(i.invoice_no||i.id)}</td><td>${esc(i.project_name||financeProjectName(i.project_id))}</td><td>${money(i.total)}</td><td>${money(invoicePaid(i.id))}</td><td><span class="badge ${financeStatusClass(invoiceComputedStatus(i))}">${financeStatusText(invoiceComputedStatus(i))}</span></td></tr>`).join('')||`<tr><td colspan="5">${data.financeError||'لا توجد فواتير'}</td></tr>`; } if(be){ const rows=[...(data.financeExpenses||[])].slice(0,6); be.innerHTML=rows.map(e=>`<tr><td>${esc(e.expense_date||'')}</td><td>${esc(e.project_name||financeProjectName(e.project_id))}</td><td>${esc(e.category||'')}</td><td>${money(e.total)}</td></tr>`).join('')||`<tr><td colspan="4">${data.financeError||'لا توجد مصروفات'}</td></tr>`; } }
function financeRenderContracts(){ const b=$('financeContractsBody'); if(!b) return; const rows=data.financeContracts||[]; b.innerHTML=rows.map(c=>{ const total=num(c.monthly_amount)+num(c.monthly_amount)*num(c.vat_rate)/100; return `<tr><td><b>${esc(c.project_name||financeProjectName(c.project_id))}</b></td><td>${esc(c.contract_start||'-')}<br>${esc(c.contract_end||'-')}</td><td>${money(c.monthly_amount)}</td><td>${num(c.vat_rate)}%</td><td>${money(total)}</td><td>${c.billing_cycle==='quarterly'?'ربع سنوي':(c.billing_cycle==='installments'?'دفعات':'شهري')}</td><td class="row-actions"><button onclick="financeEditContract('${c.id}')">تعديل</button><button class="danger" onclick="financeDelete('finance_contracts','${c.id}')">حذف</button></td></tr>`; }).join('')||`<tr><td colspan="7">${data.financeError||'لا توجد عقود مالية'}</td></tr>`; }
async function financeEnsureProject(){ let pid=$('financeProjectId')?.value || $('financeProjectSelect')?.value; const name=($('financeProjectName')?.value||'').trim(); if(!pid){ if(!name) throw new Error('اسم المشروع مطلوب'); const row={name, location:$('financeProjectLocation')?.value||'', status:'active', required_daily_minutes:180, friday_minutes:90, operation_type:'daily_visit', visit_type_default:'surface'}; const {data:newp,error}=await sb.from('projects').insert(row).select('*').single(); if(error) throw error; data.projects.unshift(newp); pid=newp.id; financeHydrateForms(); } return pid; }
async function financeSaveContract(){ try{ const pid=await financeEnsureProject(), name=($('financeProjectName')?.value||projectName(pid)); const row={project_id:Number(pid),project_name:name,contract_start:$('financeContractStart')?.value||null,contract_end:$('financeContractEnd')?.value||null,monthly_amount:num($('financeMonthlyAmount')?.value),vat_rate:num($('financeVatRate')?.value||15),billing_cycle:$('financeBillingCycle')?.value||'monthly',contact_phone:$('financeContactPhone')?.value||'',notes:$('financeContractNotes')?.value||''}; const id=$('financeContractId')?.value; const res=id?await sb.from('finance_contracts').update(row).eq('id',id):await sb.from('finance_contracts').insert(row); if(res.error) throw res.error; msg(id?'تم تحديث العقد المالي':'تم حفظ العقد المالي'); financeClearContractForm(); await financeLoadAll(); }catch(e){ msg(e.message||String(e),'err'); } }
function financeClearContractForm(){ ['financeContractId','financeProjectId','financeProjectName','financeProjectLocation','financeMonthlyAmount','financeContactPhone','financeContractNotes'].forEach(id=>$(id)&&($(id).value='')); if($('financeProjectSelect')) $('financeProjectSelect').value=''; if($('financeVatRate')) $('financeVatRate').value=15; if($('financeBillingCycle')) $('financeBillingCycle').value='monthly'; if($('financeContractStart')) $('financeContractStart').value=today(); if($('financeContractEnd')) $('financeContractEnd').value=''; $('financeProjectFormTitle')&&($('financeProjectFormTitle').textContent='إضافة / تعديل عقد مالي'); }
function financeOpenNewProject(){ showPage('financeDashboard', document.querySelector('.nav[onclick*=financeDashboard]')); financeShowTab('contracts', document.querySelectorAll('.finance-tab')[1]); financeClearContractForm(); $('financeProjectName')?.focus(); }
function financeFillProjectFromSelect(){ const pid=$('financeProjectSelect')?.value; if(!pid) return financeClearContractForm(); const p=(data.projects||[]).find(x=>String(x.id)===String(pid)); const c=(data.financeContracts||[]).find(x=>String(x.project_id)===String(pid)); $('financeProjectId').value=pid; $('financeProjectName').value=p?.name||''; $('financeProjectLocation').value=p?.location||''; if(c) financeEditContract(c.id,false); }
function financeEditContract(id,scroll=true){ const c=(data.financeContracts||[]).find(x=>String(x.id)===String(id)); if(!c) return; const p=(data.projects||[]).find(x=>String(x.id)===String(c.project_id)); $('financeContractId').value=c.id; $('financeProjectId').value=c.project_id||''; $('financeProjectSelect').value=c.project_id||''; $('financeProjectName').value=c.project_name||p?.name||''; $('financeProjectLocation').value=p?.location||''; $('financeContractStart').value=c.contract_start||''; $('financeContractEnd').value=c.contract_end||''; $('financeMonthlyAmount').value=c.monthly_amount||0; $('financeVatRate').value=c.vat_rate??15; $('financeBillingCycle').value=c.billing_cycle||'monthly'; $('financeContactPhone').value=c.contact_phone||''; $('financeContractNotes').value=c.notes||''; $('financeProjectFormTitle').textContent='تعديل عقد مالي'; if(scroll) $('financeProjectFormTitle')?.scrollIntoView({behavior:'smooth'}); }
function financeSuggestInvoiceAmount(){ const pid=$('financeInvoiceProject')?.value, c=(data.financeContracts||[]).find(x=>String(x.project_id)===String(pid)); if(c){ $('financeInvoiceSubtotal').value=num(c.monthly_amount).toFixed(2); $('financeInvoiceVat').value=(num(c.monthly_amount)*num(c.vat_rate)/100).toFixed(2); financeCalcInvoiceTotal(); } }
function financeCalcInvoiceTotal(){ if($('financeInvoiceTotal')) $('financeInvoiceTotal').value=(num($('financeInvoiceSubtotal')?.value)+num($('financeInvoiceVat')?.value)).toFixed(2); }
function financeCalcExpenseTotal(){ if($('financeExpenseTotal')) $('financeExpenseTotal').value=(num($('financeExpenseSubtotal')?.value)+num($('financeExpenseVat')?.value)).toFixed(2); }
function financeInvoiceNo(){ return 'INV-'+new Date().toISOString().slice(0,10).replace(/-/g,'')+'-'+String(Date.now()).slice(-4); }
async function financeSaveInvoice(){ try{ const pid=$('financeInvoiceProject')?.value; if(!pid) throw new Error('اختر المشروع'); const row={project_id:Number(pid),project_name:projectName(pid),invoice_no:($('financeInvoiceId')?.dataset.invoiceNo)||financeInvoiceNo(),invoice_type:$('financeInvoiceType')?.value||'فاتورة عقد شهري',issue_date:$('financeInvoiceDate')?.value||today(),due_date:$('financeInvoiceDue')?.value||today(),subtotal:num($('financeInvoiceSubtotal')?.value),vat:num($('financeInvoiceVat')?.value),total:num($('financeInvoiceTotal')?.value),status:$('financeInvoiceStatus')?.value||'draft',notes:$('financeInvoiceNotes')?.value||''}; const id=$('financeInvoiceId')?.value; const res=id?await sb.from('finance_invoices').update(row).eq('id',id):await sb.from('finance_invoices').insert(row); if(res.error) throw res.error; msg(id?'تم تحديث الفاتورة':'تم حفظ الفاتورة'); financeClearInvoiceForm(); await financeLoadAll(); }catch(e){ msg(e.message||String(e),'err'); } }
function financeClearInvoiceForm(){ ['financeInvoiceId','financeInvoiceSubtotal','financeInvoiceVat','financeInvoiceTotal','financeInvoiceNotes'].forEach(id=>$(id)&&($(id).value='')); if($('financeInvoiceId')) delete $('financeInvoiceId').dataset.invoiceNo; if($('financeInvoiceDate')) $('financeInvoiceDate').value=today(); if($('financeInvoiceDue')) $('financeInvoiceDue').value=today(); if($('financeInvoiceStatus')) $('financeInvoiceStatus').value='draft'; }
function financeRenderInvoices(){ const b=$('financeInvoicesBody'); if(!b) return; let rows=financeProjectFilterRows(data.financeInvoices||[],'issue_date'); const st=$('financeInvoiceStatusFilter')?.value; if(st) rows=rows.filter(i=>invoiceComputedStatus(i)===st); b.innerHTML=rows.map(i=>{ const paid=invoicePaid(i.id), due=invoiceDue(i); return `<tr><td><b>${esc(i.invoice_no||i.id)}</b></td><td>${esc(i.issue_date||'')}</td><td>${esc(i.project_name||financeProjectName(i.project_id))}</td><td>${esc(i.invoice_type||'')}</td><td>${money(i.total)}</td><td>${money(paid)}</td><td>${money(due)}</td><td><span class="badge ${financeStatusClass(invoiceComputedStatus(i))}">${financeStatusText(invoiceComputedStatus(i))}</span></td><td class="row-actions"><button onclick="financeEditInvoice('${i.id}')">تعديل</button><button class="light" onclick="financePrepareReceipt('${i.id}')">قبض</button><button class="danger" onclick="financeDelete('finance_invoices','${i.id}')">حذف</button></td></tr>`; }).join('')||`<tr><td colspan="9">${data.financeError||'لا توجد فواتير'}</td></tr>`; }
function financeEditInvoice(id){ const i=(data.financeInvoices||[]).find(x=>String(x.id)===String(id)); if(!i) return; $('financeInvoiceId').value=i.id; $('financeInvoiceId').dataset.invoiceNo=i.invoice_no||''; $('financeInvoiceProject').value=i.project_id||''; $('financeInvoiceType').value=i.invoice_type||'فاتورة عقد شهري'; $('financeInvoiceDate').value=i.issue_date||today(); $('financeInvoiceDue').value=i.due_date||today(); $('financeInvoiceSubtotal').value=i.subtotal||0; $('financeInvoiceVat').value=i.vat||0; $('financeInvoiceTotal').value=i.total||0; $('financeInvoiceStatus').value=i.status||'draft'; $('financeInvoiceNotes').value=i.notes||''; financeShowTab('invoices',document.querySelectorAll('.finance-tab')[2]); $('financeInvoiceFormTitle')?.scrollIntoView({behavior:'smooth'}); }
function financeFillInvoiceSelect(){ const el=$('financeReceiptInvoice'); if(!el) return; el.innerHTML='<option value="">اختر فاتورة</option>'+ (data.financeInvoices||[]).map(i=>`<option value="${i.id}">${esc(i.invoice_no||i.id)} - ${esc(i.project_name||financeProjectName(i.project_id))} - المتبقي ${money(invoiceDue(i))}</option>`).join(''); }
function financeFillReceiptInvoice(){ const id=$('financeReceiptInvoice')?.value, i=(data.financeInvoices||[]).find(x=>String(x.id)===String(id)); if(i){ $('financeReceiptProject').value=i.project_id||''; $('financeReceiptAmount').value=invoiceDue(i).toFixed(2); } }
function financePrepareReceipt(id){ financeShowTab('receipts',document.querySelectorAll('.finance-tab')[3]); $('financeReceiptInvoice').value=id; financeFillReceiptInvoice(); }
async function financeSaveReceipt(){ try{ const iid=$('financeReceiptInvoice')?.value; if(!iid) throw new Error('اختر الفاتورة'); const inv=(data.financeInvoices||[]).find(x=>String(x.id)===String(iid)); const row={invoice_id:Number(iid),project_id:Number($('financeReceiptProject')?.value||inv?.project_id),project_name:projectName($('financeReceiptProject')?.value||inv?.project_id),amount:num($('financeReceiptAmount')?.value),payment_date:$('financeReceiptDate')?.value||today(),method:$('financeReceiptMethod')?.value||'تحويل',reference_no:$('financeReceiptRef')?.value||'',notes:$('financeReceiptNotes')?.value||''}; const id=$('financeReceiptId')?.value; const res=id?await sb.from('finance_receipts').update(row).eq('id',id):await sb.from('finance_receipts').insert(row); if(res.error) throw res.error; msg('تم حفظ سند القبض'); financeClearReceiptForm(); await financeLoadAll(); await financeUpdateInvoiceStatus(iid); }catch(e){ msg(e.message||String(e),'err'); } }
async function financeUpdateInvoiceStatus(iid){ const inv=(data.financeInvoices||[]).find(x=>String(x.id)===String(iid)); if(!inv) return; const st=invoiceComputedStatus(inv); await sb.from('finance_invoices').update({status:st}).eq('id',iid); await financeLoadAll(false); }
function financeClearReceiptForm(){ ['financeReceiptId','financeReceiptInvoice','financeReceiptAmount','financeReceiptRef','financeReceiptNotes'].forEach(id=>$(id)&&($(id).value='')); if($('financeReceiptDate')) $('financeReceiptDate').value=today(); }
function financeRenderReceipts(){ const b=$('financeReceiptsBody'); if(!b) return; const rows=financeProjectFilterRows(data.financeReceipts||[],'payment_date'); b.innerHTML=rows.map(r=>`<tr><td>${esc(r.payment_date||'')}</td><td>${esc(r.project_name||financeProjectName(r.project_id))}</td><td>${esc((data.financeInvoices||[]).find(i=>String(i.id)===String(r.invoice_id))?.invoice_no||r.invoice_id)}</td><td>${money(r.amount)}</td><td>${esc(r.method||'')}</td><td>${esc(r.reference_no||'')}</td><td><button class="danger" onclick="financeDelete('finance_receipts','${r.id}')">حذف</button></td></tr>`).join('')||`<tr><td colspan="7">${data.financeError||'لا توجد سندات قبض'}</td></tr>`; }
async function financeSaveExpense(){ try{ const pid=$('financeExpenseProject')?.value; if(!pid) throw new Error('اختر المشروع'); const row={project_id:Number(pid),project_name:projectName(pid),category:$('financeExpenseCategory')?.value||'مصروف عام',expense_date:$('financeExpenseDate')?.value||today(),subtotal:num($('financeExpenseSubtotal')?.value),vat:num($('financeExpenseVat')?.value),total:num($('financeExpenseTotal')?.value),supplier:$('financeExpenseSupplier')?.value||'',payment_method:$('financeExpenseMethod')?.value||'تحويل',notes:$('financeExpenseNotes')?.value||''}; const id=$('financeExpenseId')?.value; const res=id?await sb.from('finance_expenses').update(row).eq('id',id):await sb.from('finance_expenses').insert(row); if(res.error) throw res.error; msg('تم حفظ المصروف'); financeClearExpenseForm(); await financeLoadAll(); }catch(e){ msg(e.message||String(e),'err'); } }
...
function financeRenderExpenses(){ const b=$('financeExpensesBody'); if(!b) return; let rows=financeProjectFilterRows(data.financeExpenses||[],'expense_date'); const cat=$('financeExpenseTypeFilter')?.value; if(cat) rows=rows.filter(e=>e.category===cat); b.innerHTML=rows.map(e=>`<tr><td>${esc(e.expense_date||'')}</td><td>${esc(e.project_name||financeProjectName(e.project_id))}</td><td>${esc(e.category||'')}</td><td>${esc(e.supplier||'-')}</td><td>${money(e.total)}</td><td>${esc(e.payment_method||'')}</td><td class="row-actions"><button onclick="financeEditExpense('${e.id}')">تعديل</button><button class="danger" onclick="financeDelete('finance_expenses','${e.id}')">حذف</button></td></tr>`).join('')||`<tr><td colspan="7">${data.financeError||'لا توجد مصروفات'}</td></tr>`; }
function financeEditExpense(id){ const e=(data.financeExpenses||[]).find(x=>String(x.id)===String(id)); if(!e) return; $('financeExpenseId').value=e.id; $('financeExpenseProject').value=e.project_id||''; $('financeExpenseCategory').value=e.category||'مصروف عام'; $('financeExpenseDate').value=e.expense_date||today(); $('financeExpenseSubtotal').value=e.subtotal||0; $('financeExpenseVat').value=e.vat||0; $('financeExpenseTotal').value=e.total||0; $('financeExpenseSupplier').value=e.supplier||''; $('financeExpenseMethod').value=e.payment_method||'تحويل'; $('financeExpenseNotes').value=e.notes||''; financeShowTab('expenses',document.querySelectorAll('.finance-tab')[4]); $('financeExpenseId')?.scrollIntoView({behavior:'smooth'}); }
function financeRenderProfitability(){ const b=$('financeProfitabilityBody'); if(!b) return; const rows=(data.projects||[]).map(p=>{ const inv=(data.financeInvoices||[]).filter(i=>String(i.project_id)===String(p.id)&&invoiceComputedStatus(i)!=='cancelled'); const total=inv.reduce((a,i)=>a+num(i.total),0), paid=inv.reduce((a,i)=>a+invoicePaid(i.id),0), due=Math.max(0,total-paid); const exp=(data.financeExpenses||[]).filter(e=>String(e.project_id)===String(p.id)).reduce((a,e)=>a+num(e.total),0); const profit=paid-exp, pct=paid?profit/paid*100:0; let status=profit<0?'خاسر':(pct>=25?'ممتاز':(pct>=10?'جيد':'ضعيف')); return {p,total,paid,due,exp,profit,pct,status}; }).filter(x=>x.total||x.exp); b.innerHTML=rows.map(x=>`<tr><td><b>${esc(x.p.name)}</b></td><td>${money(x.total)}</td><td>${money(x.paid)}</td><td>${money(x.due)}</td><td>${money(x.exp)}</td><td class="${x.profit>=0?'money-pos':'money-neg'}">${money(x.profit)}</td><td>${x.paid?x.pct.toFixed(1)+'%':'-'}</td><td><span class="badge ${x.status==='خاسر'?'red':(x.status==='ممتاز'?'green':'amber')}">${x.status}</span></td></tr>`).join('')||'<tr><td colspan="8">لا توجد بيانات مالية للمشاريع</td></tr>'; }
async function financeDelete(table,id){ if(!confirm('تأكيد الحذف؟')) return; const {error}=await sb.from(table).delete().eq('id',id); if(error) return msg(error.message,'err'); msg('تم الحذف'); await financeLoadAll(); }
function financeResetFilters(){ ['financeSearch','financeProjectFilter','financeInvoiceStatusFilter','financeExpenseTypeFilter'].forEach(id=>$(id)&&($(id).value='')); if($('financeMonthFilter')) $('financeMonthFilter').value=today().slice(0,7); financeRenderAll(); }
