/* ===== TASNEEF V10210 - ATTENDANCE ONE NAME STABLE =====
   هدف النسخة:
...
(function(){
  'use strict';
  if(window.__tasneefAttendanceOneNameStableV10210) return;
  window.__tasneefAttendanceOneNameStableV10210 = true;
  const BUILD='V10210';
  ...
  async function deleteExistingAttendanceForName(date,name,ids){
    const cleanName=S(name);
    const cleanIds=(ids||[]).map(x=>Number(x)).filter(Boolean);
    // حذف حسب الاسم المعروض حتى لا تبقى سجلات قديمة لنفس الاسم
    if(cleanName){
      let r=await window.sb.from('attendance').delete().eq('attendance_date',date).eq('worker_identity',cleanName);
      if(r.error) throw r.error;
    }
    // حذف حسب كل worker_id التابع لنفس الاسم حتى لا يظل تكرار أو تعارض قديم
    if(cleanIds.length){
      let r=await window.sb.from('attendance').delete().eq('attendance_date',date).in('worker_id',cleanIds);
      if(r.error) throw r.error;
    }
  }
  async function insertAttendanceRowSafe(row,ids){
    const payload=Object.assign({},row);
    let r=await window.sb.from('attendance').insert(payload);
    if(!r.error) return true;
    // fallback: في حال بقي سجل قديم بسبب RLS أو constraint، حدث السجل الموجود بدل إيقاف الحفظ
    let updatePayload={status:payload.status,notes:payload.notes||'',supervisor_id:payload.supervisor_id||null,project_id:payload.project_id||null,worker_identity:payload.worker_identity,worker_id:payload.worker_id||null,created_at:payload.created_at||new Date().toISOString()};
    let u=await window.sb.from('attendance').update(updatePayload).eq('attendance_date',payload.attendance_date).eq('worker_identity',payload.worker_identity);
    if(!u.error) return true;
    if(payload.worker_id){
      u=await window.sb.from('attendance').update(updatePayload).eq('attendance_date',payload.attendance_date).eq('worker_id',payload.worker_id);
      if(!u.error) return true;
    }
    throw r.error;
  }
  ...
  async function saveOne(row,ids){
    // الحل الجذري: اسم واحد فقط في اليوم. نحذف كل سجلات نفس الاسم وكل IDs المرتبطة به ثم ندخل سجل واحد.
    await deleteExistingAttendanceForName(row.attendance_date,row.worker_identity,ids);
    await insertAttendanceRowSafe(row,ids);
  }
  async function adminSave(){const date=$('attStableDate')?.value||today(); const cards=[...document.querySelectorAll('#attStableWorkers .att-stable-worker[data-name]')]; if(!cards.length)return msg2('لا يوجد عمال للحفظ','err'); let ok=0; for(const c of cards){const name=c.dataset.name; const ids=S(c.dataset.ids||'').split(',').map(x=>Number(x)).filter(Boolean); const wid=ids[0]||N(c.dataset.wid)||null; const worker=(adminState.workers||[]).find(w=>S(w.id)===S(wid))||{}; await saveOne({attendance_date:date,worker_id:wid,worker_identity:name,supervisor_id:workerSupId(worker,adminState.projects)||null,project_id:worker.project_id||null,status:c.querySelector('select')?.value||'present',notes:c.querySelector('input')?.value||'',created_at:new Date().toISOString()},ids); ok++;} msg2('تم حفظ تحضير '+ok+' اسم بدون تكرار'); await adminRender(true);}
  ...
  async function supervisorSave(){const u=currentUser(); const date=$('attendanceDate')?.value||today(); const cards=[...document.querySelectorAll('#supStableWorkers .att-stable-worker[data-name]')]; if(!cards.length)return msg2('لا يوجد عمال للحفظ','err'); let ok=0; for(const c of cards){const name=c.dataset.name; const ids=S(c.dataset.ids||'').split(',').map(x=>Number(x)).filter(Boolean); const wid=ids[0]||N(c.dataset.wid)||null; await saveOne({attendance_date:date,worker_id:wid,worker_identity:name,supervisor_id:N(u.id)||null,project_id:null,status:c.querySelector('select')?.value||'present',notes:c.querySelector('input')?.value||'',created_by:N(u.id)||null,created_at:new Date().toISOString()},ids); ok++;} msg2('تم حفظ تحضير '+ok+' اسم بدون تكرار'); await supervisorRender();}
