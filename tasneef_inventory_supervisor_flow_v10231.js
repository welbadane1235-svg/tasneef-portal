(function(){
  ...
  async function selectInventoryItemsV10231(){
    const c=client();
    const local=A(window.data?.inventoryItems||window.inventoryItems||window.inventory_items||[]);
    const attempts=[
      ['inventory_items','*','name',true],
      ['inventory_items','*','product_name',true],
      ['inventory_items','*','item_name',true],
      ['inventory_items','*','created_at',false],
      ['inventory_items','*',null,true]
    ];
    for(const a of attempts){
      try{
        const rows=await selectAll(a[0],a[1],a[2],a[3]);
        if(rows && rows.length) return rows;
      }catch(e){ console.warn('inventory items load attempt failed', a[2], e?.message||e); }
    }
  ...
  async function loadBase(){
    ...
    const [items,projects,requests,cons]=await Promise.all([
      selectInventoryItemsV10231().catch((e)=>{ console.warn('تعذر تحميل المنتجات', e?.message||e); return A(window.data?.inventoryItems||window.inventoryItems||[]); }),
    ...
    state.items=A(items).filter(i=>S(i.id)&&itemName(i)); state.projects=projects; state.requests=requests; state.consumptions=cons;
    if(!state.items.length) console.warn('V10231: لم يتم العثور على منتجات في inventory_items أو الذاكرة المحلية');
  }
  ...
  function fillItems(selectId){ const el=$(selectId); if(!el) return; const old=el.value; const rows=A(state.items); el.innerHTML='<option value="">'+(rows.length?'اختر المنتج':'لا توجد منتجات - تأكد من صلاحية قراءة inventory_items')+'</option>'+rows.map(i=>`<option value="${esc(i.id)}">${esc(itemName(i))} - المتوفر ${N(i.quantity)} ${esc(itemUnit(i))}</option>`).join(''); el.value=old; }
  ...
  window.InvSupervisorFlowV10230=api; window.InvSupervisorFlowV10231=api;
