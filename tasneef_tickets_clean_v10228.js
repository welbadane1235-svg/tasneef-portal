/* Tasneef V10228 - Tickets section clean single source
...
(function(){
  'use strict';
  if(window.__tasneefTicketsCleanV10228) return;
  window.__tasneefTicketsCleanV10228 = true;

  const VERSION='V10228';
  ...
  function ensureCleanFilters(){
    ...
    let wrap=$('ticketFiltersV10228');
    if(!wrap){
      const card = body.closest('.card') || body.parentElement;
      const old = card && card.querySelector('.filters');
      wrap = document.createElement('div');
      wrap.id='ticketFiltersV10228';
      wrap.className='filters ticket-clean-filters-v10228';
      wrap.innerHTML = `
    ...
    ['ticketCleanProject','ticketCleanSupervisor','ticketCleanTitle','ticketCleanStatus','ticketCleanClaimed','ticketCleanClosedBy','ticketCleanSort'].forEach(id=>{
      const el=$(id); if(el && !el.dataset.cleanBound10228){ el.dataset.cleanBound10228='1'; el.addEventListener('change', renderTicketsClean, true); }
    });
    const q=$('ticketCleanSearch');
    if(q && !q.dataset.cleanBound10228){ q.dataset.cleanBound10228='1'; q.addEventListener('input', renderTicketsClean, true); }
  }
  ...
  function sendTicketWhatsappV10228(id){
  ...
  window.sendTicketWhatsappV10228 = sendTicketWhatsappV10228;
  function ticketCard(t){
    ...
    const actions = [
      ...
      `<button type="button" class="light" onclick="sendTicketWhatsappV10228(${id})">إرسال واتساب</button>`,
      `<button type="button" class="danger" onclick="deleteRow('tickets',${id})">حذف</button>`
    ].filter(Boolean).join(' ');
    return `<article class="smart-ticket-card ticket-clean-card-v10228 ${statusClass(t.status)}" data-ticket-id="${E(id)}">
  ...
  function updateSummary(rows){
    ...
    box.innerHTML = `<div class="ticket-clean-summary-v10228"><span class="ticket-root-v10225-kpi">الإجمالي: <b>${total}</b></span><span class="ticket-root-v10225-kpi">مفتوح: <b>${open}</b></span><span class="ticket-root-v10225-kpi">تحت المعالجة: <b>${proc}</b></span><span class="ticket-root-v10225-kpi">مغلق: <b>${closed}</b></span></div>`;
  }
  function addStyle(){
    if($('ticketCleanV10228Style')) return;
    const st=document.createElement('style'); st.id='ticketCleanV10228Style';
    st.textContent = `
      .ticket-clean-filters-v10228{display:grid!important;grid-template-columns:repeat(2,minmax(170px,1fr));gap:8px!important;margin:10px 0 14px!important}
      .ticket-clean-filters-v10228 select,.ticket-clean-filters-v10228 input{width:100%!important;color:#062f26!important;background:#fff!important;border:1px solid #cfe2dc!important;border-radius:10px!important;padding:10px!important}
      .ticket-clean-filters-v10228 option{color:#111!important;background:#fff!important}
      .ticket-clean-version{color:#0b5d49;font-weight:800;align-self:center}
      .ticket-clean-summary-v10228{display:flex;gap:10px;flex-wrap:wrap;justify-content:flex-end;margin:10px 0}
      @media(max-width:800px){.ticket-clean-filters-v10228{grid-template-columns:1fr!important}}
    `;
  ...
  window.renderTicketsCleanV10228 = renderTicketsClean;
