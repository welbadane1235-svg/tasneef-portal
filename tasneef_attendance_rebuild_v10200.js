/* ===== V10200 ROOT REBUILD: Attendance section rebuilt from zero, data preserved ===== */
(function(){
  'use strict';
  if(window.__tasneefAttendanceRebuildV10200) return; window.__tasneefAttendanceRebuildV10200=true;
  const VERSION='10200';
  ...
  function supervisors(){return st.users.filter(u=>isActive(u)&&S(u.role||'').includes('supervisor')).sort((a,b)=>userName(a).localeCompare(userName(b),'ar'))}
  ...
  async function safe(q){try{const r=await q; if(r.error){console.warn('attendance v10199',r.error.message);return []} return r.data||[]}catch(e){console.warn('attendance v10199',e);return []}}
  async function loadFresh(){
    try{ if(typeof window.loadAll==='function') await window.loadAll(); }catch(_){}
    const d=D();
    if(window.sb){
      const [users,projects,workers,att]=await Promise.all([
        safe(sb.from('app_users').select('*').order('id')),
        safe(sb.from('projects').select('*').order('id')),
        safe(sb.from('workers').select('*').order('id')),
        safe(sb.from('attendance').select('*').order('attendance_date',{ascending:false}).limit(5000))
      ]);
      st.users=users.length?users:(d.users||d.supervisors||[]); st.projects=projects.length?projects:(d.projects||[]); st.workers=workers.length?workers:(d.workers||[]); st.attendance=att.length?att:(d.attendance||[]);
      const dd=D(); dd.users=st.users; dd.supervisors=supervisors(); dd.projects=st.projects; dd.workers=st.workers; dd.attendance=st.attendance;
    }else{ st.users=d.users||d.supervisors||[]; st.projects=d.projects||[]; st.workers=d.workers||[]; st.attendance=d.attendance||[]; }
    st.loaded=true; console.log('Attendance V10200 loaded',{users:st.users.length,projects:st.projects.length,workers:st.workers.length,attendance:st.attendance.length});
  }
  function installAdmin(){
    const sec=$('attendance'); if(!sec || sec.dataset.attRb10199) return !!sec;
    sec.dataset.attRb10199='1';
    sec.innerHTML=`<div class="att-rb-root">
      ...
      <div class="att-rb-controls">
        ...
        <button type="button" id="attRbRefresh" onclick="window.renderAttendanceRootV10200&&window.renderAttendanceRootV10200(true)">تحديث مباشر</button>
        <button type="button" id="attRbSave" onclick="window.saveAttendanceRootV10200&&window.saveAttendanceRootV10200()">حفظ التحضير</button>
        <button type="button" id="attRbPrint" onclick="window.printAttendanceRootV10200&&window.printAttendanceRootV10200()">طباعة</button>
        <button type="button" id="attRbCsv" onclick="window.exportAttendanceRootV10200&&window.exportAttendanceRootV10200()">تصدير CSV</button>
      </div>
  ...
  function dayRecords(date){const m=new Map(); (st.attendance||[]).filter(a=>S(a.attendance_date)===date).forEach(a=>m.set(id(a.worker_id),a)); return m;}
  ...
  function renderMatrix(workers,m){const days=daysOfMonth(m); const h=$('attRbMatrixHead'), b=$('attRbMatrixBody'); if(h) h.innerHTML='<tr><th>العامل</th>'+days.map(d=>`<th>${d}</th>`).join('')+'<th>حضور</th><th>غياب</th></tr>'; if(b) b.innerHTML=workers.map(w=>{let p=0,a=0; const cells=days.map(d=>{const ds=m+'-'+d; const r=(st.attendance||[]).find(x=>id(x.worker_id)===id(w.id)&&S(x.attendance_date)===ds); if(r?.status==='present'){p++;return '<td class="att-present">ح</td>'} if(r?.status==='absent'){a++;return '<td class="att-absent">غ</td>'} return '<td class="att-empty">-</td>';}).join(''); return `<tr><td><b>${esc(workerName(w))}</b></td>${cells}<td>${p}</td><td>${a}</td></tr>`}).join('')||'<tr><td>لا توجد بيانات</td></tr>';}
  ...
  window.renderAttendanceRootV10200=renderAdmin; window.saveAttendanceRootV10200=saveAdmin; window.printAttendanceRootV10200=printAdmin; window.exportAttendanceRootV10200=exportCsv;
  window.renderAttendanceRootV10199=renderAdmin; window.saveAttendanceRootV10199=saveAdmin; window.printAttendanceRootV10199=printAdmin; window.exportAttendanceRootV10199=exportCsv;
  ...
  console.log('Tasneef V10200 attendance rebuilt from zero loaded');
})();
