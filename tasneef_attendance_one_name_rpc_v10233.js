...
(function(){
  ...
  async function selectAttendanceMonth(month){
    const r=monthRange(month||monthNow());
    const all=[]; let from=0; const size=1000;
    while(true){
      const q=await window.sb.from('attendance').select('*').gte('attendance_date',r.from).lt('attendance_date',r.to).order('attendance_date',{ascending:true}).order('created_at',{ascending:true}).range(from,from+size-1);
    ...
    console.log('attendance month rows loaded',month,all.length);
    return all;
  }
  async function selectAttendanceDay(date){
    const d=date||today();
    const all=[]; let from=0; const size=1000;
    while(true){
      const q=await window.sb.from('attendance').select('*').eq('attendance_date',d).order('created_at',{ascending:true}).range(from,from+size-1);
      if(q.error) throw q.error;
      const rows=q.data||[]; all.push(...rows);
      if(rows.length<size) break;
      from+=size;
  ...
  async function loadBase(month){if(!window.sb) throw new Error('Supabase غير جاهز'); const [users,projects,workers,attendance]=await Promise.all([
    selectAll('app_users','*'),selectAll('projects','*'),selectAll('workers','*'),selectAttendanceMonth(month||monthNow())
  ...
  const oldShow=window.showPage; window.showPage=function(page,btn){const r=oldShow?oldShow.apply(this,arguments):undefined; if(page==='attendance') setTimeout(()=>renderAdmin(false),150); return r;};
  ...
  document.addEventListener('DOMContentLoaded',()=>{bindGlobals(); setTimeout(()=>{if($('attendance')&&!$('attendance').classList.contains('hidden'))renderAdmin(false); if($('supervisorAttendanceList'))renderSupervisor();},1200); setTimeout(()=>{if($('attendance')&&!$('attRootV10215')&&!$('attendance').classList.contains('hidden'))renderAdmin(false);},3500);});
  try{new MutationObserver(()=>{if($('attendance')&&!$('attendance').classList.contains('hidden')&&!$('attRootV10215')) setTimeout(()=>renderAdmin(false),50);}).observe(document.body,{childList:true,subtree:true});}catch(_){}
  console.log('Tasneef attendance v10233 reduced auto refresh loaded '+BUILD);
})();
