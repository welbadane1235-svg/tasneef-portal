...
(function(){
  ...
  async function saveBatchByName(date, items){
    ...
    const r = await window.sb.rpc('tasneef_save_attendance_by_name_v10214', {p_date: date, p_records: payload});
  ...
  async function saveAdmin(){try{
    ...
    const cards=[...document.querySelectorAll('#attWorkers .att-worker[data-name]')];
    if(!cards.length)return toast('لا يوجد عمال للحفظ',true);
    const items=[];
    for(const c of cards){
      const name=c.dataset.name;
      const ids=S(c.dataset.ids).split(',').map(Number).filter(Boolean);
      const worker=(adminState.workers||[]).find(w=>S(w.id)===S(ids[0]))||{};
      items.push({name,status:c.querySelector('select').value,notes:c.querySelector('input').value,supervisor_id:wSup(worker,adminState.projects)||null,project_id:worker.project_id||null});
    }
    const result=await saveBatchByName(date,items);
    toast('تم حفظ '+(result.saved||items.length)+' سجل فعليًا');
  ...
  async function saveSupervisor(){try{
    ...
    const cards=[...document.querySelectorAll('#supWorkers .att-worker[data-name]')];
    if(!cards.length)return toast('لا يوجد عمال للحفظ',true);
    const st=window.__supAttState||{};
    const items=[];
    for(const c of cards){
      const ids=S(c.dataset.ids).split(',').map(Number).filter(Boolean);
      const worker=(st.d?.workers||[]).find(w=>S(w.id)===S(ids[0]))||{};
      items.push({name:c.dataset.name,status:c.querySelector('select').value,notes:c.querySelector('input').value,supervisor_id:wSup(worker,st.d?.projects||[])||N(u.id)||null,project_id:worker.project_id||null});
    }
    const result=await saveBatchByName(date,items);
    toast('تم حفظ '+(result.saved||items.length)+' سجل فعليًا');
  ...
  document.addEventListener('DOMContentLoaded',()=>{bindGlobals(); setTimeout(()=>{if($('attendance')&&!$('attendance').classList.contains('hidden'))renderAdmin(true); if($('supervisorAttendanceList'))renderSupervisor();},1200); setTimeout(()=>{if($('attendance')&&!$('attRootV10213')&&!$('attendance').classList.contains('hidden'))renderAdmin(true);},3500);});
  try{new MutationObserver(()=>{if($('attendance')&&!$('attendance').classList.contains('hidden')&&!$('attRootV10213')) setTimeout(()=>renderAdmin(true),50);}).observe(document.body,{childList:true,subtree:true});}catch(_){}
  console.log('Tasneef attendance v10214 RPC save loaded '+BUILD);
})();
