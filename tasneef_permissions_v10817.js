...
(function(){
  ...
  const ROLE_LABELS={super_admin:'المدير العام',operations_manager:'مدير التشغيل',sales_manager:'مدير المبيعات',accountant:'المحاسب',maintenance_manager:'مسؤول الصيانة',supervisor:'مشرف',technician:'فني',sales_employee:'موظف مبيعات',read_only:'مراقب قراءة فقط',custom:'مستخدم مخصص'};
  ...
  const state={loaded:false,userId:'',version:0,roleKey:'',effective:{},sources:{},scopeType:'all',projectIds:[],loading:null,lastLoadAt:0};
  ...
  function ensureRoleOptions(){
    ...
    const roleEl=$('userRole');if(roleEl&&!roleEl.__permissionsV10817){roleEl.__permissionsV10817=true;roleEl.addEventListener('change',renderPermissionMatrix);}
  }
  function legacyRouteRole(role){return ({super_admin:'admin',operations_manager:'operations_manager',accountant:'financial_manager',maintenance_manager:'operations_manager',supervisor:'supervisor',technician:'technician',sales_manager:'general_manager',sales_employee:'general_manager',read_only:'general_manager',custom:'general_manager'}[role]||role||'general_manager');}
  ...
  function renderPermissionMatrix(){
    ensureAdminToolbar();const box=$('permissionMatrixV10700');if(!box)return;box.className='permission-matrix-v10817';const q=S($('permissionSearchV10817')?.value).toLowerCase();const groups={};CATALOG.filter(p=>!q||[p.permission_key,p.module_name,p.action_name].some(v=>S(v).toLowerCase().includes(q))).forEach(p=>(groups[p.module]??=[]).push(p));
  ...
  window.permissionOverrideChangedV10817=sel=>{const k=sel.dataset.permissionKey;adminState.overrides[k]=sel.value==='allow'?true:sel.value==='deny'?false:null;if(sel.value==='inherit')delete adminState.overrides[k];renderPermissionMatrix();};
  window.setPermissionModuleV10817=(module,mode)=>{CATALOG.filter(p=>p.module===module).forEach(p=>{if(mode==='inherit')delete adminState.overrides[p.permission_key];else adminState.overrides[p.permission_key]=mode==='allow';});renderPermissionMatrix();};
  window.setAllPermissionsV10700=on=>{CATALOG.forEach(p=>adminState.overrides[p.permission_key]=!!on);renderPermissionMatrix();};
  window.applyRoleTemplateV10817=()=>{const rk=S($('permissionRoleTemplateV10817')?.value);if(!rk)return;const roleSelect=$('userRole');if(roleSelect&&[...roleSelect.options].some(o=>o.value===rk))roleSelect.value=rk;adminState.overrides={};renderPermissionMatrix();toast('تم تطبيق قالب الدور. احفظ لتثبيت التغيير.');};
  ...
  window.saveUser=async function(){
    ...
    const btn=document.querySelector('.perm-save button');if(btn?.disabled)return;btn&&(btn.disabled=true);try{const c=client();let bundle=null,targetId=Number(id)||null;try{bundle=await callRpc('save_app_user_permissions_v10817',{p_session_token:token(),p_target_user_id:targetId,p_user_data:base,p_role_key:base.role_key,p_changes:changedOverrides(),p_scope_type:base.scope_type,p_project_ids:selectedProjects,p_reason:reason,p_source:'users_admin'});targetId=Number(bundle?.saved_user_id||bundle?.user?.id||targetId);}catch(e){if(!isMissingRpc(e))throw e;let saved;if(id){const r=await c.from('app_users').update(base).eq('id',Number(id)).select('*').maybeSingle();if(r.error)throw r.error;saved=r.data;}else{const r=await c.from('app_users').insert({...base,created_by:Number(currentUser().id)||null}).select('*').single();if(r.error)throw r.error;saved=r.data;}targetId=Number(saved?.id||id);try{bundle=await callRpc('save_user_permission_bundle_v10817',{p_session_token:token(),p_target_user_id:targetId,p_role_key:base.role_key,p_changes:changedOverrides(),p_scope_type:base.scope_type,p_project_ids:selectedProjects,p_reason:reason,p_source:'users_admin'});}catch(inner){if(!isMissingRpc(inner))throw inner;await c.from('app_users').update({permissions:adminState.overrides,allowed_project_ids:selectedProjects,scope_type:base.scope_type,permissions_version:Number(saved?.permissions_version||0)+1}).eq('id',targetId);}}
      clearPermissionCache(targetId);broadcast?.postMessage({type:'permissions-updated',userId:targetId});toast('تم حفظ المستخدم والصلاحيات داخل عملية واحدة');$('advancedUserDrawerV10700')?.classList.add('hidden');if(typeof window.refreshAll==='function')await window.refreshAll();if(typeof window.loadSecurityCenterV10700==='function')await window.loadSecurityCenterV10700(true);
    }catch(e){console.error(BUILD,e);toast(S(e.message||e),'err');}finally{btn&&(btn.disabled=false);}
  };
...
/* V10820 compatibility: supports V10818/V10819 session transport and legacy V10700 guards. */
