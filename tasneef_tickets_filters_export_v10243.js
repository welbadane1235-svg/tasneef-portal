/* Tasneef V10243 - Tickets filters + no receipt + filtered export
...
(function(){
  'use strict';
  if(window.__tasneefTicketsCleanV10243) return;
  window.__tasneefTicketsCleanV10243 = true;

  const VERSION='V10243';
  ...
  function ensureCleanFilters(){
    ...
    if(!wrap){
      ...
      wrap.innerHTML = `
        ...
        <select id="ticketCleanClaimed"><option value="">كل المستلمين</option><option value="__unclaimed__">بدون استلام</option></select>
        ...
        <button type="button" class="light" onclick="exportTicketsFilteredCSVV10243()">تصدير CSV</button>
        <button type="button" onclick="ticketsDownloadFilteredPdfV10243()">تنزيل PDF</button>
  ...
  function populateFilterOptions(){
    ...
    setSelectOptions('ticketCleanClaimed','كل المستلمين', ['بدون استلام'].concat(rows.map(ticketRecipient).concat(domCardValues('المستلم'))));
    const claimedSel=$('ticketCleanClaimed'); if(claimedSel){ Array.from(claimedSel.options).forEach(o=>{ if(o.textContent==='بدون استلام') o.value='__unclaimed__'; }); }
  ...
  function match(t,f){
    ...
    if(f.claimed === '__unclaimed__'){ if(K(ticketRecipientRaw(t)) && K(ticketRecipientRaw(t)) !== '-') return false; }
    else if(f.claimed && K(ticketRecipient(t)) !== f.claimed) return false;
  ...
  function filteredTicketsV10243(){
    ensureCleanFilters();
    const f=filters();
    let rows=tickets().filter(t=>match(t,f));
    rows.sort((a,b)=> f.sort==='oldest' ? dateMs(a)-dateMs(b) : dateMs(b)-dateMs(a));
  ...
  window.getFilteredTicketsV10243 = filteredTicketsV10243;
  function csvCell(v){ return '"'+S(v).replace(/"/g,'""')+'"'; }
  function exportTicketsFilteredCSVV10243(){
    const rows=filteredTicketsV10243();
    const header=['رقم التكت','التاريخ','المشروع','المشرف','نوع المشكلة','الأولوية','الحالة','المستلم','المغلق','الوصف','الحل'];
    const lines=[header.map(csvCell).join(',')].concat(rows.map(t=>[
      ticketNo(t), fmtDate(t), ticketProject(t), ticketSupervisor(t), t.title||'', priorityLabel(t.priority), statusLabel(t.status), ticketRecipient(t), ticketClosedBy(t), t.description||'', t.closure_note||''
    ].map(csvCell).join(',')));
    const blob=new Blob(['\ufeff'+lines.join('\n')],{type:'text/csv;charset=utf-8'});
    const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='tickets-filtered-'+new Date().toISOString().slice(0,10)+'.csv'; document.body.appendChild(a); a.click(); setTimeout(()=>{URL.revokeObjectURL(a.href); a.remove();},500);
  }
  window.exportTicketsFilteredCSVV10243 = exportTicketsFilteredCSVV10243;
  function ticketsDownloadFilteredPdfV10243(){
    const rows=filteredTicketsV10243();
    const f=filters();
    const meta=[
      ['المشروع',$('ticketCleanProject')?.selectedOptions?.[0]?.textContent||'كل المشاريع'],
      ['المشرف',$('ticketCleanSupervisor')?.selectedOptions?.[0]?.textContent||'كل المشرفين'],
      ['نوع المشكلة',$('ticketCleanTitle')?.selectedOptions?.[0]?.textContent||'كل الأنواع'],
      ['الحالة',$('ticketCleanStatus')?.selectedOptions?.[0]?.textContent||'كل الحالات'],
      ['المستلم',$('ticketCleanClaimed')?.selectedOptions?.[0]?.textContent||'كل المستلمين'],
      ['المغلق',$('ticketCleanClosedBy')?.selectedOptions?.[0]?.textContent||'كل المغلقين'],
      ['الترتيب',$('ticketCleanSort')?.selectedOptions?.[0]?.textContent||'الأحدث أولاً'],
      ['عدد التكتات',rows.length]
    ];
    const esc=E;
    const trs=rows.map((t,i)=>`<tr><td>${i+1}</td><td>${esc(ticketNo(t))}</td><td>${esc(fmtDate(t))}</td><td>${esc(ticketProject(t))}</td><td>${esc(ticketSupervisor(t))}</td><td>${esc(t.title||'-')}</td><td>${esc(statusLabel(t.status))}</td><td>${esc(ticketRecipient(t))}</td><td>${esc(ticketClosedBy(t))}</td><td>${esc(t.description||'')}</td></tr>`).join('') || '<tr><td colspan="10">لا توجد تكتات مطابقة للفلاتر</td></tr>';
    const html=`<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><title>تقرير التكتات حسب الفلاتر</title><style>@page{size:A4 landscape;margin:10mm}body{font-family:Tahoma,Arial,sans-serif;color:#063b31}h1{margin:0;color:#075443}.top{border-bottom:3px solid #075443;padding-bottom:10px;margin-bottom:12px}.meta{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin:10px 0}.box{border:1px solid #d7e6e1;border-radius:10px;padding:8px;background:#f7fbfa}.box b{display:block;color:#075443}table{width:100%;border-collapse:collapse;font-size:11px}th{background:#075443;color:white;padding:7px}td{border:1px solid #dfeae6;padding:6px;vertical-align:top}tr:nth-child(even) td{background:#f8fbfa}.footer{margin-top:12px;text-align:center;color:#657}</style></head><body><div class="top"><h1>تقرير التكتات حسب الفلاتر</h1><div>شركة تصنيف لإدارة المرافق — ${esc(new Date().toLocaleString('ar-SA'))}</div></div><div class="meta">${meta.map(x=>`<div class="box"><b>${esc(x[0])}</b><span>${esc(x[1])}</span></div>`).join('')}</div><table><thead><tr><th>#</th><th>رقم التكت</th><th>التاريخ</th><th>المشروع</th><th>المشرف</th><th>نوع المشكلة</th><th>الحالة</th><th>المستلم</th><th>المغلق</th><th>الوصف</th></tr></thead><tbody>${trs}</tbody></table><div class="footer">التقرير يعرض نفس نتائج الفلاتر المحددة في صفحة الإدارة</div><script>window.onload=function(){setTimeout(function(){window.print()},400)}</script></body></html>`;
    const w=window.open('','_blank'); if(!w) return alert('اسمح بالنوافذ المنبثقة لتنزيل PDF'); w.document.write(html); w.document.close();
  }
  window.ticketsDownloadFilteredPdfV10243 = ticketsDownloadFilteredPdfV10243;
  // اجعل أزرار PDF العامة في قسم التكتات تستخدم الفلاتر الحالية.
  window.ticketsDownloadPdfV217 = ticketsDownloadFilteredPdfV10243;
  window.ticketsDownloadPdfV211 = ticketsDownloadFilteredPdfV10243;
  window.ticketsDownloadPdfV210 = ticketsDownloadFilteredPdfV10243;
  window.renderTickets = renderTicketsClean;
  window.renderTicketsCleanV10243 = renderTicketsClean;
