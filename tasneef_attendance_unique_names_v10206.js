/* ===== TASNEEF V10206 ATTENDANCE UNIQUE NAME CANONICAL WORKER_ID =====
   حل جذري للحضور والغياب:
   - كل اسم عامل يظهر مرة واحدة فقط.
   - يوجد worker_id ثابت داخلي لكل اسم بدون عرضه في الكشف.
   - يتم دمج سجلات نفس الاسم من كل worker_id للعرض فقط.
   - كشف الشهر والطباعة والتصدير CSV/Excel من نفس مصدر بيانات الشاشة.
   - كشف الشهر مجمع حسب المشرف الحالي، وتحت كل مشرف عماله.
*/
(function(){
  'use strict';
  if(window.__tasneefAttendanceUniqueNameV10206) return;
  window.__tasneefAttendanceUniqueNameV10206 = true;
  ...
  async function render(force){
    ...
    message('جاهز - كل اسم عامل يظهر مرة واحدة فقط، والـ ID ثابت داخليًا بدون تكرار الاسم.');
  ...
  function renderWorkersBox(workers,date,recMap){
    ...
    box.innerHTML=groups.map(g=>`
      ...
      ${g.workers.map(w=>{
        ...
        return `<div class="att-rb-worker" data-wid="${esc(workerInternalId(w))}" title="ID داخلي ثابت: ${esc(workerInternalId(w))} | IDs مدمجة: ${esc(workerIds(w).join(', '))}">
  ...
  function renderToday(workers,date,recMap){
    ...
    body.innerHTML=groups.map(g=>`
      ...
      ${g.workers.map(w=>{
        const r=getWorkerRecord(w,date,recMap);
        return `<tr title="ID داخلي ثابت: ${esc(workerInternalId(w))} | IDs مدمجة: ${esc(workerIds(w).join(', '))}">
  ...
  function renderMatrix(workers,month,recMap){
    ...
    body.innerHTML=groups.map(g=>{
      const rows=g.workers.map(w=>{
        ...
        return `<tr title="ID داخلي ثابت: ${esc(workerInternalId(w))} | IDs مدمجة: ${esc(workerIds(w).join(', '))}"><td><b>${esc(workerLabel(w))}</b></td>${cells}<td>${p}</td><td>${a}</td></tr>`;
      }).join('');
  ...
  async function save(){
    ...
    message('تم حفظ التحضير، كل اسم محفوظ على ID داخلي ثابت بدون تكرار الاسم.');
    await render(true);
  }
  ...
  async function exportCsv(){
    ...
    const days=daysOfMonth(month);
    const rows=[];
    rows.push(['المشرف','العامل','ID داخلي ثابت',...days,'حضور','غياب']);
    groupWorkers(workers).forEach(g=>{
      ...
      g.workers.forEach(w=>{
        ...
        rows.push([g.name, workerLabel(w), workerInternalId(w), ...cells, p, a]);
      });
    });
    const csv=rows.map(r=>r.map(x=>'"'+S(x).replace(/"/g,'""')+'"').join(',')).join('\n');
    const blob=new Blob(['\ufeff'+csv],{type:'text/csv;charset=utf-8'});
    const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='attendance_by_supervisor_unique_names_'+month+'.csv'; a.click(); setTimeout(()=>URL.revokeObjectURL(a.href),1000);
  }
