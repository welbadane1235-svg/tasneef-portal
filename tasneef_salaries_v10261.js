(function(){
  ...
  function orderSalaryRowsForView(rows){
    ...
    const byOrder=(a,b)=>num(a.row_order||9999)-num(b.row_order||9999) || String(displayNameForRow(a)).localeCompare(String(displayNameForRow(b)),'ar');
    rows.sort(byOrder);
    ...
    supervisors.forEach(s=>{
      ...
      const sName=normName(s.employee_name||s.residency_name||s.supervisor_name);
      workers.filter(r=>{
        const wl=normName(r.work_location||r.supervisor_name);
        return sName && wl===sName;
      }).sort(byOrder).forEach(r=>{ used.add(r); out.push(r); });
    });
    workers.filter(r=>!used.has(r)).sort(byOrder).forEach(r=>out.push(r));
    guards.sort(byOrder).forEach(r=>out.push(r));
    techs.sort(byOrder).forEach(r=>out.push(r));
    other.sort(byOrder).forEach(r=>out.push(r));
    return out;
  }
  ...
  function rowInput(r,k,cls='sal-input money'){ return `<input class="${cls}" data-key="${k}" data-type="${r.entity_type}" data-id="${r.entity_id}" value="${esc(r[k]??0)}" onchange="tasneefSalariesV10261.update('${r.entity_type}','${r.entity_id}','${k}',this.value)">`; }
  function dateInput(r,k){ return `<input type="date" class="sal-input" style="width:135px" data-key="${k}" data-type="${r.entity_type}" data-id="${r.entity_id}" value="${esc(r[k]||'')}" onchange="tasneefSalariesV10261.update('${r.entity_type}','${r.entity_id}','${k}',this.value)">`; }
  function textInput(r,k,w=130){ return `<input class="sal-input" style="width:${w}px" data-key="${k}" data-type="${r.entity_type}" data-id="${r.entity_id}" value="${esc(r[k]||'')}" onchange="tasneefSalariesV10261.update('${r.entity_type}','${r.entity_id}','${k}',this.value)">`; }
  function renderSalaryRowHtml(r,i,month){
    return `<tr data-sal-row="${r.entity_type}_${r.entity_id}">
      ...
      <td><input class="sal-input" data-key="notes" data-type="${r.entity_type}" data-id="${r.entity_id}" value="${esc(r.notes||'')}" onchange="tasneefSalariesV10261.update('${r.entity_type}','${r.entity_id}','notes',this.value)"></td>
    </tr>`;
  }
  function renderRowsGroupedHtml(rows, month){
    let n=0, html='';
    rows=sanitizeSalaryRows(rows).sort((a,b)=>num(a.row_order||9999)-num(b.row_order||9999));
    const supervisors=rows.filter(r=>jobCategory(r.job_title)==='supervisors');
    const workers=rows.filter(r=>jobCategory(r.job_title)==='workers');
    const guards=rows.filter(r=>jobCategory(r.job_title)==='guards');
    const techs=rows.filter(r=>jobCategory(r.job_title)==='technicians');
    ...
    supervisors.forEach(s=>{
      ...
      const sName=normName(title);
      workers.filter(r=>normName(r.work_location||r.supervisor_name)===sName).sort((a,b)=>num(a.row_order||9999)-num(b.row_order||9999)).forEach(r=>{ attached.add(r); html+=renderSalaryRowHtml(r, ++n, month); });
    });
    const un=workers.filter(r=>!attached.has(r));
    if(un.length){ html += `<tr class="salary-group-row"><td colspan="23">عمال إضافيون</td></tr>`; un.forEach(r=>{ html+=renderSalaryRowHtml(r, ++n, month); }); }
    ...
    if(other.length){ html += `<tr class="salary-group-row"><td colspan="23">أخرى</td></tr>`; other.forEach(r=>{ html+=renderSalaryRowHtml(r, ++n, month); }); }
    return html;
  }
  ...
  function inject(){
    ...
    const main=document.querySelector('main.content'); const sec=document.createElement('section'); sec.id='salaries'; sec.className='page hidden'; sec.innerHTML=`
      ...
      <div class="card"><div class="table-head"><h2>الرواتب</h2><span class="badge green">${VERSION}</span></div><div class="salary-note">بيانات العمال والمشرفين ثابتة من ملف Excel المعتمد. شهر 2026-06 مطابق للملف، والأشهر القادمة تعتمد نفس البيانات مع احتساب الإضافات الجديدة تلقائيًا من الحضور والغياب.</div><div id="salaryMsg" class="msg hidden"></div>
      <div class="filters"><div><label>الشهر</label><input type="month" id="salaryMonth" value="${today().slice(0,7)}" onchange="tasneefSalariesV10261.load()"></div><div><label>نوع الكشف</label><select id="salaryType" onchange="tasneefSalariesV10261.buildRows()"><option value="all">الكل</option><option value="supervisors">رواتب المشرفين</option><option value="workers">رواتب العمال</option><option value="technicians">رواتب الفنيين</option><option value="guards">رواتب الحراس</option></select></div><div><label>المشرف</label><select id="salarySupervisor" onchange="tasneefSalariesV10261.buildRows()"><option value="">كل المشرفين</option></select></div><div><label>المشروع</label><select id="salaryProject" onchange="tasneefSalariesV10261.buildRows()"><option value="">كل المشاريع</option></select></div><div><label>بحث</label><input id="salarySearch" oninput="tasneefSalariesV10261.buildRows()" placeholder="اسم/إقامة/TS"></div></div>
      <div class="salary-actions"><button onclick="tasneefSalariesV10261.load()">تحديث الرواتب</button><button class="light" onclick="tasneefSalariesV10261.save(false)">حفظ التعديلات</button><button class="light" onclick="tasneefSalariesV10261.save(true)">اعتماد الرواتب</button><button class="light" onclick="tasneefSalariesV10261.print()">طباعة</button><button class="light" onclick="tasneefSalariesV10261.exportExcel()">تصدير Excel</button></div><div id="salaryKpis" class="kpis small"></div>
  ...
  window.tasneefSalariesV10261={inject,load:loadSalary,buildRows,update,save:saveSalary,print:printSalary,exportExcel:exportSalaryExcel};
  window.tasneefSalariesV10261=window.tasneefSalariesV10261;
  window.tasneefSalariesV10261=window.tasneefSalariesV10261;
