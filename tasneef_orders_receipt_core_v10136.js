/* Tasneef v10136 - Orders receipt from original order field
   Scope: ORDERS ONLY
...
(function(){
  'use strict';
  if(window.__tasneefOrdersReceiptCoreV10136) return;
  window.__tasneefOrdersReceiptCoreV10136 = true;

  const VERSION='orders_receipt_v10136';
  ...
  window.openCurrentOrderReceiptV10136=async function(){openReceipt(await loadCurrentReceipt());};
  window.openOrderReceiptV10136=function(orderNo){fetchOrder(orderNo).then(rec=>openReceipt(receiptFromData(rec?.data||{}))).catch(()=>alert('تعذر فتح الإيصال الآن'));};

  function syncReceiptUi(receipt){
    const inp=getField(RECEIPT_FIELD); const btn=$('orderReceiptBtnV10136'); const hint=$('orderReceiptHintV10136');
    const has=!!((pendingReceipt&&pendingReceipt.url)||(receipt&&receipt.url));
    if(btn){btn.disabled=!has;btn.textContent='الإيصال';btn.title=has?'عرض الإيصال':'لا يوجد إيصال مرفق';}
  ...
  function enhanceOriginalReceiptField(){
    ...
    if(!box.querySelector('#orderReceiptBtnV10136')){
      const row=document.createElement('div'); row.className='order-receipt-inline-v10136';
      inp.insertAdjacentElement('beforebegin',row); row.appendChild(inp);
      const btn=document.createElement('button'); btn.type='button'; btn.className='light'; btn.id='orderReceiptBtnV10136'; btn.textContent='الإيصال'; btn.onclick=ev=>{ev.preventDefault(); window.openCurrentOrderReceiptV10136();};
      row.appendChild(btn);
      const hint=document.createElement('small'); hint.id='orderReceiptHintV10136'; hint.textContent='اختياري'; box.appendChild(hint);
    }
    if(!inp.__receiptV10136){
      inp.__receiptV10136=1;
  ...
  function patchSaveEditClear(){
    if(typeof window.saveOrderV233==='function' && !window.saveOrderV233.__receiptV10136){
      ...
      window.saveOrderV233=async function(){
        ...
        try{
          ...
          const after=currentOrderNo()||before;
          try{ await attachPendingReceiptOnly(after); }catch(e){ console.warn(VERSION,'receipt attach after old save failed',e); }
          setTimeout(()=>{try{ if(typeof window.renderOrdersV233==='function') window.renderOrdersV233(); }catch(_){ }},250);
          return r;
        }catch(e){
          if(isOldSourceLockError(e)){
            console.warn(VERSION,'old orders source locked, using direct save fallback',e);
            return directSaveOrderFromForm();
          }
          throw e;
        }
      };
      window.saveOrderV233.__receiptV10136=true;
    }
    if(typeof window.editOrderV233==='function' && !window.editOrderV233.__receiptV10136){
      const oldEdit=window.editOrderV233;
      window.editOrderV233=function(){ const r=oldEdit.apply(this,arguments); pendingReceipt=null; setTimeout(enhanceOriginalReceiptField,250); return r; };
      window.editOrderV233.__receiptV10136=true;
    }
    if(typeof window.clearOrderFormV233==='function' && !window.clearOrderFormV233.__receiptV10136){
      ...
      window.clearOrderFormV233.__receiptV10136=true;
    }
  }
  ...
  async function injectReceiptButtonsInCards(){
    ...
    let records=[]; try{records=await fetchRecentOrders(5000);}catch(_){return;}
    const receiptMap=new Map(); records.forEach(rec=>{const d=rec.data||{};const no=normalizeNo(rec.order_no||d['رقم الطلب']); if(no&&receiptFromData(d)?.url) receiptMap.set(no,true);});
    [...box.children].filter(n=>n&&n.nodeType===1).forEach(node=>{
      if(node.querySelector('[data-v10136-receipt-btn]')) return;
      let no=orderNoFromElement(node); if(!no){for(const key of receiptMap.keys()){if(S(node.textContent).replace(/\s+/g,'').toUpperCase().includes(key)){no=key;break;}}}
      if(!no) return;
      const actions=node.querySelector('.row-actions,.actions,.order-actions,.card-actions')||node;
      const btn=document.createElement('button'); btn.type='button'; btn.className='light'; btn.dataset.v10136ReceiptBtn='1'; btn.textContent='الإيصال'; btn.style.marginInlineStart='6px'; btn.disabled=!receiptMap.has(no); btn.title=receiptMap.has(no)?'عرض الإيصال':'لا يوجد إيصال مرفق'; btn.onclick=ev=>{ev.preventDefault();ev.stopPropagation();window.openOrderReceiptV10136(no);};
      actions.appendChild(btn);
    });
  }
  function patchRender(){ if(typeof window.renderOrdersV233==='function'&&!window.renderOrdersV233.__receiptV10136){const old=window.renderOrdersV233;window.renderOrdersV233=function(){const r=old.apply(this,arguments);setTimeout(injectReceiptButtonsInCards,450);return r;};window.renderOrdersV233.__receiptV10136=true;} }
  function injectStyle(){ if($('ordersReceiptStyleV10136')) return; const st=document.createElement('style'); st.id='ordersReceiptStyleV10136'; st.textContent='.order-receipt-inline-v10136{display:grid;grid-template-columns:1fr auto;gap:8px;align-items:center}.order-receipt-inline-v10136 button:disabled,[data-v10136-receipt-btn]:disabled{opacity:.55;cursor:not-allowed;background:#eef6f3!important;color:#60706a!important}#orderReceiptHintV10136{display:block;color:#60706a;margin-top:4px}@media(max-width:760px){.order-receipt-inline-v10136{grid-template-columns:1fr}}'; document.head.appendChild(st); }
  function boot(){ try{injectStyle(); enhanceOriginalReceiptField(); patchSaveEditClear(); patchRender(); setTimeout(injectReceiptButtonsInCards,1000);}catch(e){console.warn(VERSION,e);} }
