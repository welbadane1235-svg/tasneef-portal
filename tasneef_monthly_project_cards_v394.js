/* V394 - إصلاح جذري: قراءة شهر 6 من ملف الأوقات الأصلي، عدم تصفير الوقت، كل مشروع دوام كامل كرت مستقل */
(function(){
  'use strict';
  if(window.__tasneefMonthlyCardsV394) return;
  window.__tasneefMonthlyCardsV394=true;
  const VERSION='394';
  ...
  async function loadJuneRows(){
    ...
    try{
      const res=await fetch('monthly_june_codes_v389.json?v=394',{cache:'no-store'});
      if(!res.ok) return [];
      const data=await res.json();
      return normalizeRows((data||[]).map(x=>({
        ...
        source:'ملف الأوقات الشهرية لشهر 6'
      })));
    }catch(e){ console.warn('V394 June JSON load failed',e); return []; }
  }
  ...
  function ensureCss(){
    if($('mt394Css')) return;
    const st=document.createElement('style'); st.id='mt394Css';
    st.textContent=`.mt394-note{background:#fff8e8;border:1px solid #ead28d;color:#6a4d00;border-radius:14px;padding:10px 12px;margin:8px 0 12px;font-weight:800;line-height:1.8}.mt394-super{border:1px solid #cfe2da;border-radius:18px;background:#fff;overflow:hidden;margin:12px 0}.mt394-super>h3{margin:0;background:#0a4539;color:#fff;padding:12px 14px}.mt394-grid{display:grid!important;grid-template-columns:repeat(3,minmax(280px,1fr))!important;gap:12px!important}.mt394-card{border:1px solid #dbe8e2!important;border-radius:18px!important;background:#fbfdfc!important;padding:13px!important;break-inside:avoid!important;box-shadow:0 8px 22px rgba(0,0,0,.04)!important}.mt394-card.full{border:2px solid #123b70!important;background:#fbfdff!important}.mt394-card h4{margin:0 0 10px!important;text-align:right!important;color:#0a4539!important;font-size:18px!important}.mt394-card.full h4{color:#123b70!important}.mt394-line{display:flex;justify-content:space-between;gap:10px;border-bottom:1px dashed #d7e5df;padding:7px 0}.mt394-line span{color:#60746c}.mt394-line b{color:#0a4539;text-align:left}.mt394-workers{display:flex;flex-wrap:wrap;gap:6px;margin-top:9px}.mt394-workers span{background:#eef6f3;border:1px solid #dbe8e2;border-radius:999px;padding:5px 9px;font-weight:800;color:#0a4539}.mt394-bar{height:10px;background:#e7f0ec;border-radius:999px;overflow:hidden;margin-top:8px}.mt394-bar i{display:block;height:100%;background:#0a4539}.mt394-card.full .mt394-bar i{background:#123b70}@media(max-width:1200px){.mt394-grid{grid-template-columns:repeat(2,minmax(260px,1fr))!important}}@media(max-width:760px){.mt394-grid{grid-template-columns:1fr!important}}`;
    document.head.appendChild(st);
  }
  function card(r){
    const full=isFull(r), workers=(r.workers||[]).length?r.workers:['لا يوجد عمال مربوطين'];
    return `<article class="mt394-card ${full?'full':''}"><h4>${esc(r.projectName)}</h4><div class="mt394-line"><span>المشرف</span><b>${esc(r.supervisorName||'-')}</b></div><div class="mt394-line"><span>نوع التشغيل</span><b>${esc(r.projectType)}</b></div><div class="mt394-line"><span>الوقت المستغرق</span><b>${esc(arMins(r.totalMinutes))}</b></div><div class="mt394-line"><span>إجمالي الدقائق</span><b>${Math.round(r.totalMinutes).toLocaleString('en-US')}</b></div>${full?`<div class="mt394-line"><span>الوقت المطلوب</span><b>${r.requiredMinutes?esc(arMins(r.requiredMinutes)):'-'}</b></div>`:''}<div class="mt394-line"><span>نسبة المشروع</span><b>${pct(r.percentage)}</b></div><div class="mt394-bar"><i style="width:${Math.min(100,Math.max(0,r.percentage)).toFixed(0)}%"></i></div><div class="mt394-workers">${workers.map(w=>`<span>${esc(w)}</span>`).join('')}</div></article>`;
  }
  async function render(){
    if(window.__mt394Rendering) return window.tasneefMonthlyV394Rows||[];
    window.__mt394Rendering=true;
    ensureCss();
    const rows=await getRows();
    window.__mt394Rendering=false;
    if(!rows.length) return rows;
    window.tasneefMonthlyV394Rows=rows; window.tasneefMonthlyV393Rows=rows;
    ...
    if(visitGrid){
      ...
      visitGrid.className='';
      visitGrid.innerHTML=`<div class="mt394-note">تصحيح v394: تم إلغاء الصفر وقراءة شهر 6 من ملف الأوقات الأصلي. مشاريع الزيارة حسب المشرف، وكل مشروع مربع مستقل.</div>`+[...bySup.entries()].map(([sup,list])=>`<section class="mt394-super"><h3>${esc(sup)}</h3><div class="mt394-grid">${list.map(card).join('')}</div></section>`).join('');
    }
    ...
    if(workersCard){
      ...
      const grid=$('mt52WorkersGrid'); if(grid){grid.className='mt394-grid'; grid.innerHTML=full.length?full.map(card).join(''):'<div class="mt394-note">لا توجد مشاريع دوام كامل لهذا الشهر.</div>';}
    }
    ...
    const msg=$('mt52Message'); if(msg){msg.classList.remove('hidden'); msg.textContent='تم التصحيح v394: الأوقات لم تعد صفر، وكل مشروع دوام كامل يظهر مربع مستقل.';}
    patchTable(rows);
    return rows;
  }
  ...
  async function printReport(ev){
    ...
    const rows=(window.tasneefMonthlyV394Rows&&window.tasneefMonthlyV394Rows.length)?window.tasneefMonthlyV394Rows:await render();
    const daily=rows.filter(r=>!isFull(r)), full=rows.filter(r=>isFull(r));
    const bySup=new Map(); daily.forEach(r=>{const k=r.supervisorName||'غير محدد'; if(!bySup.has(k)) bySup.set(k,[]); bySup.get(k).push(r);});
