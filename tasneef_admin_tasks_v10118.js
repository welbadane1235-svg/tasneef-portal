/* Tasneef v10118 - Administrative Tasks Users-Only Recipients Workflow
   Scope: new section only. Does not touch finance, inventory, orders, tickets, contracts, attendance, monthly, or permissions.
*/
(function(){
  'use strict';
  if(window.__tasneefAdminTasksV10118) return;
  window.__tasneefAdminTasksV10118 = true;

  const VERSION='v10118-admin-tasks-advanced';
  const SUPABASE_URL='https://zmjdqiswytxlbfgnfjfv.supabase.co';
  const SUPABASE_ANON_KEY='sb_publishable_ADsAC5MtBCusDgX62c8NaQ_LyyuTPeb';
  const TABLE='admin_tasks';
  const LOCAL_KEY='tasneef_admin_tasks_v10118_local';
  const DISMISS_PREFIX='tasneef_admin_task_dismiss_v10118_';
  ...
  function currentUser(){
    ...
    ['currentUser','loggedUser','tasneefUser','user','authUser'].forEach(k=>{ try{ consider(window[k]); }catch(_){} });
    return best || {};
  }
  function currentKeys(u=currentUser()){
    const vals=[u.id,u.uid,u.user_id,u.username,u.user_name,u.email,u.full_name,u.fullName,u.name,u.display_name].map(S).filter(Boolean);
    const out=[]; const seen=new Set();
    vals.forEach(v=>{ const low=v.toLowerCase(); if(!seen.has(low)){ seen.add(low); out.push(low); } });
    return out;
  }
  ...
  function readLocal(){
    ...
    try{ rows.push(...(JSON.parse(localStorage.getItem('tasneef_admin_tasks_v10117_local')||'[]')||[])); }catch(_){}
    const seen=new Set();
  ...
  async function loadTasks(){
    try{
      ...
      console.warn('v10118 load online failed, using local', e);
  ...
  async function saveTask(payload){
    ...
    try{
      ...
      console.warn('v10118 save online failed, using local', e);
  ...
  function injectCss(){
    if($('tasneefAdminTasksStyleV10118')) return;
    const st=document.createElement('style'); st.id='tasneefAdminTasksStyleV10118';
    st.textContent=`
      .at10118-head{display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap}
      .at10118-grid{display:grid;grid-template-columns:repeat(4,minmax(160px,1fr));gap:10px}.at10118-grid .wide{grid-column:span 2}.at10118-grid .full{grid-column:1/-1}
      .at10118-muted{color:#60706a;font-size:13px;line-height:1.7}.at10118-actions{display:flex;gap:8px;flex-wrap:wrap;align-items:center}.at10118-chip{display:inline-flex;align-items:center;gap:5px;border-radius:999px;padding:5px 9px;background:#eef6f3;color:#0A4033;font-weight:900;font-size:12px}.at10118-chip.red{background:#fde8e8;color:#9d2222}.at10118-chip.amber{background:#fff5da;color:#8a6700}.at10118-chip.green{background:#e8f4ee;color:#137a4b}.at10118-btn-mini{padding:7px 9px!important;border-radius:9px!important;font-size:12px!important}.at10118-form{display:none}.at10118-form.show{display:block}.at10118-empty{padding:18px;text-align:center;color:#60706a}.at10118-task-title{font-weight:900;color:#0A4033}.at10118-details{max-width:420px;white-space:normal;line-height:1.7;color:#263b35}.at10118-badge-count{display:inline-grid;place-items:center;min-width:20px;height:20px;border-radius:999px;background:#b83232;color:#fff;font-size:11px;margin-inline-start:6px;padding:0 5px}
      .at10118-modal{position:fixed;inset:0;background:rgba(2,15,11,.48);z-index:999999;display:none;align-items:center;justify-content:center;padding:18px}.at10118-modal.show{display:flex}.at10118-box{width:min(560px,96vw);background:#fff;border-radius:24px;border:1px solid #dce6e2;box-shadow:0 24px 70px rgba(0,0,0,.25);overflow:hidden}.at10118-box-head{background:linear-gradient(135deg,#0A4033,#0f5a47);color:#fff;padding:18px 20px}.at10118-box-head h3{margin:0 0 6px}.at10118-box-body{padding:18px 20px}.at10118-reminder-card{background:#f8fbfa;border:1px solid #dce6e2;border-radius:16px;padding:13px;line-height:1.8}.at10118-box-actions{display:flex;gap:8px;justify-content:flex-end;flex-wrap:wrap;padding:0 20px 20px}.at10118-box-actions .light{background:#eef6f3;color:#0A4033;border:1px solid #dce6e2}
      .at10118-tabs{display:flex;gap:8px;flex-wrap:wrap;margin:12px 0}.at10118-tab{border:1px solid #dce6e2;background:#fff;color:#0A4033;border-radius:999px;padding:9px 14px;font-weight:900;cursor:pointer}.at10118-tab.active{background:#0A4033;color:#fff}.at10118-tab .count{display:inline-grid;place-items:center;min-width:20px;height:20px;background:#f0b429;color:#0A4033;border-radius:999px;margin-inline-start:6px;padding:0 5px;font-size:11px}
      @media(max-width:900px){.at10118-grid{grid-template-columns:1fr}.at10118-grid .wide{grid-column:auto}.at10118-details{max-width:260px}.at10118-head{align-items:flex-start}}
    `;
  ...
  function ensureNavAndSection(){
    ...
    if(!$('adminTasks')){
      ...
      sec.innerHTML=`
        <div class="card section-head at10118-head">
          ...
          <div class="at10118-actions"><button onclick="window.tasneefAdminTasksV10118.openNew()" id="at10118NewBtn">إضافة مهمة</button><button class="light" onclick="window.tasneefAdminTasksV10118.refresh()">تحديث</button></div>
        </div>
        <div class="card at10118-form" id="at10118FormCard">
          <h2>إضافة مهمة إدارية</h2>
          <div class="at10118-muted">المرسل يتم تسجيله تلقائيًا باسم المستخدم الحالي. المستلم يتم اختياره من أسماء المستخدمين في إدارة المستخدمين فقط، وتظهر المهمة في حساب المستلم وللمرسل لمتابعة الإغلاق والاعتماد.</div>
          <div class="at10118-grid">
            <div class="wide"><label>عنوان المهمة</label><input id="at10118Title" placeholder="مثال: مراجعة تقرير التشغيل اليومي"></div>
            <div><label>نوع الجدولة</label><select id="at10118ScheduleType" onchange="window.tasneefAdminTasksV10118.toggleSchedule()"><option>مهمة فورية</option><option selected>مجدولة</option></select></div>
            <div><label>الأولوية</label><select id="at10118Priority"><option>عادي</option><option>مهم</option><option>عاجل</option></select></div>
            <div><label>تاريخ ووقت الجدولة</label><input id="at10118Due" type="datetime-local"></div>
            <div class="wide"><label>المستلم من إدارة المستخدمين</label><select id="at10118Recipient"></select><div class="at10118-muted">تظهر هنا أسماء المستخدمين المسجلين في قسم إدارة المستخدمين فقط.</div></div>
            <div class="wide"><label>نوع المهمة</label><select id="at10118Type"><option>مهمة إدارية</option><option>متابعة تشغيل</option><option>متابعة مالية</option><option>متابعة مشروع</option><option>طلب تقرير</option><option>ملاحظة عاجلة</option></select></div>
            <div class="full"><label>تفاصيل المهمة</label><textarea id="at10118Details" placeholder="اكتب تفاصيل المهمة المطلوبة بوضوح..."></textarea></div>
          </div>
          <div class="actions"><button onclick="window.tasneefAdminTasksV10118.create()">حفظ المهمة</button><button class="light" onclick="window.tasneefAdminTasksV10118.closeNew()">إلغاء</button></div>
        </div>
        <div class="kpis small" id="at10118Kpis"></div>
        <div class="card">
          <div class="at10118-tabs" id="at10118Tabs">
            <button class="at10118-tab active" data-tab="mine" onclick="window.tasneefAdminTasksV10118.setTab('mine')">مهامي <span class="count" id="at10118CountMine">0</span></button>
            <button class="at10118-tab" data-tab="open" onclick="window.tasneefAdminTasksV10118.setTab('open')">مهام مفتوحة <span class="count" id="at10118CountOpen">0</span></button>
            <button class="at10118-tab" data-tab="closed" onclick="window.tasneefAdminTasksV10118.setTab('closed')">مهام مغلقة <span class="count" id="at10118CountClosed">0</span></button>
            <button class="at10118-tab" data-tab="approved" onclick="window.tasneefAdminTasksV10118.setTab('approved')">تم الاعتماد <span class="count" id="at10118CountApproved">0</span></button>
          </div>
          <div class="filters">
            <input id="at10118Search" placeholder="بحث في المهام" oninput="window.tasneefAdminTasksV10118.render()">
            <select id="at10118PriorityFilter" onchange="window.tasneefAdminTasksV10118.render()"><option value="">كل الأولويات</option><option>عادي</option><option>مهم</option><option>عاجل</option></select>
            <select id="at10118TypeFilter" onchange="window.tasneefAdminTasksV10118.render()"><option value="">كل أنواع المهام</option><option>مهمة إدارية</option><option>متابعة تشغيل</option><option>متابعة مالية</option><option>متابعة مشروع</option><option>طلب تقرير</option><option>ملاحظة عاجلة</option></select>
            <select id="at10118ScheduleFilter" onchange="window.tasneefAdminTasksV10118.render()"><option value="">فورية ومجدولة</option><option>مهمة فورية</option><option>مجدولة</option></select>
            <input id="at10118FromDate" type="date" title="من تاريخ" onchange="window.tasneefAdminTasksV10118.render()">
            <input id="at10118ToDate" type="date" title="إلى تاريخ" onchange="window.tasneefAdminTasksV10118.render()">
            <button class="light" onclick="window.tasneefAdminTasksV10118.print()">طباعة المعروض</button>
          </div>
          <div class="table-wrap"><table><thead><tr><th>الحالة</th><th>المهمة</th><th>من</th><th>إلى</th><th>الجدولة</th><th>أنشئت</th><th>أغلقت</th><th>اعتمدت</th><th>إجراء</th></tr></thead><tbody id="at10118Body"></tbody></table></div>
        </div>`;
      content.appendChild(sec);
    }
    if(!$('at10118Modal')){
      const modal=document.createElement('div'); modal.id='at10118Modal'; modal.className='at10118-modal';
      modal.innerHTML=`<div class="at10118-box"><div class="at10118-box-head"><h3>تذكير مهمة إدارية</h3><div id="at10118ModalSub">لديك مهمة تحتاج متابعة</div></div><div class="at10118-box-body"><div id="at10118ModalContent" class="at10118-reminder-card"></div></div><div class="at10118-box-actions"><button class="light" onclick="window.tasneefAdminTasksV10118.dismissReminder()">إغلاق التذكير</button><button onclick="window.tasneefAdminTasksV10118.openFromReminder()">فتح المهمة</button></div></div>`;
    ...
    if(!document.querySelector('button.nav[data-page="adminTasks"]')){
      ...
      btn.innerHTML='مهام إدارية <span id="at10118NavBadge" class="at10118-badge-count" style="display:none">0</span>';
    ...
    fillRecipients();
    if(!canCreate()){
      const nb=$('at10118NewBtn'); if(nb) nb.style.display='none';
      const fc=$('at10118FormCard'); if(fc) fc.classList.remove('show');
    }
  }

  async function fillRecipients(){
    const sel=$('at10118Recipient'); if(!sel) return;
    const old=sel.value;
  ...
  function parseRecipient(){
    const val=S($('at10118Recipient')&&$('at10118Recipient').value);
  ...
  function priorityChip(p){ p=S(p)||'عادي'; const cls=p==='عاجل'?'red':(p==='مهم'?'amber':'green'); return `<span class="at10118-chip ${cls}">${esc(p)}</span>`; }
  function statusChip(t){ return t.status==='approved'?'<span class="at10118-chip green">تم الاعتماد</span>':(t.status==='closed'?'<span class="at10118-chip amber">مغلقة</span>':'<span class="at10118-chip amber">مفتوحة</span>'); }
  ...
  function matchesCurrentUser(t){
    const keys=currentKeys(); const role=userRole().toLowerCase();
    const targets=[t.to_user,t.to_name].map(S).filter(Boolean).map(x=>x.toLowerCase());
    if(targets.some(target=>keys.some(k=>sameUserMatch(target,k)))) return true;
    const tr=S(t.to_role).toLowerCase();
    if(tr && (tr===role || (tr==='supervisor' && isSupervisor()) || (tr==='admin' && isManager()))) return true;
    return false;
  }
  function isSender(t){
    const keys=currentKeys();
    const targets=[t.from_user,t.from_name].map(S).filter(Boolean).map(x=>x.toLowerCase());
  ...
  function filteredRows(){
    const q=S($('at10118Search')&&$('at10118Search').value).toLowerCase();
    const pr=S($('at10118PriorityFilter')&&$('at10118PriorityFilter').value);
    const tf=S($('at10118TypeFilter')&&$('at10118TypeFilter').value);
    const sf=S($('at10118ScheduleFilter')&&$('at10118ScheduleFilter').value);
    const from=S($('at10118FromDate')&&$('at10118FromDate').value);
    const to=S($('at10118ToDate')&&$('at10118ToDate').value);
  ...
  function render(){
    ...
    const k=$('at10118Kpis'); if(k) k.innerHTML=`<div class="kpi"><small>مهامي</small><b>${mine}</b></div><div class="kpi"><small>مهام مفتوحة</small><b>${open}</b></div><div class="kpi"><small>مهام مغلقة تنتظر اعتماد</small><b>${closed}</b></div><div class="kpi"><small>المهام المتأخرة</small><b>${overdue}</b></div>`;
    const badge=$('at10118NavBadge'); if(badge){ badge.textContent=mine; badge.style.display=mine? 'inline-grid':'none'; }
    const counts={Mine:mine,Open:open,Closed:closed,Approved:approved}; Object.keys(counts).forEach(k2=>{ const el=$('at10118Count'+k2); if(el) el.textContent=counts[k2]; });
    document.querySelectorAll('.at10118-tab').forEach(b=>b.classList.toggle('active', b.dataset.tab===currentTab));
    const body=$('at10118Body'); if(!body) return;
    if(!rows.length){ body.innerHTML=`<tr><td colspan="9" class="at10118-empty">لا توجد مهام حسب الفلتر الحالي</td></tr>`; return; }
    body.innerHTML=rows.map(t=>`
      <tr data-at-id="${esc(t.id)}">
        <td>${statusChip(t)}${isOverdue(t)?' <span class="at10118-chip red">متأخرة</span>':''}</td>
        <td><div class="at10118-task-title">${esc(t.title||'بدون عنوان')}</div><div>${priorityChip(t.priority)} <span class="at10118-chip">${esc(t.task_type||'مهمة إدارية')}</span> <span class="at10118-chip">${esc(t.schedule_type||'مجدولة')}</span></div><div class="at10118-details">${esc(t.details||'')}</div></td>
        ...
        <td><div class="row-actions"><button class="light at10118-btn-mini" onclick="window.tasneefAdminTasksV10118.view('${esc(t.id)}')">عرض</button>${t.status==='open'&&matchesCurrentUser(t)?`<button class="at10118-btn-mini" onclick="window.tasneefAdminTasksV10118.closeTask('${esc(t.id)}')">إغلاق المهمة</button>`:''}${t.status==='closed'&&isSender(t)?`<button class="at10118-btn-mini" onclick="window.tasneefAdminTasksV10118.approveTask('${esc(t.id)}')">اعتماد</button>`:''}</div></td>
      </tr>`).join('');
  }
  async function refresh(){ await loadTasks(); render(); }
  function openNew(){ if(!canCreate()) return alert('إضافة المهام متاحة للإدارة فقط'); ensureNavAndSection(); $('at10118FormCard').classList.add('show'); fillRecipients(); }
  function closeNew(){ const f=$('at10118FormCard'); if(f) f.classList.remove('show'); }
  async function create(){
    if(!canCreate()) return alert('إضافة المهام متاحة للإدارة فقط');
    const title=S($('at10118Title')&&$('at10118Title').value); if(!title) return alert('اكتب عنوان المهمة');
    const rec=parseRecipient(); if(!rec.to_user && !rec.to_name) return alert('اختر المستلم من إدارة المستخدمين');
    const u=currentUser();
    const scheduleType=S($('at10118ScheduleType')&&$('at10118ScheduleType').value)||'مجدولة';
    const dueVal = scheduleType==='مهمة فورية' ? nowIso() : dtLocalToIso($('at10118Due')&&$('at10118Due').value);
    const payload=Object.assign({
      title, details:S($('at10118Details')&&$('at10118Details').value), task_type:S($('at10118Type')&&$('at10118Type').value)||'مهمة إدارية', priority:S($('at10118Priority')&&$('at10118Priority').value)||'عادي', schedule_type:scheduleType,
    ...
    ['at10118Title','at10118Details','at10118Due'].forEach(id=>{ const el=$(id); if(el) el.value=''; });
    currentTab='open';
    closeNew();
    await refresh();
  ...
  function view(id){
    ...
    ensureNavAndSection();
    $('at10118ModalSub').textContent='عرض مهمة إدارية';
    $('at10118ModalContent').innerHTML=`<b>${esc(t.title)}</b><br><br><b>من:</b> ${esc(t.from_name||t.from_user||'—')}<br><b>إلى:</b> ${esc(t.to_name||roleLabel(t.to_role)||t.to_user||'—')}<br><b>الأولوية:</b> ${esc(t.priority)}<br><b>نوع الجدولة:</b> ${esc(t.schedule_type||'مجدولة')}<br><b>الجدولة:</b> ${formatDate(t.due_at)}<br><b>أنشئت:</b> ${formatDate(t.created_at)}<br><b>أغلقت:</b> ${t.closed_at?formatDate(t.closed_at):'—'}<br><b>اعتمدت:</b> ${t.approved_at?formatDate(t.approved_at):'—'}<hr style="border:0;border-top:1px solid #dce6e2"><div style="white-space:pre-wrap;line-height:1.8">${esc(t.details||'لا توجد تفاصيل')}</div>`;
    const modal=$('at10118Modal'); if(modal) modal.classList.add('show');
    window.__at10118ReminderTaskId=id;
  }
  ...
  function toggleSchedule(){
    const st=S($('at10118ScheduleType')&&$('at10118ScheduleType').value);
    const due=$('at10118Due'); if(!due) return;
  ...
  function showReminderIfAny(){
    ...
    ensureNavAndSection();
    $('at10118ModalSub').textContent=tasks.length>1?`لديك ${tasks.length} مهام مفتوحة تحتاج متابعة`:'لديك مهمة مفتوحة تحتاج متابعة';
    $('at10118ModalContent').innerHTML=`<b>${esc(reminderTask.title)}</b><br><br><b>من:</b> ${esc(reminderTask.from_name||reminderTask.from_user||'—')}<br><b>الأولوية:</b> ${esc(reminderTask.priority)}<br><b>الجدولة:</b> ${formatDate(reminderTask.due_at)}<br><br><div style="white-space:pre-wrap">${esc(reminderTask.details||'')}</div>`;
    $('at10118Modal').classList.add('show');
  }
  function dismissReminder(){
    const m=$('at10118Modal'); if(m) m.classList.remove('show');
    const id=reminderTask && reminderTask.id || window.__at10118ReminderTaskId;
  ...
  window.tasneefAdminTasksV10118={init,refresh,render,open:openPage,openNew,closeNew,create,view,closeTask,approveTask,setTab,toggleSchedule,dismissReminder,openFromReminder,print:printTasks,diagnose};
