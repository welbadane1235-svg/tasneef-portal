/* Tasneef v10172 - Project Service Command Center
   Standalone read-only reporting module. Does not modify existing modules or data.
*/
(function(){
  'use strict';
  const VERSION='v10172-project-command-center';
  ...
  const userPhone=u=>S(field(u,['phone','mobile','phone_number','mobile_number','whatsapp','contact_phone'],''));
  const cleanNum=v=>{const x=S(v).replace(/[٠-٩]/g,d=>'٠١٢٣٤٥٦٧٨٩'.indexOf(d)).replace(/[^\d.]/g,''); return N(x);};

  function projectDistrict(p){return S(field(p,['district','neighborhood','area','city_area','حي','district_name','location_area'],''));}
  function projectLocation(p){return S(field(p,['location_url','map_url','maps_url','google_maps','location_link','location','site_url','site_link','map'],''));}
  function projectBuildings(p){return cleanNum(field(p,['buildings_count','building_count','buildings','num_buildings','no_buildings','buildingsNo','عدد_المباني'],0));}
  function projectApartments(p){return cleanNum(field(p,['apartments_count','apartment_count','apartments','units','flats','num_apartments','عدد_الشقق'],0));}
  function projectScentDevices(p){return cleanNum(field(p,['scent_devices','scent_device_count','fragrance_devices','perfume_devices','aroma_devices','diffusers','عدد_اجهزة_التعطير'],0));}
  function propertyManagerName(p){return S(field(p,['property_manager','property_manager_name','manager_name','real_estate_manager','asset_manager','مدير_العقار'],''));}
  function propertyManagerPhone(p){return S(field(p,['property_manager_phone','manager_phone','real_estate_manager_phone','asset_manager_phone'],''));}
  function associationPresidentName(p){return S(field(p,['association_president','association_president_name','chairman','hoa_president','president_name','رئيس_الجمعية'],''));}
  function associationPresidentPhone(p){return S(field(p,['association_president_phone','chairman_phone','hoa_president_phone','president_phone'],''));}
  ...
  function supervisorOf(p){
    ...
    const u=sid?STATE.users.find(x=>S(field(x,['id','user_id','uuid']))===sid):STATE.users.find(x=>nameDirect && userName(x)===nameDirect);
    return {id:sid||S(field(u||{},['id','user_id','uuid'],'')),name:nameDirect||userName(u)||'-',phone:phoneDirect||userPhone(u)||''};
  }
