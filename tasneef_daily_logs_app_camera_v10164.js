...
(function(){
  'use strict';
  const VERSION='v10164-daily-worker-attendance-project-flow';
  ...
  async function wWorkers(){
    ...
    return rows.filter(w=>{const st=S(w.status||w.worker_status||w.active); if(/inactive|موقوف|false/i.test(st)) return false; const sup=[w.supervisor_id,w.supervisor_key,w.supervisor_name,w.supervisor,w.manager_name,w.project_supervisor].map(S).filter(Boolean); return !sup.length||sup.includes(uk)||sup.includes(un)||sup.some(x=>x.includes(un)||un.includes(x));})
  ...
  async function wStats(){
    ...
    const preparedKeys=new Set(prep.map(x=>S(x.worker_key)));
    return {prepared:prep.length, working:open.length, available:[...preparedKeys].filter(k=>!openKeys.has(k)).length, prep, open, openKeys};
  }
