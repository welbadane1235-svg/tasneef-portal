/* Tasneef v10142 - Monthly Times Foundation
...
(function(){
  'use strict';
  if(window.__tasneefMonthlyFoundationV10142) return;
  window.__tasneefMonthlyFoundationV10142 = true;

  const SUPABASE_URL='https://zmjdqiswytxlbfgnfjfv.supabase.co';
  const SUPABASE_ANON_KEY='sb_publishable_ADsAC5MtBCusDgX62c8NaQ_LyyuTPeb';
  const MANUAL_TABLE='monthly_time_manual_adjustments';
  const LOCAL_MANUAL='tasneef_monthly_manual_rows_foundation_v10142';
  ...
  async function loadLookups(){
    const [users,projects,workers]=await Promise.all([
      allRows('app_users','id,username,full_name,name,role,is_active',10000),
      allRows('projects','id,name,project_name,title,supervisor_id',10000),
      allRows('workers','id,name,full_name,worker_name,project_id,supervisor_id,assigned_project_id,assigned_supervisor_id',10000)
    ]);
  ...
  function nameUser(id){const u=state.usersById[String(id||'')];return u?S(u.full_name||u.name||u.username||id):'';}
  function nameProject(id){const p=state.projectsById[String(id||'')];return p?S(p.name||p.project_name||p.title||id):'';}
  function nameWorker(id){const w=state.workersById[String(id||'')];return w?S(w.full_name||w.name||w.worker_name||id):'';}
  ...
  function normalize(row){
    const uid=row.supervisor_id??row.user_id??row.created_by??row.manager_id??row.employee_id??'';
    const pid=row.project_id??row.assigned_project_id??row.current_project_id??row.site_id??'';
    const supervisor=pick(row,['supervisor_name','user_name','username','full_name','created_by_name','employee_name','manager_name','المشرف','اسم المشرف'])||nameUser(uid)||'غير محدد';
    const project=pick(row,['project_name','project','site_name','building_name','projectTitle','المشروع','اسم المشروع'])||nameProject(pid)||'غير محدد';
    const workers=pick(row,['worker_names','workers','team_names','worker_name','employee_names','employees','أسماء العمال','العامل'])||nameWorker(row.worker_id)||'';
