/* Tasneef v10144 - Monthly Times Foundation Fix
...
(function(){
  'use strict';
  if(window.__tasneefMonthlyFoundationV10144) return;
  window.__tasneefMonthlyFoundationV10144 = true;

  const SUPABASE_URL='https://zmjdqiswytxlbfgnfjfv.supabase.co';
  const SUPABASE_ANON_KEY='sb_publishable_ADsAC5MtBCusDgX62c8NaQ_LyyuTPeb';
  const MANUAL_TABLE='monthly_time_manual_adjustments';
  const LOCAL_MANUAL='tasneef_monthly_manual_rows_foundation_v10144';
  ...
  async function loadLookups(){
    // v10144: نستخدم select(*) بدل أعمدة محددة لأن اختلاف أسماء الأعمدة في Supabase كان يجعل قراءة المشاريع/المشرفين تفشل بالكامل.
  ...
  function looksLikeMonthlyLog(v){
    if(!v || typeof v!=='object') return false;
    const hasDate=['log_date','date','day','work_date','created_at','start_at','check_in_at','check_out_at','التاريخ','تاريخ'].some(k=>v[k]!=null && S(v[k]));
    if(!hasDate) return false;
    const hasTime=['check_in','check_out','time_in','time_out','log_in','log_out','in_time','out_time','start_time','end_time','logIn','logOut','وقت الدخول','وقت الخروج'].some(k=>v[k]!=null && S(v[k]));
    const hasDuration=['actual_minutes','duration_minutes','total_minutes','minutes','work_minutes','actual_hours','duration_hours','required_minutes','required_hours','travel_minutes','وقت الانتقال'].some(k=>v[k]!=null && S(v[k])!=='');
    const hasIdentity=['project_id','project','project_name','supervisor_id','user_id','worker_id','المشروع','المشرف'].some(k=>v[k]!=null && S(v[k]));
    return hasIdentity && (hasTime || hasDuration);
  }
  function collectLogsFromObject(root,ym,source){
    const out=[]; const seen=new Set();
    function walk(v,depth){
      if(depth>7||v==null) return;
      if(Array.isArray(v)){ for(const x of v) walk(x,depth+1); return; }
      if(typeof v==='object'){
        if(looksLikeMonthlyLog(v) && inMonth(v,ym)){
          let sig='';
          try{sig=JSON.stringify(v).slice(0,1200)}catch(_){sig=String(Math.random())}
          if(!seen.has(sig)){seen.add(sig);out.push({...v,__sourceTable:source});}
        }
    ...
    try{walk(root,0)}catch(e){console.warn('[monthly] collect failed',source,e.message||e)}
    return out;
  }
  function storageLogs(ym){
    ...
    try{
      for(let i=0;i<localStorage.length;i++){
        ...
        if(/orders|inventory|invoice|products|tickets|language|image|photo|base64/i.test(k)) continue;
        const likely=/time|log|daily|attendance|journey|register|سجل|حضور|tasneef|data|sync|storage/i.test(k);
        if(!likely) continue;
        const raw=localStorage.getItem(k);
        if(!raw) continue;
        if(raw.length>15000000) { console.warn('[monthly] skipped huge storage key',k,raw.length); continue; }
        try{ out.push(...collectLogsFromObject(JSON.parse(raw),ym,'localStorage:'+k)); }catch(_){ }
      }
    }catch(e){console.warn('[monthly] localStorage scan failed',e.message||e)}
    return out;
  }

  async function loadLogs(ym){
    // تأسيس v10144: لا نعتمد على فلتر قاعدة البيانات فقط، بل نجمع السجلات من كل مصادر التسجيلات ثم نفلتر الشهر محليًا.
    const tables=['time_logs','daily_logs','work_logs','supervisor_time_logs','daily_registrations','registrations','operation_logs','project_time_logs'];
    let all=[];
    ...
    try{
      const d=window.data||{};
      ['logs','time_logs','timeLogs','dailyLogs','daily_logs','workLogs','registrations','dailyRegistrations','operationLogs','supervisorLogs'].forEach(k=>all.push(...A(d[k]).map(r=>({...r,__sourceTable:'window.data:'+k}))));
      all.push(...collectLogsFromObject(d,ym,'window.data.deep'));
    }catch(_){ }
    all.push(...storageLogs(ym));
    const seen=new Set();
    return all.filter(r=>looksLikeMonthlyLog(r) && inMonth(r,ym)).filter(r=>{
      const idPart=S(r.id||r.uuid||r.log_id||r.record_id||'');
      const datePart=datesOf(r).join('|');
      const sig=(idPart?((r.__sourceTable||'')+'|'+idPart):(datePart+'|'+S(anyVal(r,['project_id','project','project_name','المشروع']))+'|'+S(anyVal(r,['supervisor_id','user_id','supervisor_name','المشرف']))+'|'+S(anyVal(r,['check_in','time_in','logIn','وقت الدخول']))+'|'+S(anyVal(r,['check_out','time_out','logOut','وقت الخروج']))));
