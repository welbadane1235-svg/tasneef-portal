(function(){
  ...
  function calcRow(r, dim){
    // V10282: معادلة الرواتب مطابقة للإكسل:
    // الإجمالي = الأساسي + البدلات
    // إجمالي الراتب على أيام الفترة = الإجمالي / 30 * أيام العمل
    // الصافي = إجمالي الراتب على أيام الفترة + العمولات - الخصومات - خصم السلف + جبر الكسور
    dim = num(dim) || 30;
    r.basic_salary = num(r.basic_salary);
    r.allowance = num(r.allowance);
    r.gross_salary = r.basic_salary + r.allowance;
    r.work_days = num(r.work_days);
    r.absent_days = Math.max(0, num(r.absent_days));
    r.payable_days = Math.max(0, r.work_days - r.absent_days);
    r.salary_by_days = (r.gross_salary / dim) * r.work_days;
    r.absence_deduction = (r.gross_salary / dim) * r.absent_days;

    // إذا المستخدم لم يكتب خصم يدوي، يحسب خصم الغياب تلقائيًا في عمود الخصومات.
    // إذا عدل المستخدم خانة الخصومات، الرقم المكتوب يكون هو الخصم النهائي ويُطرح مباشرة من الصافي.
    if(!r._manual_deductions){
      r.deductions = r.absence_deduction + num(r.manual_extra_deductions || 0);
    }else{
      r.deductions = num(r.deductions);
    }

    r.commission = num(r.commission);
    r.rounding = num(r.rounding);
    r.advance_deduction = num(r.advance_deduction);
    r.net_salary = r.salary_by_days + r.commission - r.deductions - r.advance_deduction + r.rounding;
    return r;
  }
  ...
  function renderSalaryRowHtml(r,i,month){
    return `<tr data-sal-row="${r.entity_type}_${r.entity_id}">
      ...
      <td>${textInput(r,'work_location',120)}</td><td>${esc(r.job_title||'')}</td>
  ...
  function update(type,id,key,value){
    ...
    r[key]=['commission','deductions','rounding','advance_deduction','basic_salary','allowance','absent_days','work_days'].includes(key)?num(value):value;
    ...
    if(key==='deductions'){
      // المستخدم عدّل الخصومات يدويًا؛ نعتبر الرقم المكتوب هو الخصم النهائي حسب معادلة الإكسل.
      r._manual_deductions=true;
      r.deductions=num(value);
    }
    if(key==='absent_days'){
      // تعديل أيام الغياب يحسب الخصم تلقائيًا من جديد.
      r.absent_days=Math.max(0, num(value));
      r.payable_days=Math.max(0, num(r.work_days)-num(r.absent_days));
      r._manual_deductions=false;
      if(r.absent_days>0 && !r._manual_notes) r.notes=absenceNote(r.absent_days);
      if(r.absent_days<=0 && !r._manual_notes && String(r.notes||'').startsWith('خصم غياب')) r.notes='';
    ...
    if(key==='work_days'){
      r.work_days=Math.max(0, num(value));
      r.payable_days=Math.max(0, num(r.work_days)-num(r.absent_days));
    }
