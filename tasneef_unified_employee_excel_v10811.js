/* ===== TASNEEF V10834 - Attendance table + current supervisor payroll Excel ===== */
(function(){
  'use strict';
  if(window.__tasneefUnifiedPayrollExcelV10834) return;
  window.__tasneefUnifiedPayrollExcelV10834=true;

  const VERSION='V10834';
  ...
  function findSupervisorWorker(source,workers){
    const code=normalizeCode(source?.supervisor_employee_code||source?.current_supervisor_code||source?.supervisor_code||'');
    const name=S(source?.supervisor_name||source?.current_supervisor_name||source?.supervisor_display_name||'');
    const id=S(source?.supervisor_id||source?.app_supervisor_id||source?.current_supervisor_id||'');
    return A(workers).find(w=>isSupervisor(w)&&((code&&workerCode(w)===code)||(id&&strongEmployeeAliases(w).includes('id:'+norm(id)))||(name&&norm(workerName(w))===norm(name))))||null;
  }
  function supervisorIdentity(r,workers,project){
    /* V10834: المشرف الحالي في المشروع هو المرجع الأول، ثم صف التوزيع. المشرف الموقوف لا يُصدَّر. */
    const projectWorker=findSupervisorWorker(project,workers);
    const rowWorker=findSupervisorWorker(r,workers);
    const projectHasIdentity=!!S(project?.supervisor_employee_code||project?.current_supervisor_code||project?.supervisor_code||project?.supervisor_name||project?.current_supervisor_name||project?.supervisor_id||project?.current_supervisor_id);
    const rowHasIdentity=!!S(r?.supervisor_employee_code||r?.supervisor_code||r?.supervisor_name||r?.supervisor_display_name||r?.supervisor_id||r?.current_supervisor_id);
    let selected=null,source=null;
    if(projectWorker){if(isActiveStatus(projectWorker))selected=projectWorker;else if(rowWorker&&isActiveStatus(rowWorker))selected=rowWorker;}
    else if(projectHasIdentity)source=project;
    if(!selected&&!source){if(rowWorker){if(isActiveStatus(rowWorker))selected=rowWorker;}else if(rowHasIdentity)source=r;}
    if(!selected&&!source)return{key:'unassigned',code:'',name:'بدون مشرف',id:'',active:false};
    const code=normalizeCode(workerCode(selected)||source?.supervisor_employee_code||source?.current_supervisor_code||source?.supervisor_code||'');
    const name=S(workerName(selected)||source?.supervisor_name||source?.current_supervisor_name||source?.supervisor_display_name||'');
    const id=S(selected?.id||source?.supervisor_id||source?.app_supervisor_id||source?.current_supervisor_id||'');
    if(!code&&!name&&!id)return{key:'unassigned',code:'',name:'بدون مشرف',id:'',active:false};
    return{key:code?('code:'+norm(code)):(id?'id:'+norm(id):('name:'+norm(name))),code,name:name||code||'بدون مشرف',id,active:true};
  }
  function projectInfoFromAssignment(r,projectMap){const pid=S(r?.project_id||r?.project_code||r?.app_project_id||''),p=projectMap.get(pid)||projectMap.get('name:'+norm(S(r?.project_name||r?.project_display_name||'')))||{};const name=S(r?.project_name||r?.project_display_name||projectName(p)||'بدون مشروع');const full=isFullTimeProject(r)||isFullTimeProject(p);return{id:pid||projectId(p),name,full,type:full?'دوام كامل':'زيارة يومية',start:assignmentStart(r),end:assignmentEnd(r),project:p};}
  ...
  function dedupeDistributionRows(rows,month,index,workers,projectMap){
    ...
    A(rows).forEach((r,sourceIndex)=>{
      ...
      const pi=projectInfoFromAssignment(r,projectMap),sup=supervisorIdentity(r,workers,pi.project);if(!assignmentProjectKey(pi))return;
  ...
  function chooseAssignmentSupervisor(asg,filters){
    ...
    entries=entries.filter(x=>x.active!==false);
    entries.sort((a,b)=>N(b.latestMs)-N(a.latestMs)||N(b.projectCount)-N(a.projectCount)||N(b.weight)-N(a.weight)||S(a.name).localeCompare(S(b.name),'ar'));
    return entries[0];
  }
  ...
  async function addMainPayrollSheet(wb,month,details,filters){
    ...
    /* V10834: الملف قابل للتعديل بالكامل؛ لا تُفعّل حماية الورقة حتى لا يطلب Excel كلمة مرور. */
    return ws;
  }
  async function addMonthlyAttendanceSheet(wb,month,details){
    ...
    /* V10834: خلايا التحضير والمعادلات قابلة للتعديل دون كلمة مرور. */
    return ws;
  }
  ...
  async function exportExcel(btn){
    ...
    try{
      ...
      try{currentEffective=A(window.tasneefCoreUnifiedV413?.effectiveDistributionRows?.(f.month));}catch(_){currentEffective=[];}
      const distributionSource=currentEffective.length?currentEffective:[...A(dist),...A(extraAssignments)];
      const deduped=dedupeDistributionRows(distributionSource,f.month,index,workers,projectMap),assignmentRows=deduped.rows;
      ...
      const diagnostics={activeEmployeesCount,employeesAfterFilters:sourceCandidates.length,employeesAfterJoin,uniqueEmployeesCount:uniqueDetails.length,exportedEmployeesCount:filtered.length,missingEmployees,duplicatedEmployees,distributionSource:currentEffective.length?'effectiveDistributionRows':'monthly_distribution',attendanceSource:'attendance',distributionDuplicateRows:deduped.duplicates,multiSupervisorConflicts:[...new Set(multiSupervisorConflicts)],weakIdentityEmployees,dataPreparationMs:Math.round(performance.now()-started),generationMs:0};console.table(diagnostics);
  ...
  function bind(){const btn=$('cu413ExportWorkersExcel');if(!btn||btn.dataset.v10834==='1')return;btn.dataset.v10834='1';btn.onclick=function(e){e?.preventDefault?.();exportExcel(btn);return false;};btn.textContent='تنزيل كشف الموظفين والرواتب Excel';window.exportSupervisorEmployeesExcelV10834=exportExcel;window.exportSupervisorEmployeesExcel=exportExcel;if(window.tasneefCoreUnifiedV413)window.tasneefCoreUnifiedV413.exportSupervisorEmployeesExcel=()=>exportExcel($('cu413ExportWorkersExcel'));}
  window.__tasneefUnifiedPayrollDiagnosticsV10834={roleGroup,canonicalEmployeeKey,detailMatchesExactFilter,cycleEligibility,attendanceSymbol,mergeWorkers,buildEmployeeAliasIndex,resolveEmployeeKey,dedupeDistributionRows,sortByUnifiedHierarchy,supervisorGroups,chooseAssignmentSupervisor,supervisorIdentity,buildAttendanceStats};
