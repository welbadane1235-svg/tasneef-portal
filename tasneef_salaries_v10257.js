(function(){
  ...
  function groupMetaFromAttendance(g){
    ...
    const sid=first.supervisor_id || workerSupId(w) || projectSupId(first.project_id||workerProjectId(w)) || '';
    ...
    const pids=[...new Set(g.records.map(a=>a.project_id||workerProjectId(w)).filter(Boolean).map(String))];
    const projects=pids.map(projectNameFromId).filter(Boolean).join('، ');
    return {name, profile, supervisor_id:sid||null, supervisor_name:sname||'', project_name:projects, projectIds:pids};
  }
