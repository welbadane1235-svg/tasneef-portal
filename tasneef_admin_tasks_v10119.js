/* Tasneef v10119 - Administrative Tasks Visibility & Reminder Stable Fix
   Scope: new section only. Does not touch finance, inventory, orders, tickets, contracts, attendance, monthly, or permissions.
*/
(function(){
  'use strict';
  if(window.__tasneefAdminTasksV10119) return;
  window.__tasneefAdminTasksV10119 = true;

  const VERSION='v10119-admin-tasks-advanced';
  const SUPABASE_URL='https://zmjdqiswytxlbfgnfjfv.supabase.co';
  const SUPABASE_ANON_KEY='sb_publishable_ADsAC5MtBCusDgX62c8NaQ_LyyuTPeb';
  const TABLE='admin_tasks';
  const LOCAL_KEY='tasneef_admin_tasks_v10119_local';
  const DISMISS_PREFIX='tasneef_admin_task_dismiss_v10119_';
  ...
  async function loadTasks(){
    try{
      ...
      console.warn('v10119 load online failed, using local', e);
  ...
  async function saveTask(payload){
    ...
    try{
      ...
      console.warn('v10119 save online failed, using local', e);
  ...
  function injectCss(){
    if($('tasneefAdminTasksStyleV10119')) return;
    const st=document.createElement('style'); st.id='tasneefAdminTasksStyleV10119';
    st.textContent=`
      .at10119-head{display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap}
      .at10119-grid{display:grid;grid-template-columns:repeat(4,minmax(160px,1fr));gap:10px}.at10119-grid .wide{grid-column:span 2}.at10119-grid .full{grid-column:1/-1}
      .at10119-muted{color:#60706a;font-size:13px;line-height:1.7}.at10119-actions{display:flex;gap:8px;flex-wrap:wrap;align-items:center}.at10119-chip{display:inline-flex;align-items:center;gap:5px;border-radius:999px;padding:5px 9px;background:#eef6f3;color:#0A4033;font-weight:900;font-size:12px}.at10119-chip.red{background:#fde8e8;color:#9d2222}.at10119-chip.amber{background:#fff5da;color:#8a6700}.at10119-chip.green{background:#e8f4ee;color:#137a4b}.at10119-btn-mini{padding:7px 9px!important;border-radius:9px!important;font-size:12px!important}.at10119-form{display:none}.at10119-form.show{display:block}.at10119-empty{padding:18px;text-align:center;color:#60706a}.at10119-task-title{font-weight:900;color:#0A4033}.at10119-details{max-width:420px;white-space:normal;line-height:1.7;color:#263b35}.at10119-badge-count{display:inline-grid;place-items:center;min-width:20px;height:20px;border-radius:999px;background:#b83232;color:#fff;font-size:11px;margin-inline-start:6px;padding:0 5px}
      .at10119-modal{position:fixed;inset:0;background:rgba(2,15,11,.48);z-index:999999;display:none;align-items:center;justify-content:center;padding:18px}.at10119-modal.show{display:flex}.at10119-box{width:min(560px,96vw);background:#fff;border-radius:24px;border:1px solid #dce6e2;box-shadow:0 24px 70px rgba(0,0,0,.25);overflow:hidden}.at10119-box-head{background:linear-gradient(135deg,#0A4033,#0f5a47);color:#fff;padding:18px 20px}.at10119-box-head h3{margin:0 0 6px}.at10119-box-body{padding:18px 20px}.at10119-reminder-card{background:#f8fbfa;border:1px solid #dce6e2;border-radius:16px;padding:13px;line-height:1.8}.at10119-box-actions{display:flex;gap:8px;justify-content:flex-end;flex-wrap:wrap;padding:0 20px 20px}.at10119-box-actions .light{background:#eef6f3;color:#0A4033;border:1px solid #dce6e2}
      .at10119-tabs{display:flex;gap:8px;flex-wrap:wrap;margin:12px 0}.at10119-tab{border:1px solid #dce6e2;background:#fff;color:#0A4033;border-radius:999px;padding:9px 14px;font-weight:900;cursor:pointer}.at10119-tab.active{background:#0A4033;color:#fff}.at10119-tab .count{display:inline-grid;place-items:center;min-width:20px;height:20px;background:#f0b429;color:#0A4033;border-radius:999px;margin-inline-start:6px;padding:0 5px;font-size:11px}
      @media(max-width:900px){.at10119-grid{grid-template-columns:1fr}.at10119-grid .wide{grid-column:auto}.at10119-details{max-width:260px}.at10119-head{align-items:flex-start}}
    `;
  ...
  function ensureNavAndSection(){
    ...
    if(!$('adminTasks')){
      ...
      sec.innerHTML=`
        <div class="card section-head at10119-head">
          ...
          <div class="at10119-actions"><button onclick="window.tasneefAdminTasksV10119.openNew()" id="at10119NewBtn">إضافة مهمة</button><button class="light" onclick="window.tasneefAdminTasksV10119.refresh()">تحديث</button></div>
        </div>
        <div class="card at10119-form" id="at10119FormCard">
          <h2>إضافة مهمة إدارية</h2>
          <div class="at10119-muted">المرسل يتم تسجيله تلقائيًا باسم المستخدم الحالي. المستلم يتم اختياره من أسماء المستخدمين في إدارة المستخدمين فقط، وتظهر المهمة في حساب المستلم وللمرسل لمتابعة الإغلاق والاعتماد.</div>
          <div class="at10119-grid">
            <div class="wide"><label>عنوان المهمة</label><input id="at10119Title" placeholder="مثال: مراجعة تقرير التشغيل اليومي"></div>
            <div><label>نوع الجدولة</label><select id="at10119ScheduleType" onchange="window.tasneefAdminTasksV10119.toggleSchedule()"><option>مهمة فورية</option><option selected>مجدولة</option></select></div>
            <div><label>الأولوية</label><select id="at10119Priority"><option>عادي</option><option>مهم</option><option>عاجل</option></select></div>
            <div><label>تاريخ ووقت الجدولة</label><input id="at10119Due" type="datetime-local"></div>
            <div class="wide"><label>المستلم من إدارة المستخدمين</label><select id="at10119Recipient"></select><div class="at10119-muted">تظهر هنا أسماء المستخدمين المسجلين في قسم إدارة المستخدمين فقط.</div></div>
            <div class="wide"><label>نوع المهمة</label><select id="at10119Type"><option>مهمة إدارية</option><option>متابعة تشغيل</option><option>متابعة مالية</option><option>متابعة مشروع</option><option>طلب تقرير</option><option>ملاحظة عاجلة</option></select></div>
            <div class="full"><label>تفاصيل المهمة</label><textarea id="at10119Details" placeholder="اكتب تفاصيل المهمة المطلوبة بوضوح..."></textarea></div>
          </div>
          <div class="actions"><button onclick="window.tasneefAdminTasksV10119.create()">حفظ المهمة</button><button class="light" onclick="window.tasneefAdminTasksV10119.closeNew()">إلغاء</button></div>
        </div>
        <div class="kpis small" id="at10119Kpis"></div>
        <div class="card">
          <div class="at10119-tabs" id="at10119Tabs">
            <button class="at10119-tab active" data-tab="mine" onclick="window.tasneefAdminTasksV10119.setTab('mine')">مهامي <span class="count" id="at10119CountMine">0</span></button>
            <button class="at10119-tab" data-tab="open" onclick="window.tasneefAdminTasksV10119.setTab('open')">مهام مفتوحة <span class="count" id="at10119CountOpen">0</span></button>
            <button class="at10119-tab" data-tab="closed" onclick="window.tasneefAdminTasksV10119.setTab('closed')">مهام مغلقة <span class="count" id="at10119CountClosed">0</span></button>
            <button class="at10119-tab" data-tab="approved" onclick="window.tasneefAdminTasksV10119.setTab('approved')">تم الاعتماد <span class="count" id="at10119CountApproved">0</span></button>
          </div>
          <div class="filters">
            <input id="at10119Search" placeholder="بحث في المهام" oninput="window.tasneefAdminTasksV10119.render()">
            <select id="at10119PriorityFilter" onchange="window.tasneefAdminTasksV10119.render()"><option value="">كل الأولويات</option><option>عادي</option><option>مهم</option><option>عاجل</option></select>
            <select id="at10119TypeFilter" onchange="window.tasneefAdminTasksV10119.render()"><option value="">كل أنواع المهام</option><option>مهمة إدارية</option><option>متابعة تشغيل</option><option>متابعة مالية</option><option>متابعة مشروع</option><option>طلب تقرير</option><option>ملاحظة عاجلة</option></select>
            <select id="at10119ScheduleFilter" onchange="window.tasneefAdminTasksV10119.render()"><option value="">فورية ومجدولة</option><option>مهمة فورية</option><option>مجدولة</option></select>
            <input id="at10119FromDate" type="date" title="من تاريخ" onchange="window.tasneefAdminTasksV10119.render()">
            <input id="at10119ToDate" type="date" title="إلى تاريخ" onchange="window.tasneefAdminTasksV10119.render()">
            <button class="light" onclick="window.tasneefAdminTasksV10119.print()">طباعة المعروض</button>
          </div>
          <div class="table-wrap"><table><thead><tr><th>الحالة</th><th>المهمة</th><th>من</th><th>إلى</th><th>الجدولة</th><th>أنشئت</th><th>أغلقت</th><th>اعتمدت</th><th>إجراء</th></tr></thead><tbody id="at10119Body"></tbody></table></div>
        </div>`;
      content.appendChild(sec);
    }
    if(!$('at10119Modal')){
      const modal=document.createElement('div'); modal.id='at10119Modal'; modal.className='at10119-modal';
      modal.innerHTML=`<div class="at10119-box"><div class="at10119-box-head"><h3>تذكير مهمة إدارية</h3><div id="at10119ModalSub">لديك مهمة تحتاج متابعة</div></div><div class="at10119-box-body"><div id="at10119ModalContent" class="at10119-reminder-card"></div></div><div class="at10119-box-actions"><button class="light" onclick="window.tasneefAdminTasksV10119.dismissReminder()">إغلاق التذكير</button><button onclick="window.tasneefAdminTasksV10119.openFromReminder()">فتح المهمة</button></div></div>`;
    ...
    if(!document.querySelector('button.nav[data-page="adminTasks"]')){
      ...
      btn.innerHTML='مهام إدارية <span id="at10119NavBadge" class="at10119-badge-count" style="display:none">0</span>';
    ...
    fillRecipients();
    if(!canCreate()){
      const nb=$('at10119NewBtn'); if(nb) nb.style.display='none';
      const fc=$('at10119FormCard'); if(fc) fc.classList.remove('show');
    }
  }

  async function fillRecipients(){
    const sel=$('at10119Recipient'); if(!sel) return;
    const old=sel.value;
  ...
  function parseRecipient(){
    const val=S($('at10119Recipient')&&$('at10119Recipient').value);
  ...
  function priorityChip(p){ p=S(p)||'عادي'; const cls=p==='عاجل'?'red':(p==='مهم'?'amber':'green'); return `<span class="at10119-chip ${cls}">${esc(p)}</span>`; }
  function statusChip(t){ return t.status==='approved'?'<span class="at10119-chip green">تم الاعتماد</span>':(t.status==='closed'?'<span class="at10119-chip amber">مغلقة</span>':'<span class="at10119-chip amber">مفتوحة</span>'); }
  ...
  function filteredRows(){
    const q=S($('at10119Search')&&$('at10119Search').value).toLowerCase();
    const pr=S($('at10119PriorityFilter')&&$('at10119PriorityFilter').value);
    const tf=S($('at10119TypeFilter')&&$('at10119TypeFilter').value);
    const sf=S($('at10119ScheduleFilter')&&$('at10119ScheduleFilter').value);
    const from=S($('at10119FromDate')&&$('at10119FromDate').value);
    const to=S($('at10119ToDate')&&$('at10119ToDate').value);
  ...
  function render(){
    ...
    const k=$('at10119Kpis'); if(k) k.innerHTML=`<div class="kpi"><small>مهامي</small><b>${mine}</b></div><div class="kpi"><small>مهام مفتوحة</small><b>${open}</b></div><div class="kpi"><small>مهام مغلقة تنتظر اعتماد</small><b>${closed}</b></div><div class="kpi"><small>المهام المتأخرة</small><b>${overdue}</b></div>`;
    const badge=$('at10119NavBadge'); if(badge){ badge.textContent=mine; badge.style.display=mine? 'inline-grid':'none'; }
    const counts={Mine:mine,Open:open,Closed:closed,Approved:approved}; Object.keys(counts).forEach(k2=>{ const el=$('at10119Count'+k2); if(el) el.textContent=counts[k2]; });
    document.querySelectorAll('.at10119-tab').forEach(b=>b.classList.toggle('active', b.dataset.tab===currentTab));
    const body=$('at10119Body'); if(!body) return;
    if(!rows.length){ body.innerHTML=`<tr><td colspan="9" class="at10119-empty">لا توجد مهام حسب الفلتر الحالي</td></tr>`; return; }
    body.innerHTML=rows.map(t=>`
      <tr data-at-id="${esc(t.id)}">
        <td>${statusChip(t)}${isOverdue(t)?' <span class="at10119-chip red">متأخرة</span>':''}</td>
        <td><div class="at10119-task-title">${esc(t.title||'بدون عنوان')}</div><div>${priorityChip(t.priority)} <span class="at10119-chip">${esc(t.task_type||'مهمة إدارية')}</span> <span class="at10119-chip">${esc(t.schedule_type||'مجدولة')}</span></div><div class="at10119-details">${esc(t.details||'')}</div></td>
        ...
        <td><div class="row-actions"><button class="light at10119-btn-mini" onclick="window.tasneefAdminTasksV10119.view('${esc(t.id)}')">عرض</button>${t.status==='open'&&matchesCurrentUser(t)?`<button class="at10119-btn-mini" onclick="window.tasneefAdminTasksV10119.closeTask('${esc(t.id)}')">إغلاق المهمة</button>`:''}${t.status==='closed'&&isSender(t)?`<button class="at10119-btn-mini" onclick="window.tasneefAdminTasksV10119.approveTask('${esc(t.id)}')">اعتماد</button>`:''}</div></td>
      </tr>`).join('');
  }
  async function refresh(){ await loadTasks(); render(); }
  function openNew(){ if(!canCreate()) return alert('إضافة المهام متاحة للإدارة فقط'); ensureNavAndSection(); $('at10119FormCard').classList.add('show'); fillRecipients(); }
  function closeNew(){ const f=$('at10119FormCard'); if(f) f.classList.remove('show'); }
  async function create(){
    if(!canCreate()) return alert('إضافة المهام متاحة للإدارة فقط');
    const title=S($('at10119Title')&&$('at10119Title').value); if(!title) return alert('اكتب عنوان المهمة');
    const rec=parseRecipient(); if(!rec.to_user && !rec.to_name) return alert('اختر المستلم من إدارة المستخدمين');
    const u=currentUser();
    const scheduleType=S($('at10119ScheduleType')&&$('at10119ScheduleType').value)||'مجدولة';
    const dueVal = scheduleType==='مهمة فورية' ? nowIso() : dtLocalToIso($('at10119Due')&&$('at10119Due').value);
    const payload=Object.assign({
      title, details:S($('at10119Details')&&$('at10119Details').value), task_type:S($('at10119Type')&&$('at10119Type').value)||'مهمة إدارية', priority:S($('at10119Priority')&&$('at10119Priority').value)||'عادي', schedule_type:scheduleType,
    ...
    ['at10119Title','at10119Details','at10119Due'].forEach(id=>{ const el=$(id); if(el) el.value=''; });
    currentTab='open';
    closeNew();
    await refresh();
  ...
  function view(id){
    ...
    ensureNavAndSection();
    $('at10119ModalSub').textContent='عرض مهمة إدارية';
    $('at10119ModalContent').innerHTML=`<b>${esc(t.title)}</b><br><br><b>من:</b> ${esc(t.from_name||t.from_user||'—')}<br><b>إلى:</b> ${esc(t.to_name||roleLabel(t.to_role)||t.to_user||'—')}<br><b>الأولوية:</b> ${esc(t.priority)}<br><b>نوع الجدولة:</b> ${esc(t.schedule_type||'مجدولة')}<br><b>الجدولة:</b> ${formatDate(t.due_at)}<br><b>أنشئت:</b> ${formatDate(t.created_at)}<br><b>أغلقت:</b> ${t.closed_at?formatDate(t.closed_at):'—'}<br><b>اعتمدت:</b> ${t.approved_at?formatDate(t.approved_at):'—'}<hr style="border:0;border-top:1px solid #dce6e2"><div style="white-space:pre-wrap;line-height:1.8">${esc(t.details||'لا توجد تفاصيل')}</div>`;
    const modal=$('at10119Modal'); if(modal) modal.classList.add('show');
    window.__at10119ReminderTaskId=id;
  }
  ...
  function toggleSchedule(){
    const st=S($('at10119ScheduleType')&&$('at10119ScheduleType').value);
    const due=$('at10119Due'); if(!due) return;
  ...
  function showReminderIfAny(){
    ...
    ensureNavAndSection();
    $('at10119ModalSub').textContent=tasks.length>1?`لديك ${tasks.length} مهام مفتوحة تحتاج متابعة`:'لديك مهمة مفتوحة تحتاج متابعة';
    $('at10119ModalContent').innerHTML=`<b>${esc(reminderTask.title)}</b><br><br><b>من:</b> ${esc(reminderTask.from_name||reminderTask.from_user||'—')}<br><b>الأولوية:</b> ${esc(reminderTask.priority)}<br><b>الجدولة:</b> ${formatDate(reminderTask.due_at)}<br><br><div style="white-space:pre-wrap">${esc(reminderTask.details||'')}</div>`;
    $('at10119Modal').classList.add('show');
  }
  function dismissReminder(){
    const m=$('at10119Modal'); if(m) m.classList.remove('show');
    const id=reminderTask && reminderTask.id || window.__at10119ReminderTaskId;
  ...
  window.tasneefAdminTasksV10119={init,refresh,render,open:openPage,openNew,closeNew,create,view,closeTask,approveTask,setTab,toggleSchedule,dismissReminder,openFromReminder,print:printTasks,diagnose};
