...
(function(){
  'use strict';
  if(window.__tasneefInventoryAuditRootV10059) return;
  window.__tasneefInventoryAuditRootV10059 = true;
  ...
  function updateWelcomeName(){
    try{
      const name = userName();
      if(!name) return;
      const hero = document.querySelector('.hero-title-v155');
      const h1 = hero && hero.querySelector('h1');
      if(h1){
        h1.textContent = 'مرحباً بك، ' + name;
        h1.dataset.tasneefWelcomeV10059 = '1';
      }
      const p = hero && hero.querySelector('p');
      if(p && !p.dataset.originalTextV10059){
        p.dataset.originalTextV10059 = p.textContent || '';
        p.textContent = 'نتمنى لك يوماً موفقاً في إدارة أعمال التشغيل والمتابعة.';
      }
    }catch(_){ }
  }

  async function nextAuditNo(){
    try{
      if(window.sb){
        const r = await sb.rpc('tasneef_next_inventory_audit_no_v10059');
        if(!r.error && r.data) return S(r.data);
      }
    }catch(_){ }
    try{
      const r = await sb.from(SESSION_TABLE).select('id,audit_no').order('id',{ascending:false}).limit(1);
      const rows = r.data || [];
      const nums = rows.map(x => {
        const m = S(x.audit_no).match(/(\d+)$/);
        return m ? Number(m[1]) : N(x.id);
      }).filter(Boolean);
      const n = (nums.length ? Math.max(...nums) : 0) + 1;
      return 'AUD-' + String(n).padStart(6,'0');
    }catch(_){
      return 'AUD-' + String(Date.now()).slice(-6);
    }
  }

  function ensureStyle(){
    if($('auditStyleV10059')) return;
    const st = document.createElement('style');
    st.id = 'auditStyleV10059';
    st.textContent = `
      .inventory-audit-page-v10059 .audit-toolbar-v10059{display:flex;gap:8px;flex-wrap:wrap;align-items:end}
      .audit-kpis-v10059{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin:12px 0}
      .audit-kpi-v10059{background:#fff;border:1px solid var(--line,#dce6e2);border-radius:16px;padding:12px}
      .audit-kpi-v10059 small{color:#60706a}.audit-kpi-v10059 b{display:block;color:#0A4033;font-size:22px;margin-top:5px}
      .audit-open-banner-v10059{background:#eef8f4;border:1px solid #cfe4dc;color:#064b38;border-radius:14px;padding:10px 12px;margin:10px 0;font-weight:800}
      .audit-table-v10059 input,.audit-table-v10059 select{min-width:90px}
      .audit-glass-overlay-v10059{position:absolute;inset:0;z-index:999998;background:rgba(245,250,248,.72);backdrop-filter:blur(7px);-webkit-backdrop-filter:blur(7px);display:flex;align-items:center;justify-content:center;padding:18px;border-radius:22px}
      .audit-glass-card-v10059{width:min(560px,92vw);background:rgba(255,255,255,.86);border:1px solid rgba(207,226,220,.95);box-shadow:0 22px 60px rgba(0,45,35,.16);border-radius:26px;padding:26px;text-align:center;color:#063f32}
      .audit-glass-card-v10059 h2{margin:0 0 10px;color:#063f32}.audit-glass-card-v10059 p{line-height:1.9;color:#35554c}
      .audit-glass-actions-v10059{display:flex;gap:10px;justify-content:center;flex-wrap:wrap;margin-top:14px}
      @media(max-width:900px){.audit-kpis-v10059{grid-template-columns:1fr 1fr}}
      @media print{body *{visibility:hidden!important}#inventoryAudit,#inventoryAudit *{visibility:visible!important}#inventoryAudit{position:absolute;inset:0;background:#fff}.side,.hero,.audit-no-print-v10059{display:none!important}}
    `;
  ...
  function sectionHtml(){
    return `
      <div class="card section-head audit-head-v10059">
        <div>
          <h2>قسم الجرد</h2>
          <p>جرد مستقل مرتبط بالمالية والمخزون. عند فتح عملية جرد يتم قفل المالية والمخزون حتى إغلاق العملية.</p>
        </div>
        <div class="actions audit-no-print-v10059">
          <button onclick="tasneefInventoryAuditV10059.newAudit()">+ عملية جرد جديدة</button>
          <button class="light" onclick="tasneefInventoryAuditV10059.load(true)">تحديث</button>
        </div>
      </div>

      <div id="auditLockMsgV10059" class="audit-open-banner-v10059 hidden"></div>

      <div class="card">
        <div class="table-head">
          <h2>عمليات الجرد</h2>
          <div class="row-actions audit-no-print-v10059">
            <button class="light" onclick="tasneefInventoryAuditV10059.load(true)">تحديث</button>
            <button class="light" onclick="tasneefInventoryAuditV10059.print()">طباعة</button>
            <button class="light" onclick="tasneefInventoryAuditV10059.exportCsv()">تصدير CSV</button>
            <button class="light" onclick="tasneefInventoryAuditV10059.reloadOpenItems(true)">إعادة تحميل بنود الجرد</button>
          </div>
        </div>
        <div id="auditSessionsV10059" class="quick-list"></div>
      </div>

      <div class="card" id="auditDetailsCardV10059">
        <div class="table-head">
          <h2 id="auditTitleV10059">تفاصيل الجرد</h2>
          <div class="row-actions audit-no-print-v10059">
            <button class="light" onclick="tasneefInventoryAuditV10059.reloadOpenItems(true)">تحميل المنتجات</button>
            <button class="light" onclick="tasneefInventoryAuditV10059.printCurrentAudit()">طباعة عملية الجرد</button>
            <button onclick="tasneefInventoryAuditV10059.closeAudit(true)">اعتماد وتعديل المخزون</button>
            <button class="light" onclick="tasneefInventoryAuditV10059.closeAudit(false)">إغلاق بدون تعديل المخزون</button>
          </div>
        </div>
        <div class="filters audit-no-print-v10059">
          <input id="auditSearchV10059" placeholder="بحث باسم المنتج أو الكود" oninput="tasneefInventoryAuditV10059.renderCurrentItems()">
          <select id="auditStatusFilterV10059" onchange="tasneefInventoryAuditV10059.renderCurrentItems()">
            <option value="">كل الحالات</option>
            <option>لم يتم جرده</option><option>مطابق</option><option>نقص</option><option>زيادة</option>
          </select>
        </div>
        <div id="auditSummaryV10059" class="audit-kpis-v10059"></div>
        <div class="table-wrap">
          <table class="audit-table-v10059">
            ...
            <tbody id="auditItemsBodyV10059"></tbody>
          </table>
        </div>
      </div>`;
  }
  ...
  function ensureSection(){
    ...
    if(!sec){
      ...
      sec.className = 'page hidden inventory-audit-page-v10059';
      main.appendChild(sec);
    }
    if(sec.dataset.auditV10059 !== '1'){
      sec.classList.add('inventory-audit-page-v10059');
      sec.innerHTML = sectionHtml();
      sec.dataset.auditV10059 = '1';
    }
  }
  ...
  async function renderSessions(){
    const wrap = $('auditSessionsV10059') || $('auditSessionsV10046');
    ...
    wrap.innerHTML = rows.map(a => `
      <div class="quick-item">
        ...
        <div class="row-actions"><button class="light" onclick="tasneefInventoryAuditV10059.renderItems(${Number(a.id)})">عرض</button><button class="light" onclick="tasneefInventoryAuditV10059.printAudit(${Number(a.id)})">طباعة</button>${a.status==='open'?`<button onclick="tasneefInventoryAuditV10059.closeSpecificAudit(${Number(a.id)},true)">اعتماد وتعديل</button><button class="danger" onclick="tasneefInventoryAuditV10059.closeSpecificAudit(${Number(a.id)},false)">إغلاق</button>`:''}</div>
  ...
  function summaryHtml(rows){
    ...
    return `
      <div class="audit-kpi-v10059"><small>إجمالي المنتجات</small><b>${s.total}</b></div>
      <div class="audit-kpi-v10059"><small>تم جردها</small><b>${s.done}</b></div>
      <div class="audit-kpi-v10059"><small>مطابق</small><b>${s.equal}</b></div>
      <div class="audit-kpi-v10059"><small>نقص / زيادة</small><b>${s.shortage} / ${s.extra}</b></div>`;
  }
  ...
  async function renderItems(auditId, auditObj=null){
    ...
    const body = $('auditItemsBodyV10059') || $('auditItemsBodyV10046');
    ...
    const search = S($('auditSearchV10059')?.value).toLowerCase();
    const fstatus = S($('auditStatusFilterV10059')?.value);
    ...
    if($('auditTitleV10059')) $('auditTitleV10059').textContent = `تفاصيل الجرد ${E(currentAudit?.audit_no || auditId)}`;
    if($('auditSummaryV10059')) $('auditSummaryV10059').innerHTML = summaryHtml(rows);
  ...
  function rowHtml(it){
    ...
    return `<tr data-audit-item-id="${Number(it.id)}">
      ...
      <td><input type="number" step="0.01" value="${has ? E(it.actual_qty) : ''}" onchange="tasneefInventoryAuditV10059.saveQty(${Number(it.id)},this.value,${N(it.system_qty)})"></td>
      <td>${diff === null ? '-' : diff.toFixed(2)}</td>
      <td><span class="badge ${cls}">${st}</span></td>
      <td><input value="${E(it.notes || '')}" onchange="tasneefInventoryAuditV10059.saveNote(${Number(it.id)},this.value)"></td>
    </tr>`;
  }
  ...
  async function load(force=false){
    ...
    const box = $('auditLockMsgV10059') || $('auditLockMsgV10046');
    if(box){
    ...
    if(currentAudit){
      ...
      if($('auditSummaryV10059')) $('auditSummaryV10059').innerHTML = '';
    }
  ...
  async function newAudit(){
    ...
    const auditNo = await nextAuditNo();
  ...
  async function applyAuditToInventory(a){
    ...
    try{
      const rpc = await sb.rpc('tasneef_apply_inventory_audit_v10059', {p_audit_id:Number(a.id)});
      if(!rpc.error){
  ...
  function financeElements(fin){
    return [...fin.querySelectorAll('input,select,textarea,button')].filter(el => !el.closest('#financeAuditGlassV10059'));
  }

  function restoreFinance(fin){
    const lock = $('financeAuditGlassV10059');
    lock?.remove();
    financeElements(fin).forEach(el => {
      if(el.dataset.auditDisabledV10059 === '1'){
        if(el.dataset.auditWasDisabledV10059 !== '1') el.disabled = false;
        delete el.dataset.auditDisabledV10059;
        delete el.dataset.auditWasDisabledV10059;
      }
    });
  }
  ...
  async function applyFinanceLock(){
    ...
    financeElements(fin).forEach(el => {
      if(el.dataset.auditDisabledV10059 !== '1'){
        el.dataset.auditWasDisabledV10059 = el.disabled ? '1' : '0';
        el.dataset.auditDisabledV10059 = '1';
      }
      el.disabled = true;
    });
    let overlay = $('financeAuditGlassV10059');
    if(!overlay){
      overlay = document.createElement('div');
      overlay.id = 'financeAuditGlassV10059';
      overlay.className = 'audit-glass-overlay-v10059';
      fin.appendChild(overlay);
    }
    overlay.innerHTML = `<div class="audit-glass-card-v10059">
      ...
      <div class="audit-glass-actions-v10059">
        <button onclick="showPage('inventoryAudit',document.querySelector('[data-page=inventoryAudit]')); setTimeout(function(){tasneefInventoryAuditV10059.load(true)},80)">فتح الجرد</button>
        <button class="light" onclick="tasneefInventoryAuditV10059.load(true)">تحديث</button>
      </div>
    </div>`;
  }
  ...
  const api = {load,newAudit,reloadOpenItems,renderItems,renderCurrentItems,saveQty,saveNote,closeAudit,closeSpecificAudit,print,printAudit,printCurrentAudit,exportCsv,applyFinanceLock,updateWelcomeName};
  window.tasneefInventoryAuditV10059 = api;
  window.tasneefInventoryAuditV10046 = api; window.tasneefInventoryAuditV10056 = api; window.tasneefInventoryAuditV10057 = api; // aliases for existing sidebar/buttons

  setInterval(removeOldVersionMark, 1200);
  setTimeout(updateWelcomeName, 300);
  setInterval(updateWelcomeName, 2500);
