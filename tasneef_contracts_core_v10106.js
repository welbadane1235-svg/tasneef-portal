/* Tasneef v10106 - Annual Services Linked To Contract Services
...
(function(){
  'use strict';
  if(window.__tasneefContractsCoreV10106) return;
  window.__tasneefContractsCoreV10106 = true;
  window.__tasneefContractsCoreV10106 = true;

  const VERSION='v10106-annual-linked-contract-services';
  const TABLE='project_contract_smart';
  const LS_KEY='tasneef_contract_smart_v10106_cache';
  ...
  function renderCore(record){const box=$('contractCoreServices'); if(!box) return; box.classList.remove('three'); box.classList.add('contract-grid-v10'); box.innerHTML=CORE.map(item=>{const r=record.contracts[item.key]||{}; const miss=missing(r); return `<section class="contract-card-v10 ${r.onUs?'on-us':''}" data-v10106-contract="${item.key}" data-v10-contract="${item.key}"><div class="contract-card-head-v10"><h3>${esc(item.label)}</h3><label class="contract-switch-v10"><input type="checkbox" data-field="onUs" ${r.onUs?'checked':''} ${readonlyMode?'disabled':''} onchange="contractV10106RefreshCore()"><span>العقد علينا</span></label></div><div class="contract-fields-v10 ${r.onUs?'':'muted-off'}"><div><label>اسم الشركة</label><input data-field="company" value="${esc(r.company)}" ${readonlyMode?'disabled':''}></div><div><label>رقم الشركة</label><input data-field="phone" value="${esc(r.phone)}" ${readonlyMode?'disabled':''}></div><div><label>من تاريخ</label><input type="date" data-field="start" value="${esc(r.start)}" ${readonlyMode?'disabled':''}></div><div><label>إلى تاريخ</label><input type="date" data-field="end" value="${esc(r.end)}" ${readonlyMode?'disabled':''}></div><div><label>عدد الزيارات</label><input type="number" min="0" data-field="visits" value="${esc(r.visits)}" ${readonlyMode?'disabled':''} onchange="contractV10106RefreshCore()"></div><div><label>ملاحظات</label><input data-field="notes" value="${esc(r.notes)}" ${readonlyMode?'disabled':''}></div></div><div class="visit-row-v10">${visitChips('contractV10106ToggleCoreVisit',item.key,r.visits,r.done)}</div>${r.onUs&&miss.length?`<div class="contract-warning-v10">ناقص: ${esc(miss.join('، '))}</div>`:''}</section>`;}).join('');}
  function renderAnnual(record){fillAnnualSelect(); const body=$('csAnnualBody'); if(!body) return; if($('csAssocName')) $('csAssocName').value=record.association?.name||''; if($('csAssocManager')) $('csAssocManager').value=record.association?.manager||''; if($('csAssocPhone')) $('csAssocPhone').value=record.association?.phone||''; body.innerHTML=A(record.annual).map(item=>`<tr><td><b>${esc(item.name)}</b>${item.notes?`<br><small>${esc(item.notes)}</small>`:''}</td><td>${N(item.visits)}</td><td><div class="visit-row-v10">${visitChips('contractV10106ToggleAnnualVisit',item.id,item.visits,item.done)}</div></td><td>${A(item.done).length}</td><td>${Math.max(0,N(item.visits)-A(item.done).length)}</td><td>${readonlyMode?'-':`<button class="danger" onclick="contractV10106DeleteAnnual('${esc(item.id)}')">حذف</button>`}</td></tr>`).join('')||'<tr><td colspan="6">لا توجد خدمات سنوية بعد</td></tr>';}
  ...
  window.contractV10106RefreshCore=function(){currentRecord=collectCoreFromScreen(); renderCore(currentRecord); renderReport(currentRecord);};
  window.contractV10106ToggleCoreVisit=function(key,no){if(readonlyMode) return; currentRecord=collectCoreFromScreen(); const r=currentRecord.contracts[key]; if(!r) return; const set=new Set(A(r.done).map(Number)); const n=Number(no); if(set.has(n)) set.delete(n); else set.add(n); r.done=[...set].sort((a,b)=>a-b); putRecord(currentProjectId,currentRecord); renderCore(currentRecord); renderReport(currentRecord);};
  ...
  window.contractV10106ToggleAnnualVisit=function(id,no){if(readonlyMode) return; currentRecord=collectCoreFromScreen(); const item=A(currentRecord.annual).find(x=>S(x.id)===S(id)); if(!item) return; const set=new Set(A(item.done).map(Number)); const n=Number(no); if(set.has(n)) set.delete(n); else set.add(n); item.done=[...set].sort((a,b)=>a-b); putRecord(currentProjectId,currentRecord); renderAnnual(currentRecord); renderReport(currentRecord);};
  window.contractV10106DeleteAnnual=function(id){if(readonlyMode) return; if(!confirm('حذف هذه الخدمة السنوية؟')) return; currentRecord=collectCoreFromScreen(); currentRecord.annual=A(currentRecord.annual).filter(x=>S(x.id)!==S(id)); putRecord(currentProjectId,currentRecord); renderAnnual(currentRecord); renderReport(currentRecord);};
  // Aliases for old inline buttons if any remain.


  // ===== V10106: Smart contractual-services schedule and accurate printing =====
  ...
  function contractualTable(){
    // V10106: المصدر الرسمي للخدمات التعاقدية هو جدول contractServicesBody، وليس تقرير الخدمات السنوية.
  ...
  function injectAnnualRowsIntoContractualTable(){
    ...
    [...tbody.querySelectorAll('[data-v10106-annual-row]')].forEach(x=>x.remove());
    const headers=[...table.querySelectorAll('thead th')].map(th=>th.textContent);
    const rows=annualContractRows(false);
    if(!rows.length) return;
    rows.forEach(row=>{
      const tr=document.createElement('tr');
      tr.dataset.v10106AnnualRow='1';
      tr.className='v10106-annual-linked-row';
  ...
  function currentContractualRows(includeDone){
    ...
    if(table){
      ...
      rows=[...table.querySelectorAll('tbody tr:not([data-v10106-annual-row])')].map(tr=>rowObjFromTr(tr,map)).filter(r=>r.service||r.project);
    }
  ...
  function hideDoneContractualRows(){
    const table=contractualTable(); if(!table || table.dataset.v10106HideInstalled==='1') return;
    table.dataset.v10106HideInstalled='1';
    const apply=()=>{
      const map=tableMap(table);
  ...
  window.printContractServicesSmartV10106=function(){
  ...
  window.printContractServicesSmartV10106=window.printContractServicesSmartV10106;
  window.printContractServicesSmartV10105=window.printContractServicesSmartV10106;
  window.tasneefPrintContractsServicesV10097=window.printContractServicesSmartV10106;
  window.printContractAnnualSmartV10106=function(){
  ...
  window.printContractAnnualSmartV10106=window.printContractAnnualSmartV10106;
  window.printContractAnnualSmartV10105=window.printContractAnnualSmartV10106;
  window.printContractClientTable=window.printContractAnnualSmartV10106;
  function installSmartButtonsV10106(){
    if(!isContractsPageVisible()) return;
    const section=findContractualServicesSection();
    if(section && !section.querySelector('[data-v10106-smart-print]')){
      const btn=document.createElement('button'); btn.type='button'; btn.className='light'; btn.dataset.v10106SmartPrint='1'; btn.textContent='جدول ذكي'; btn.onclick=window.printContractServicesSmartV10106;
    ...
    const reportBtn=$('contractSmartReportTab')?.querySelector?.('button[data-v10106-print]');
    const reportBox=$('contractSmartReportTab');
    if(reportBox && !reportBox.querySelector('[data-v10106-print-contract-report]')){
      const b=document.createElement('button'); b.type='button'; b.className='light'; b.dataset.v10106PrintContractReport='1'; b.textContent='طباعة التقرير الذكي'; b.onclick=window.printContractAnnualSmartV10106; reportBox.insertBefore(b, reportBox.firstChild);
    }
  }
  window.contractV10105RefreshCore=window.contractV10106RefreshCore;
  window.contractV10105ToggleCoreVisit=window.contractV10106ToggleCoreVisit;
  window.contractV10105ToggleAnnualVisit=window.contractV10106ToggleAnnualVisit;
  window.contractV10105DeleteAnnual=window.contractV10106DeleteAnnual;
  window.contractV10RefreshCore=window.contractV10106RefreshCore;
  window.contractV10ToggleCoreVisit=window.contractV10106ToggleCoreVisit;
  window.contractV10ToggleAnnualVisit=window.contractV10106ToggleAnnualVisit;
  window.contractV10DeleteAnnual=window.contractV10106DeleteAnnual;
  ...
  window.saveContractSmartModal=async function(){
    ...
    try{if(btn){btn.disabled=true; btn.textContent='جاري الحفظ...';}
      ...
      const pending=S($('csAnnualCustom')?.value)||S($('csAnnualSelect')?.value&&window.__tasneefAnnualTouchedV10106?$('csAnnualSelect')?.value:'');
  ...
  ['csAnnualSelect','csAnnualCustom','csAnnualVisits'].forEach(id=>document.addEventListener('input',e=>{if(e.target&&e.target.id===id) window.__tasneefAnnualTouchedV10106=true;},true));
  ['csAnnualSelect','csAnnualCustom','csAnnualVisits'].forEach(id=>document.addEventListener('change',e=>{if(e.target&&e.target.id===id) window.__tasneefAnnualTouchedV10106=true;},true));
