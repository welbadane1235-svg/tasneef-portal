/* Tasneef Daily Logs Root V10511 - single source, loaded last */
(function(){
  'use strict';
  if(window.__tasneefDailyRootV10511) return;
  window.__tasneefDailyRootV10511=true;
  ...
  function bindExternalRefresh(){
    window.addEventListener('storage',e=>{if(e.key==='tasneef_time_logs_changed')scheduleReload(150)});
    window.addEventListener('tasneef:time-log-changed',()=>scheduleReload(100));
    try{
      if(window.sb&&window.sb.channel){
        const ch=window.sb.channel('tasneef-daily-root-v10511').on('postgres_changes',{event:'*',schema:'public',table:'time_logs'},()=>scheduleReload(250)).subscribe();
