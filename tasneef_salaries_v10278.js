  ...
  async function exportSalaryExcel(){
    try{
      ...
      rows.forEach((r,idx)=>{
        ...
        // V10278: تطابق أرقام السستم أولًا، ثم تبقى المعادلات قابلة للتعديل داخل Excel.
        // بعض صفوف شهر 6 فيها إجمالي يدوي لا يساوي الأساسي + البدلات، أو أيام عمل يدوية لا تساوي فرق التاريخ.
        // لذلك نضيف فرق تسوية داخل المعادلة حتى يفتح Excel بنفس أرقام السستم بالضبط.
        const startSerialV10278=excelSerialFromIsoDateV10276(r.start_date);
        const endSerialV10278=excelSerialFromIsoDateV10276(r.end_date);
        const dateWorkDaysV10278=(startSerialV10278!==null && endSerialV10278!==null) ? Math.max(0, endSerialV10278-startSerialV10278+1) : 0;
        const workDaysAdjustV10278=Math.round((num(r.work_days)-dateWorkDaysV10278)*100)/100;
        row.getCell(11).value={formula:`IF(OR(I${erow}="",J${erow}=""),0,J${erow}-I${erow}+1)${excelFormulaAdjustmentV10278(workDaysAdjustV10278)}`, result:num(r.work_days)};
        row.getCell(13).value={formula:`MAX(0,K${erow}-L${erow})`, result:num(r.payable_days)};
        const grossAdjustV10278=Math.round((num(r.gross_salary)-(num(r.basic_salary)+num(r.allowance)))*100)/100;
        row.getCell(16).value={formula:`N${erow}+O${erow}${excelFormulaAdjustmentV10278(grossAdjustV10278)}`, result:num(r.gross_salary)};
        const computedPeriodV10278=Math.round((num(r.gross_salary)/monthDays*num(r.work_days))*100)/100;
        const periodAdjustV10278=Math.round((num(r.salary_by_days)-computedPeriodV10278)*100)/100;
        row.getCell(17).value={formula:`ROUND(P${erow}/${monthDays}*K${erow},2)${excelFormulaAdjustmentV10278(periodAdjustV10278)}`, result:num(r.salary_by_days)};
        const absenceDeductionBase=Math.round((num(r.gross_salary)/monthDays*num(r.absent_days))*100)/100;
        const deductionAdjustV10278=Math.round((num(r.deductions)-absenceDeductionBase)*100)/100;
        row.getCell(19).value={formula:`ROUND(P${erow}/${monthDays}*L${erow},2)${excelFormulaAdjustmentV10278(deductionAdjustV10278)}`, result:num(r.deductions)};
        const expectedNetV10278=Math.round((num(r.salary_by_days)+num(r.commission)-num(r.deductions)+num(r.rounding)-num(r.advance_deduction))*100)/100;
        const netAdjustV10278=Math.round((num(r.net_salary)-expectedNetV10278)*100)/100;
        row.getCell(22).value={formula:`Q${erow}+R${erow}-S${erow}+T${erow}-U${erow}${excelFormulaAdjustmentV10278(netAdjustV10278)}`, result:num(r.net_salary)};
      ...
      ws.getCell(`A${signRow+2}`).value='ملاحظة V10278: التصدير يطابق أرقام السستم لحظة التصدير، مع بقاء المعادلات قابلة للتعديل داخل Excel. تم تثبيت فروقات التسوية اليدوية حتى لا يتغير الصافي عند فتح الملف.';
