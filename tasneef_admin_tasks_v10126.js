...
(function(){
  ...
  const VERSION='v10145-admin-tasks-smart-strips-actions';
  ...
  function injectCss(){
    ...
    const st=document.createElement('style'); st.id='at26Style'; st.textContent=`
      .at26-smart-list{display:flex;flex-direction:column;gap:10px;margin-top:16px}.at26-task-strip{position:relative;display:grid;grid-template-columns:minmax(220px,1.2fr) minmax(280px,2fr) auto;gap:14px;align-items:center;border:1px solid #dce6e2;border-right:7px solid #0A4033;border-radius:18px;background:#fff;padding:13px 14px;box-shadow:0 10px 24px rgba(10,64,51,.07);overflow:hidden}.at26-task-strip.open{border-right-color:#c53030}.at26-task-strip.closed{border-right-color:#b7791f}.at26-task-strip.approved{border-right-color:#167a4c}.at26-strip-main{min-width:0}.at26-strip-title{font-weight:950;color:#0A4033;font-size:18px;line-height:1.45;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.at26-strip-sub{display:flex;gap:7px;flex-wrap:wrap;margin-top:7px}.at26-strip-info{display:grid;grid-template-columns:repeat(4,minmax(95px,1fr));gap:8px}.at26-strip-info div{background:#f7fbfa;border:1px solid #edf2f0;border-radius:12px;padding:8px;min-height:52px}.at26-strip-info small{display:block;color:#61736d;font-weight:800;font-size:11px;margin-bottom:3px}.at26-strip-info b{display:block;color:#10231d;font-size:13px;line-height:1.45;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.at26-strip-details{margin-top:8px;color:#33443e;line-height:1.6;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;border:1px dashed #d6e3df;border-radius:12px;padding:8px;background:#fbfdfc}.at26-strip-actions{display:flex;gap:7px;flex-wrap:wrap;justify-content:flex-end;min-width:250px}.at26-strip-actions .at26-btn{border-radius:11px;padding:9px 12px;font-weight:950;box-shadow:0 6px 14px rgba(10,64,51,.08)}@media(max-width:1000px){.at26-task-strip{grid-template-columns:1fr}.at26-strip-actions{min-width:0;justify-content:flex-start}.at26-strip-info{grid-template-columns:repeat(2,1fr)}}
  ...
  function render(){
    ...
    body.innerHTML=list.map(t=>{
      ...
      return `<div class="at26-task-strip ${esc(t.status||'open')}" data-id="${esc(t.id)}"><div class="at26-strip-main"><div class="at26-strip-title">${esc(t.title)}</div><div class="at26-strip-sub">${statusLabel(t.status)}<span class="at26-badge">${esc(t.priority||'—')}</span><span class="at26-badge">${esc(t.schedule_type||'—')}</span></div><div class="at26-strip-details">${esc(t.details||'بدون تفاصيل')}${t.approved_note?' — ملاحظة: '+esc(t.approved_note):''}</div></div><div class="at26-strip-info"><div><small>من</small><b>${esc(t.from_name||t.from_user||'—')}</b></div><div><small>إلى</small><b>${esc(t.to_name||t.to_user||'—')}</b></div><div><small>الجدولة</small><b style="color:${late?'#a32626':'#10231d'}">${formatDate(t.due_at)}</b></div><div><small>أنشئت</small><b>${formatDate(t.created_at)}</b></div></div><div class="at26-strip-actions">${actions.join(' ')}</div></div>`;
    }).join('');
