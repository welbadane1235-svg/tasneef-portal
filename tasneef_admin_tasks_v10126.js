...
(function(){
  ...
  function injectCss(){
    ...
    const st=document.createElement('style'); st.id='at26Style'; st.textContent=`
      ...
      .at26-cards-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:12px;margin-top:12px}.at26-task-card{border:1px solid #dce6e2;border-radius:18px;background:linear-gradient(180deg,#fff,#fbfdfc);padding:14px;box-shadow:0 8px 22px rgba(10,64,51,.06);display:flex;flex-direction:column;gap:10px}.at26-task-top{display:flex;justify-content:space-between;gap:8px;align-items:flex-start}.at26-task-title{font-weight:900;color:#0A4033;font-size:17px;line-height:1.5}.at26-task-meta{display:grid;grid-template-columns:1fr 1fr;gap:8px}.at26-task-meta div{background:#f7fbfa;border:1px solid #edf2f0;border-radius:12px;padding:8px}.at26-task-meta small{display:block;color:#61736d;margin-bottom:3px}.at26-task-meta b{color:#10231d}.at26-task-details{white-space:pre-wrap;line-height:1.8;background:#fff;border:1px dashed #d6e3df;border-radius:14px;padding:10px;color:#233}.at26-task-actions{display:flex;gap:8px;flex-wrap:wrap;justify-content:flex-end;margin-top:auto}.at26-modal{display:none;position:fixed;inset:0;background:rgba(0,0,0,.45);z-index:999999;align-items:center;justify-content:center;padding:16px}.at26-modal.show{display:flex}.at26-box{background:#fff;border-radius:22px;max-width:560px;width:min(96vw,560px);overflow:hidden;box-shadow:0 20px 70px rgba(0,0,0,.25)}.at26-box h3{margin:0;background:#0A4033;color:#fff;padding:18px}.at26-box-body{padding:18px;line-height:1.8}.at26-box-actions{display:flex;justify-content:flex-end;gap:8px;padding:0 18px 18px}
  ...
  function render(){
    ...
    body.innerHTML=list.map(t=>{
      ...
      return `<div class="at26-task-card" data-id="${esc(t.id)}"><div class="at26-task-top"><div class="at26-task-title">${esc(t.title)}</div><div>${statusLabel(t.status)}</div></div><div class="at26-task-meta"><div><small>من</small><b>${esc(t.from_name||t.from_user||'—')}</b></div><div><small>إلى</small><b>${esc(t.to_name||t.to_user||'—')}</b></div><div><small>الأولوية</small><b>${esc(t.priority||'—')}</b></div><div><small>الجدولة</small><b style="color:${late?'#a32626':'#10231d'}">${esc(t.schedule_type||'—')}<br>${formatDate(t.due_at)}</b></div><div><small>أنشئت</small><b>${formatDate(t.created_at)}</b></div><div><small>أغلقت / اعتمدت</small><b>${formatDate(t.closed_at)}<br>${formatDate(t.approved_at)}</b></div></div><div class="at26-task-details">${esc(t.details||'بدون تفاصيل')}${t.approved_note?'\n\nملاحظة: '+esc(t.approved_note):''}</div><div class="at26-task-actions">${actions.join(' ')||'<span class="at26-muted">لا يوجد إجراء متاح</span>'}</div></div>`;
    }).join('');
