...
(function(){
  ...
  function currentQtyForOutMovement(m){
    ...
    if(dist.length){
      ...
      return Math.max(0, N(m&&m.quantity)-used+returned);
    }
    const pid=S(m&&m.parent_id || m&&m.id);
    if(!pid) return N(m&&m.quantity);
    let used=0, returned=0;
    A(state().movements).forEach(x=>{
      const same=S(x.parent_id||x.source_movement_id||x.original_movement_id||'')===pid;
      if(!same) return;
  ...
  function printRows(selectedOnly){
    ...
    const bodyRows=rows.map((r,idx)=>{
      ...
      return `<tr>
        ...
        ${needsProjectCurrent?`<td>${esc(projectNameForMovement(r))}</td><td>${esc(orderNoForMovement(r))}</td><td>${N(currentQtyForItem(item,r))}</td>`:''}
        ...
        <table>
          <thead>
            <tr>
              ...
              ${needsProjectCurrent?'<th>اسم المشروع</th><th>الأوردر</th><th>الكمية الحالية</th>':''}
              <th>نوع المنتج</th>
              <th>كود الموزع</th>
              <th>الكمية</th>
              <th>سعر الحبة</th>
